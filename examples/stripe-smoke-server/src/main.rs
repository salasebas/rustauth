//! Minimal RustAuth + Stripe server for test-mode smoke (see crates/rustauth-stripe/SMOKE.md).
//!
//! Webhook signing secret comes from a local `stripe listen` child process (test mode only).
//! Other secrets still come from the environment (`STRIPE_SECRET_KEY`, prices, `RUSTAUTH_SECRET`).
//!
//! ```bash
//! set -a && source .env && set +a   # optional: API key + price IDs only
//! cargo run -p rustauth-example-stripe-smoke
//! ```
...
use rustauth_stripe::{stripe, StripeClient};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let host = env::var("RUSTAUTH_EXAMPLE_HOST").unwrap_or_else(|_| "127.0.0.1".to_owned());
    let secret = env::var("RUSTAUTH_SECRET")
    ...
    // Serve with ConnectInfo so RustAuth rate limiting sees the real peer IP.
...
fn spawn_stripe_listen(webhook_url: &str) -> Result<Child, Box<dyn std::error::Error>> {
    // Spawn the Stripe CLI directly (no shell) so a hostile RUSTAUTH_EXAMPLE_HOST cannot
