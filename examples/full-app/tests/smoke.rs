...
fn with_client_ip(request: Request<Body>) -> Request<Body> {
    rustauth_example_full_app::smoke_request(request)
}

#[tokio::test]
async fn home_page_responds_ok() -> Result<(), Box<dyn std::error::Error>> {
    let response = rustauth_example_full_app::app()
...
{
    let app = rustauth_example_full_app::app();
    let response = app
        .clone()
    ...
    assert!(response
        ...
        .any(|value| value
            .to_str()
            .is_ok_and(|cookie| cookie.starts_with("rustauth-memory.session_token="))));
...
async fn home_page_exposes_profile_selectors() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(html.contains(r#"value="postgres-sqlx""#));
    assert!(html.contains(r#"value="postgres-deadpool""#));
    assert!(html.contains(r#"value="mysql-sqlx""#));
    ...
    let request = || -> Result<Request<Body>, Box<dyn std::error::Error>> {
        Ok(with_client_ip(
            Request::builder()
                ...
                .header("x-rustauth-example-rate-enabled", "true")
                .header("x-rustauth-example-rate-window", "60")
                .header("x-rustauth-example-rate-max", "2")
                .body(Body::empty())?,
        ))
    };
...
{
    let app = rustauth_example_full_app::app_hardened();
    ...
    let request = || -> Result<Request<Body>, Box<dyn std::error::Error>> {
        Ok(with_client_ip(
            Request::builder()
                ...
                .header("x-rustauth-example-rate-enabled", "true")
                .header("x-rustauth-example-rate-window", "60")
                .header("x-rustauth-example-rate-max", "1")
                .body(Body::empty())?,
        ))
    };
