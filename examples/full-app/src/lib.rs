...
use rustauth::options::{
    AdvancedOptions, EmailPasswordOptions, HybridRateLimitOptions, RustAuthOptions,
    RateLimitOptions, RateLimitRule, RateLimitStore,
};
use rustauth::plugin::AuthPlugin;
use rustauth::rate_limit::GovernorMemoryRateLimitStore;
...
const DEFAULT_SECRET: &str = "rustauth-example-dev-secret-at-least-32-chars";
const RATE_LIMIT_WINDOW_HEADER: &str = "x-rustauth-example-rate-window";
const RATE_LIMIT_MAX_HEADER: &str = "x-rustauth-example-rate-max";
const RATE_LIMIT_ENABLED_HEADER: &str = "x-rustauth-example-rate-enabled";
const PREFERENCES_KEY: &str = "rustauth:full-app:preferences";
...
impl DbBackend {
    ...
    fn default_database_url(self) -> String {
        match self {
            ...
            Self::Sqlite => "sqlite://examples/full-app/data/rustauth.sqlite".to_owned(),
            Self::PostgresSqlx | Self::PostgresDeadpool => {
                "postgres://user:password@127.0.0.1:5432/rustauth".to_owned()
            }
            Self::MysqlSqlx => "mysql://user:password@127.0.0.1:3306/rustauth".to_owned(),
        }
    }
    ...
    fn per_backend_database_url_env(self) -> Option<&'static str> {
        match self {
            Self::Memory => None,
            Self::Sqlite => Some("RUSTAUTH_EXAMPLE_SQLITE_DATABASE_URL"),
            Self::PostgresSqlx | Self::PostgresDeadpool => {
                Some("RUSTAUTH_EXAMPLE_POSTGRES_DATABASE_URL")
            }
            Self::MysqlSqlx => Some("RUSTAUTH_EXAMPLE_MYSQL_DATABASE_URL"),
        }
    }
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value {
            ...
            other => Err(ExampleError::InvalidConfig(format!(
                "unsupported RUSTAUTH_EXAMPLE_DB `{other}`; use memory, sqlite, postgres-sqlx, postgres-deadpool, or mysql-sqlx"
            ))),
        }
    }
}
...
impl FromStr for RateLimitBackend {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value {
            ...
            other => Err(ExampleError::InvalidConfig(format!(
                "unsupported RUSTAUTH_EXAMPLE_RATE_LIMIT `{other}`"
            ))),
        }
    }
}
...
pub struct ExampleConfig {
    ...
    /// backend) and optional `RUSTAUTH_EXAMPLE_*_DATABASE_URL` overrides.
    ...
    /// only on for loopback binds unless `RUSTAUTH_EXAMPLE_DEV_CONTROLS` is set.
    pub dev_controls: bool,
}

impl ExampleConfig {
    pub fn from_env() -> Result<Self, ExampleError> {
        let host = env_or("RUSTAUTH_EXAMPLE_HOST", "127.0.0.1");
        let port = env_or("RUSTAUTH_EXAMPLE_PORT", "3000")
            .parse::<u16>()
            .map_err(|error| {
                ExampleError::InvalidConfig(format!("RUSTAUTH_EXAMPLE_PORT is invalid: {error}"))
            })?;
        let default_base_url = format!("http://{host}:{port}{AUTH_BASE_PATH}");
        let base_url = env::var("RUSTAUTH_EXAMPLE_BASE_URL").unwrap_or(default_base_url);
        validate_example_base_url(&base_url)?;
        let secret = env::var("RUSTAUTH_SECRET").unwrap_or_else(|_| DEFAULT_SECRET.to_owned());
        let db = env_or("RUSTAUTH_EXAMPLE_DB", "sqlite").parse::<DbBackend>()?;
        let rate_limit =
            env_or("RUSTAUTH_EXAMPLE_RATE_LIMIT", "memory").parse::<RateLimitBackend>()?;
        let rate_limit_enabled = env_or("RUSTAUTH_EXAMPLE_RATE_LIMIT_ENABLED", "true")
            .parse::<bool>()
            .map_err(|error| {
                ExampleError::InvalidConfig(format!(
                    "RUSTAUTH_EXAMPLE_RATE_LIMIT_ENABLED is invalid: {error}"
                ))
            })?;
        let rate_limit_window = env_or("RUSTAUTH_EXAMPLE_RATE_LIMIT_WINDOW", "60")
            .parse::<u64>()
            .map_err(|error| {
                ExampleError::InvalidConfig(format!(
                    "RUSTAUTH_EXAMPLE_RATE_LIMIT_WINDOW is invalid: {error}"
                ))
            })?;
        let rate_limit_max = env_or("RUSTAUTH_EXAMPLE_RATE_LIMIT_MAX", "120")
            .parse::<u64>()
            .map_err(|error| {
                ExampleError::InvalidConfig(format!(
                    "RUSTAUTH_EXAMPLE_RATE_LIMIT_MAX is invalid: {error}"
                ))
            })?;
        ...
        let dev_controls = match env::var("RUSTAUTH_EXAMPLE_DEV_CONTROLS") {
            Ok(value) => value.parse::<bool>().map_err(|error| {
                ExampleError::InvalidConfig(format!(
                    "RUSTAUTH_EXAMPLE_DEV_CONTROLS is invalid: {error}"
                ))
            })?,
            ...
            db => {
                ...
                if self.allow_default_database_urls {
                    ...
                    Err(ExampleError::InvalidConfig(format!(
                        "no database URL configured for backend `{}`; set DATABASE_URL when it is the startup backend (`RUSTAUTH_EXAMPLE_DB`) or `{}`",
                        db.as_str(),
                        db.per_backend_database_url_env()
                            .unwrap_or("RUSTAUTH_EXAMPLE_DATABASE_URL"),
                    )))
                }
            }
        }
...
impl From<RustAuthError> for ExampleError {
    fn from(error: RustAuthError) -> Self {
        Self::RustAuth(error)
    }
}

impl From<rustauth_axum::RustAuthAxumError> for ExampleError {
...
#[derive(Default)]
struct ProfileCache {
    entries: tokio::sync::Mutex<HashMap<ProfileKey, Arc<RustAuth>>>,
...
impl ProfileCache {
    async fn get_or_insert<F, Fut>(
        ...
        key: ProfileKey,
        build: F,
    ) -> Result<Arc<RustAuth>, ExampleError>
    where
        F: FnOnce() -> Fut,
        Fut: Future<Output = Result<RustAuth, ExampleError>>,
    {
...
struct AppState {
    ...
    memory_adapter: rustauth::db::MemoryAdapter,
...
#[doc(hidden)]
pub fn smoke_request<B>(request: axum::http::Request<B>) -> axum::http::Request<B> {
    rustauth_axum::test_utils::with_loopback_client_ip(request)
}
    ...
    match config.db {
        ...
        DbBackend::Sqlite => {
            ...
            let auth = build_auth(
                config.clone(),
                AUTH_BASE_PATH.to_owned(),
                stores.adapter.clone(),
                Some(ConnectedSqlStores::Sqlite(&stores)),
                memory_rate_limit_store.clone(),
            )
            .await?;
            Ok(router_with_auth(auth, &config, memory_rate_limit_store).await?)
        ...
        DbBackend::PostgresSqlx => {
            ...
            let auth = build_auth(
                ...
                AUTH_BASE_PATH.to_owned(),
                stores.adapter.clone(),
                Some(ConnectedSqlStores::PostgresSqlx(&stores)),
                memory_rate_limit_store.clone(),
            )
            .await?;
            Ok(router_with_auth(auth, &config, memory_rate_limit_store).await?)
        ...
        DbBackend::PostgresDeadpool => {
            ...
            let auth = build_auth(
                ...
                AUTH_BASE_PATH.to_owned(),
                stores.adapter.clone(),
                Some(ConnectedSqlStores::PostgresDeadpool(&stores)),
                memory_rate_limit_store.clone(),
            )
            .await?;
            Ok(router_with_auth(auth, &config, memory_rate_limit_store).await?)
        ...
        DbBackend::MysqlSqlx => {
            ...
            let auth = build_auth(
                ...
                AUTH_BASE_PATH.to_owned(),
                stores.adapter.clone(),
                Some(ConnectedSqlStores::Mysql(&stores)),
                memory_rate_limit_store.clone(),
            )
            .await?;
            Ok(router_with_auth(auth, &config, memory_rate_limit_store).await?)
...
fn example_plugins() -> Result<Vec<AuthPlugin>, ExampleError> {
    ...
    struct ExampleEmailOtpSender;

    impl SendEmailOtp for ExampleEmailOtpSender {
        fn send_email_otp(
            &self,
    ...
    Ok(vec![
        ...
        device_authorization(DeviceAuthorizationOptions::default())?,
        email_otp(EmailOtpOptions {
            sender: Some(Arc::new(ExampleEmailOtpSender)),
        ...
        jwt(JwtOptions::default()).map_err(ExampleError::from)?,
        ...
        multi_session(MultiSessionOptions::default()),
        oauth_proxy(OAuthProxyOptions::default()),
        one_tap(OneTapOptions::default()),
        one_time_token(OneTimeTokenOptions::default()),
        ...
        siwe(SiweOptions::new(
            "localhost",
            || async { Ok("rustauth-example-nonce".to_owned()) },
            |_args| async { Ok(true) },
        ))
        .map_err(ExampleError::from)?,
        two_factor(TwoFactorOptions::default()),
        username(UsernameOptions::default()),
        passkey(PasskeyOptions::default()),
        sso(SsoOptions::default()),
        scim(ScimOptions::default()),
        stripe(
            StripeOptions::new(StripeClient::new("sk_test"), "whsec_rustauth_example_dev")
...
fn example_db_schema(
    adapter: Arc<dyn rustauth::db::DbAdapter>,
) -> Result<rustauth::db::DbSchema, ExampleError> {
    Ok(create_auth_context_with_adapter(example_rustauth_options()?, adapter)?.db_schema)
}

/// Plugin-augmented schema used by the example's SQL adapters and viewer.
fn example_auth_schema() -> Result<rustauth::db::DbSchema, ExampleError> {
    example_db_schema(Arc::new(rustauth::db::MemoryAdapter::new()))
}
...
async fn connect_sql_adapter(
    db: DbBackend,
    database_url: &str,
) -> Result<Arc<dyn rustauth::db::DbAdapter>, ExampleError> {
    let adapter: Arc<dyn rustauth::db::DbAdapter> = match db {
...
fn example_rustauth_options() -> Result<RustAuthOptions, ExampleError> {
...
enum ConnectedSqlStores<'a> {
    Sqlite(&'a SqliteStores),
    PostgresSqlx(&'a PostgresStores),
    PostgresDeadpool(&'a DeadpoolPostgresStores),
    Mysql(&'a MySqlStores),
}

impl ConnectedSqlStores<'_> {
    fn apply_to_options(&self, options: RustAuthOptions) -> RustAuthOptions {
        match self {
            Self::Sqlite(stores) => stores.apply_to_options(options),
            Self::PostgresSqlx(stores) => stores.apply_to_options(options),
            Self::PostgresDeadpool(stores) => stores.apply_to_options(options),
            Self::Mysql(stores) => stores.apply_to_options(options),
        }
    }

    fn database_rate_limit(&self) -> RateLimitOptions {
        match self {
            Self::Sqlite(stores) => RateLimitOptions::database(stores.rate_limit.clone()),
            Self::PostgresSqlx(stores) => RateLimitOptions::database(stores.rate_limit.clone()),
            Self::PostgresDeadpool(stores) => RateLimitOptions::database(stores.rate_limit.clone()),
            Self::Mysql(stores) => RateLimitOptions::database(stores.rate_limit.clone()),
        }
    }
}

async fn wire_auth_options(
    config: &ExampleConfig,
    auth_base_path: &str,
    sql_stores: Option<ConnectedSqlStores<'_>>,
    memory_rate_limit_store: Arc<GovernorMemoryRateLimitStore>,
) -> Result<RustAuthOptions, ExampleError> {
    let options = example_rustauth_options()?
        .base_url(auth_base_url_for_path(&config.base_url, auth_base_path)?)
        .base_path(auth_base_path.to_owned())
        .secret(config.secret.clone())
        .advanced(AdvancedOptions::new().cookie_prefix(cookie_prefix(config.db)));

    let options = match config.rate_limit {
        RateLimitBackend::Memory => options.rate_limit(
            rate_limit_defaults(config, RateLimitOptions::memory())
                .custom_store_arc(memory_rate_limit_store as Arc<dyn RateLimitStore>),
        ),
        RateLimitBackend::Database => {
            let stores = sql_stores.ok_or_else(|| {
                ExampleError::InvalidConfig(
                    "database rate limiting requires a SQL RUSTAUTH_EXAMPLE_DB backend".to_owned(),
                )
            })?;
            let options = stores.apply_to_options(options);
            options.rate_limit(rate_limit_defaults(config, stores.database_rate_limit()))
        }
        RateLimitBackend::Redis => {
            let stores = RedisStores::connect(&config.redis_url).await?;
            let options = stores.apply_to_options(options);
            options.rate_limit(rate_limit_defaults(
                config,
                RateLimitOptions::secondary_storage(stores.rate_limit.clone()),
            ))
        }
        RateLimitBackend::Valkey => {
            let stores = RedisStores::connect(&config.valkey_url).await?;
            let options = stores.apply_to_options(options);
            options.rate_limit(rate_limit_defaults(
                config,
                RateLimitOptions::secondary_storage(stores.rate_limit.clone()),
        ...
        RateLimitBackend::FredRedis => {
            let stores = FredStores::connect(&config.redis_url).await?;
            let options = stores.apply_to_options(options);
            options.rate_limit(rate_limit_defaults(
                config,
                RateLimitOptions::secondary_storage(stores.rate_limit.clone()),
        ...
        RateLimitBackend::FredValkey => {
            let stores = FredStores::connect(&config.valkey_url).await?;
            let options = stores.apply_to_options(options);
            options.rate_limit(rate_limit_defaults(
                config,
                RateLimitOptions::secondary_storage(stores.rate_limit.clone()),
...
async fn build_auth<A>(
    ...
    sql_stores: Option<ConnectedSqlStores<'_>>,
    memory_rate_limit_store: Arc<GovernorMemoryRateLimitStore>,
) -> Result<RustAuth, ExampleError>
where
    A: rustauth::db::DbAdapter + 'static,
{
    let options = wire_auth_options(
        &config,
        &auth_base_path,
        sql_stores,
...
fn rate_limit_defaults(config: &ExampleConfig, options: RateLimitOptions) -> RateLimitOptions {
    let window = time::Duration::seconds(
        i64::try_from(config.rate_limit_window).unwrap_or(i64::MAX),
    );
    let rule = RateLimitRule::new(window, config.rate_limit_max);
    options
        .enabled(config.rate_limit_enabled)
        .window(window)
        .max(config.rate_limit_max)
        .missing_ip_policy(rustauth::options::MissingIpPolicy::SharedBucket)
        .custom_rule("/sign-in/*", rule.clone())
        .custom_rule("/sign-up/*", rule)
}

fn cookie_prefix(db: DbBackend) -> String {
    format!("rustauth-{}", db.as_str())
}
...
async fn router_with_auth(
    ...
    let rustauth_routes = auth.mount_routes(RustAuthAxumOptions::default())?;
    ...
    .nest(AUTH_BASE_PATH, rustauth_routes))
}
...
fn static_app_with_data(
    ...
    let state = AppState {
        ...
        openapi,
        services,
        memory_adapter: rustauth::db::MemoryAdapter::new(),
        memory_rate_limit_store,
...
<head>
  ...
  <title>RustAuth API Reference</title>
  <style>
    html, body {{
      margin: 0;
      min-height: 100dvh;
...
<body class="docs-page">
  <header class="docs-topbar">
    <a class="docs-back" href='/#openapi' title="Back to RustAuth example">
...
async fn dynamic_auth_handler(
    ...
    {
        Ok(auth) => {
            rustauth_axum::handle(auth.as_ref(), RustAuthAxumOptions::default(), request).await
        }
        Err(error) => json_error(StatusCode::BAD_GATEWAY, &error.to_string()),
    }
}
...
/// `RUSTAUTH_EXAMPLE_DEV_CONTROLS=true`) expose the database viewer, schema
...
fn control_plane_guard(state: &AppState) -> Option<Response> {
    ...
    Some(json_error(
        ...
        "example control endpoints are disabled; set RUSTAUTH_EXAMPLE_DEV_CONTROLS=true for local development",
    ))
}
...
async fn reset_sqlite_schema(config: &ExampleConfig) -> Result<(), ExampleError> {
    ...
    sqlx::query("PRAGMA foreign_keys = ON")
        .execute(&pool)
        .await?;
    let adapter = Arc::new(open_sqlite_adapter(&config.database_url).await?);
...
async fn reset_postgres_schema(config: &ExampleConfig) -> Result<(), ExampleError> {
    ...
    let adapter = Arc::new(open_postgres_sqlx_adapter(&config.database_url).await?);
    rebuild_auth_schema(config.clone(), config.db, adapter, None).await
}
...
async fn reset_mysql_schema(config: &ExampleConfig) -> Result<(), ExampleError> {
    ...
    let adapter = Arc::new(open_mysql_sqlx_adapter(&config.database_url).await?);
    rebuild_auth_schema(config.clone(), config.db, adapter, None).await
}
    ...
    A: DbAdapter + 'static,
{
    ...
    adapter.run_migrations(&schema).await?;
    adapter
        .run_plugin_migrations(&auth.context().plugin_migrations)
        .await?;
    Ok(())
}
...
async fn read_table(
    ...
    {
        Ok(rows) => rows,
        Err(RustAuthError::TableNotFound { .. }) => Vec::new(),
...
where
    A: DbAdapter,
{
    let schema = example_db_schema(Arc::new(rustauth::db::MemoryAdapter::new()))?;
    let mut deleted = 0;
...
fn viewer_schema() -> rustauth::db::DbSchema {
...
fn table_columns(table: &rustauth::db::DbTable) -> Vec<TableColumnView> {
    table
        .fields
        .iter()
...
fn selected_columns(columns: Option<&str>, table: &rustauth::db::DbTable) -> Vec<String> {
    let columns = columns
        .unwrap_or_default()
        .split(',')
        .map(str::trim)
...
fn validate_example_base_url(base_url: &str) -> Result<(), ExampleError> {
    ...
    if url.cannot_be_a_base() || url.host_str().is_none() {
        return Err(ExampleError::InvalidConfig(
            "RUSTAUTH_EXAMPLE_BASE_URL must be an absolute URL with a host".to_owned(),
        ));
    }
    Ok(())
}
...
fn parse_example_base_url(base_url: &str) -> Result<url::Url, ExampleError> {
    url::Url::parse(base_url).map_err(|error| {
        ExampleError::InvalidConfig(format!("RUSTAUTH_EXAMPLE_BASE_URL is invalid: {error}"))
    })
}
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>RustAuth full app example</title>
...
<body data-auth-root="/api/example/auth" data-rate-limit-enabled="{rate_limit_enabled}" data-rate-limit-window="{rate_limit_window}" data-rate-limit-max="{rate_limit_max}">
  ...
  <main class="layout">
    ...
    <section id="overview" class="panel active">
      ...
      <div class="grid metrics">
        <div><span>RustAuth</span><strong>{version}</strong></div>
    ...
    <section id="storage" class="panel">
      ...
      <p class="note">Use <code>RUSTAUTH_EXAMPLE_DB</code> with <code>memory</code>, <code>sqlite</code>, <code>postgres-sqlx</code>, <code>postgres-deadpool</code>, or <code>mysql-sqlx</code>.</p>
    </section>
      ...
      <div class="columns">
        ...
        <div class="box">
          ...
          <dl class="details compact-details">
            ...
            <dt>Cookie prefix</dt><dd><code id="settings-cookie-prefix">rustauth-{db}</code></dd>
          </dl>
    ...
    <section id="openapi" class="panel">
      ...
      <div class="actions">
        <a class="button ghost" href="/api/example/openapi.json" download="rustauth-openapi.json">Download spec</a>
  ...
  <dialog id="signout-dialog">
    <form method="dialog" class="dialog-card">
      <h2>Sign out?</h2>
      <p>This will clear the current RustAuth session cookie for this browser.</p>
      <menu>
  ...
  <dialog id="drop-dialog">
    <form method="dialog" class="dialog-card">
      ...
      <p id="drop-dialog-copy">This will reset RustAuth tables for the selected adapter and run migrations again.</p>
      <menu>
        <button value="cancel" class="button ghost">Cancel</button>
  ...
  <script src="/app.js"></script>
        ...
        version = escape_html(&state.runtime.rustauth_version),
...
function savedTheme() {
  return localStorage.getItem("rustauth-example-theme") || "system";
}
...
function applyTheme(theme) {
  ...
  localStorage.setItem("rustauth-example-theme", selected);
...
function savedRateLimitSettings() {
  try {
    return JSON.parse(localStorage.getItem("rustauth-example-rate-limit") || "null");
  } catch (_error) {
    return null;
  }
}
...
function rateHeaders() {
  return {
    "x-rustauth-example-rate-enabled": String(rateSettings.enabled),
    "x-rustauth-example-rate-window": String(rateSettings.window),
    "x-rustauth-example-rate-max": String(rateSettings.max),
  };
}
...
function updateProfile(options = {}) {
  ...
  if (settingsCookiePrefix) settingsCookiePrefix.textContent = `rustauth-${dbSelect?.value || "sqlite"}`;
  hideSidebarUser();
  void refreshSession();
...
function initials(user) {
  const source = user?.name || user?.email || "RustAuth";
  return source
    .split(/[\s@._-]+/)
    .filter(Boolean)
    .slice(0, 2)
...
document.getElementById("rate-settings-form")?.addEventListener("submit", async (event) => {
  ...
  await withLoading(button, "settings-output", async () => {
    ...
    localStorage.setItem("rustauth-example-rate-limit", JSON.stringify(rateSettings));
    hydrateSettingsForm();
...
document.getElementById("drop-database")?.addEventListener("click", async (event) => {
  if (dropDialogCopy) dropDialogCopy.textContent = `This will reset RustAuth tables for ${studioDb?.value || "sqlite"} and run migrations again.`;
...
mod tests {
    ...
    async fn fred_profiles_wire_secondary_storage_with_rate_limit() -> Result<(), ExampleError> {
        ...
        assert!(
            ...
            "fred profiles should demonstrate FredStores, not only FredRateLimitStore"
        );
        assert!(auth.context().rate_limit.custom_store.is_some());
        assert_eq!(
            auth.context().rate_limit.storage,
            rustauth::options::RateLimitStorageOption::SecondaryStorage
        );
        Ok(())
    }
    ...
    fn invalid_example_base_url_is_rejected() {
        ...
        assert!(matches!(
            ...
            Err(ExampleError::InvalidConfig(message))
                if message.contains("RUSTAUTH_EXAMPLE_BASE_URL is invalid")
        ));
    ...
    async fn dynamic_profiles_do_not_replace_configured_database_urls(
        ...
        let sqlite_path = std::env::temp_dir().join(format!(
            "rustauth-ope150-profile-{}-{nanos}.sqlite",
            std::process::id()
        ));
        let custom_sqlite = format!("sqlite://{}", sqlite_path.display());
        let custom_postgres = "postgres://custom:pass@127.0.0.1:5432/custom_rustauth".to_owned();
    ...
    async fn viewer_routes_reuse_selected_backend_config_without_default_fallback(
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        let sqlite_path = std::env::temp_dir().join(format!(
            "rustauth-ope150-viewer-{}-{nanos}.sqlite",
            std::process::id()
        ));
        let custom_sqlite = format!("sqlite://{}", sqlite_path.display());
        ensure_sqlite_parent(&test_example_config(
        ...
        let adapter = Arc::new(SqliteAdapter::connect(&custom_sqlite).await?);
        let auth = build_auth(
            config.clone(),
            profile_base_path(DbBackend::Sqlite, RateLimitBackend::Memory),
            Arc::clone(&adapter),
        ...
        .await?;
        adapter
            .run_migrations(&auth.context().db_schema)
            .await?;
        drop(auth);
    ...
    async fn seed_populates_memory_tables() -> Result<(), ExampleError> {
        let adapter = rustauth::db::MemoryAdapter::new();
        let schema = viewer_schema();
        let password_hash = seed::seed_password_hash()?;
        ...
        let users = adapter
            .count(rustauth::db::Count::new("user"))
            .await
            .map_err(ExampleError::from)?;
        assert!(users >= 1);
        Ok(())
    ...
    async fn seed_populates_sqlite_tables() -> Result<(), ExampleError> {
        ...
        let path = std::env::temp_dir().join(format!(
            "rustauth-example-seed-{}-{nanos}.sqlite",
            std::process::id()
        ));
    ...
    async fn mysql_seed_and_schema_reset_integration() -> Result<(), ExampleError> {
        ...
        const LEGACY_TABLE: &str = "rustauth_example_legacy_reset_probe";
    ...
    async fn schema_reset_rejects_unconfigured_alternate_backend() -> Result<(), ExampleError> {
        let config = test_example_config(
            DbBackend::PostgresSqlx,
            "postgres://custom:pass@127.0.0.1:5432/custom_rustauth".to_owned(),
        );
        let state = test_app_state(config);
    ...
    async fn dynamic_profile_cache_reuses_auth_instances() -> Result<(), ExampleError> {
        ...
        for _ in 0..5 {
            let config_for_build = config.clone();
            let memory_adapter = rustauth::db::MemoryAdapter::new();
        ...
        assert_eq!(
            cache.build_count(),
            1,
            "repeated requests to the same profile must reuse one cached RustAuth instance"
        );
        Ok(())
    }
    ...
    async fn viewer_adapter_cache_reuses_sql_connections() -> Result<(), ExampleError> {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "rustauth-ope93-viewer-{}-{nanos}.sqlite",
            std::process::id()
        ));
        let database_url = format!("sqlite://{}", path.display());
        ensure_sqlite_parent(&test_example_config(
    ...
    fn readme_documents_rate_limit_surface() {
        ...
        for var in [
            "RUSTAUTH_EXAMPLE_RATE_LIMIT_ENABLED",
            "RUSTAUTH_EXAMPLE_RATE_LIMIT_WINDOW",
            "RUSTAUTH_EXAMPLE_RATE_LIMIT_MAX",
        ] {
