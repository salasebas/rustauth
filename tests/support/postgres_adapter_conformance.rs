...
pub fn database_url() -> String {
    database_url_from_env(env_var("TEST_POSTGRES_URL"))
}
...
pub async fn raw_client() -> Result<tokio_postgres::Client, RustAuthError> {
    ...
    A: DbAdapter + ?Sized,
{
    ...
    let found = adapter
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter("missing json array user".to_owned()))?;
    assert_eq!(
        found.get("profile"),
        Some(&DbValue::Json(
...
pub async fn assert_supports_native_and_fallback_joins<A>(adapter: &A) -> Result<(), RustAuthError>
where
    A: DbAdapter + ?Sized,
{
    seed_user(
    ...
    let joined = users
        ...
        .ok_or_else(|| RustAuthError::Adapter("missing fallback joined user".to_owned()))?;
    assert!(matches!(
        joined.get("account"),
        Some(DbValue::RecordArray(accounts)) if accounts.len() == 1
    ));
    ...
    A: DbAdapter + ?Sized,
{
    ...
    let without = users
        ...
        .ok_or_else(|| RustAuthError::Adapter("missing user without children".to_owned()))?;
    assert!(matches!(
        without.get("session"),
    ...
    let session = adapter
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter("missing joined session".to_owned()))?;
    assert!(matches!(
        session.get("user"),
...
pub async fn assert_rolls_back_failed_transactions<A>(adapter: &A) -> Result<(), RustAuthError>
where
    A: DbAdapter + ?Sized,
{
    let result = adapter
...
pub async fn assert_commits_successful_transactions<A>(adapter: &A) -> Result<(), RustAuthError>
where
    A: DbAdapter + ?Sized,
{
    adapter
...
pub async fn assert_rolls_back_on_cancelled_transaction<A>(adapter: &A) -> Result<(), RustAuthError>
where
    A: DbAdapter + ?Sized,
{
    let cancelled = tokio::time::timeout(
        ...
        adapter.transaction(Box::new(|tx| {
            Box::pin(async move {
                ...
                .await?;
                std::future::pending::<Result<(), RustAuthError>>().await
            })
        })),
    )
    .await;
...
pub async fn assert_no_commit_bleed_after_cancel<A>(adapter: &A) -> Result<(), RustAuthError>
where
    A: DbAdapter + ?Sized,
{
    let cancelled = tokio::time::timeout(
...
pub async fn assert_rejects_nested_transactions<A>(adapter: &A) -> Result<(), RustAuthError>
where
    A: DbAdapter + ?Sized,
{
    let result = adapter
...
pub async fn seed_account<A>(adapter: &A, id: &str, user_id: &str) -> Result<(), RustAuthError>
where
    A: DbAdapter + ?Sized,
{
    let now = OffsetDateTime::now_utc();
...
pub async fn seed_session<A>(adapter: &A, id: &str, user_id: &str) -> Result<(), RustAuthError>
where
    A: DbAdapter + ?Sized,
{
    let now = OffsetDateTime::now_utc();
    ...
    A: DbAdapter + ?Sized,
{
    adapter
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter(format!("missing {model} record `{value}`")))
}
