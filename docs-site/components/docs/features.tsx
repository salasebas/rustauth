		...
		<div className="py-2 max-w-[1300px]">
			<div className="mt-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-10 md:gap-2 max-w-7xl mx-auto">
				{grid.map((feature, i) => (
					<div
						key={feature.title}
						className="relative bg-gradient-to-b min-h-[180px] dark:from-card from-neutral-100 dark:to-background to-white px-6 py-2 overflow-hidden"
					>
						<Grid size={i * 5 + 10} />
						<p className="text-base font-bold text-foreground relative z-0">
							{feature.title}
						</p>
...
const grid = [
	{
		title: "Server-first Rust",
		description: "Auth routes, adapters, and plugins on your Axum server",
	},
	{
