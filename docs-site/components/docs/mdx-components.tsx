...
import { UnderDevelopment as UnderDevelopmentCallout } from "@/components/docs/under-development-callout";

export { UnderDevelopmentCallout as UnderDevelopment };
			...
			<Button
				...
				onClick={() => {
					...
					for (let i = 0; i < elements.length; i++) {
						if (
							elements[i].textContent === "RUSTAUTH_SECRET=" ||
							elements[i].textContent === "BETTER_AUTH_SECRET="
						) {
							elements[i].textContent =
								`RUSTAUTH_SECRET=${generateRandomString(32)}`;
							setGenerated(true);
							setTimeout(() => {
								elements[i].textContent = "RUSTAUTH_SECRET=";
								setGenerated(false);
...
export function AddToCursor() {
	return (
		<div className="w-max">
			<Link
				href="cursor://anysphere.cursor-deeplink/mcp/install?name=RustAuth&config=eyJ1cmwiOiJodHRwczovL21jcC5ydXN0YXV0aC5kZXYvbWNwIn0="
				className="dark:hidden"
			>
				<img
					src="https://cursor.com/deeplink/mcp-install-dark.svg"
					alt="Add RustAuth MCP to Cursor"
					height="32"
				/>
			</Link>
