...
2. Create src/auth.rs — build RustAuthOptions with RustAuth::builder() (secret, base_url, plugins).

3. Attach a SQLx adapter (SqliteAdapter or DeadpoolPostgresAdapter) when building RustAuth.

4. Run: rustauth db migrate --yes

5. Mount routes with rustauth_axum::RustAuthAxumExt on your Axum Router.

6. No browser authClient — frontends call REST under your auth base path.

Refer to /docs and examples/backend-reference for exact APIs.`;
							...
							{mode === "cli" ? (
								...
								<div className="bg-neutral-100/50 dark:bg-card px-5 py-4">
									<p className="text-[13px] font-medium text-neutral-700 dark:text-neutral-200 leading-relaxed">
										Set up authentication in my Rust project using RustAuth.
									</p>
									<div className="relative mt-1.5">
										<p className="text-[11px] text-neutral-400 dark:text-neutral-500 leading-relaxed line-clamp-2">
											Add rustauth + rustauth-axum to Cargo.toml. Create
											src/auth.rs with{" "}
											<code className="text-neutral-500 dark:text-neutral-400">
												RustAuth::builder()
											</code>
											, attach a SQLx adapter, run rustauth db migrate, mount
											Axum routes...
										</p>
...
const MARQUEE_CONTRIBUTOR_THRESHOLD = 24;
...
function ContributorsSection({
	...
	const useMarquee = contributors.length >= MARQUEE_CONTRIBUTOR_THRESHOLD;
	const countLabel =
	...
	const colCount = 18;
	const columns = useMarquee
		? Array.from({ length: colCount }, (_, i) =>
				contributors.filter((_, j) => j % colCount === i),
			)
		: [];
			...
			<div className="relative space-y-6">
						...
						>
							<div className="flex items-center gap-1.5 px-2.5 hover:bg-foreground/4 rounded-sm transition-colors text-foreground/50 dark:text-foreground/50">
								...
								>
									<path
										fill="currentColor"
										d="M12 2a10 10 0 0 0-9.95 8.86L2 12l.05.14L12 22l9.95-9.86L22 12l-.05-.14A10 10 0 0 0 12 2zm0 2.2a7.8 7.8 0 1 1 0 15.6 7.8 7.8 0 0 1 0-15.6z"
									/>
								</svg>
								<span className="text-xs font-mono">
									{formatCount(stats.crateRecentDownloads)} / 90d
								</span>
							</div>
						</a>
					)}
				...
				<div className="flex flex-wrap items-center justify-center gap-4 pt-1">
					<Link
						...
						className="inline-flex items-center gap-1.5 px-4 sm:px-5 py-2 rounded-full bg-primary text-primary-foreground text-xs sm:text-sm font-medium hover:bg-primary/90 transition-colors"
					>
						Get Started
					</Link>
					<Link
				...
				<div className="p-5 lg:px-8 lg:pt-20">
					...
					>
						...
						<div className="relative grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mb-2 border border-foreground/[0.08] overflow-hidden">
							{[
								{
									label: "Axum Integration",
									headline: "Server-first for Rust.",
									desc: "Axum mount + HTTP API for any frontend.",
									logos: true,
									href: "/docs/integrations/axum",
								},
								{
								...
								{
									label: "Plugins",
									headline: "Cargo workspace plugins.",
								...
								>
									...
									>
										...
										{feature.label === "Axum Integration" ? (
										...
										{feature.label === "Email & Password" ? (
											<CredentialFields />
										) : null}
										{feature.label === "Organizations" ? (
						...
						<div className="my-4">
							...
							<p className="text-[15px] sm:text-base text-foreground/50 mt-1">
								Server-first authentication for Rust applications.
							</p>
						</div>
						...
						<div className="mt-8 mb-10">
							<div className="border-r border-foreground/[0.1] bg-foreground/[0.01] overflow-hidden">
								<div className="flex flex-col lg:flex-row">
									...
									<div className="flex flex-row lg:flex-col lg:w-56 lg:shrink-0 border-t lg:border-t-0 lg:border-l border-foreground/[0.1] bg-neutral-50 dark:bg-background overflow-x-auto lg:overflow-visible">
										{[
										...
										<div className="hidden lg:flex flex-1 items-end p-4">
											<p className="text-[13px] leading-relaxed text-foreground/60 dark:text-foreground/50">
												{frameworkTab === "declarative" &&
													"rustauth.toml + version-controlled Rust config — no dashboard clicks."}
												{frameworkTab === "database" &&
													"SQLx, Deadpool Postgres, and Redis-backed adapters. Your data stays yours."}
												{frameworkTab === "oauth" &&
													"35+ Rust social providers in rustauth-social-providers."}
												{frameworkTab === "integrations" &&
													"Axum server integration — see /docs/integrations/axum."}
											</p>
										</div>
									</div>
								</div>
