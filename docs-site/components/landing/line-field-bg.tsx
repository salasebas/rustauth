...
// behind the logo, strong fade above, lighter fade below. In dark mode the
// canvas mixes against the surface token from CSS; light mode keeps the
// monochrome output and lets CSS invert handle the flip.
...
void main() {
  ...
  vec3 mono = vec3(brightness);
  vec3 surface = mix(u_bg, vec3(1.0), brightness);
  gl_FragColor = vec4(mix(mono, surface, u_mix_bg), 1.0);
}`;

function parseRgb(color: string): [number, number, number] {
	const match = color.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
	if (!match) {
		return [20 / 255, 20 / 255, 19 / 255];
	}
...
export function LineFieldBackground() {
	...
	useLayoutEffect(() => {
		...
		const uMixBg = gl.getUniformLocation(program, "u_mix_bg");
		const container = canvas.parentElement;

		const syncSurfaceColor = () => {
			if (!container) return;

			gl.useProgram(program);

			const isDark = document.documentElement.classList.contains("dark");
			gl.uniform1f(uMixBg, isDark ? 1 : 0);
		...
		const themeObserver = new MutationObserver(syncSurfaceColor);
