...
export const APIMethod = ({
	 ...
	 * See other RustAuth docs pages that use this component for layout examples.
	 */
				...
				<ApiMethodTabsList className="relative flex justify-start w-full p-0 bg-background hover:[&>div>a>button]:opacity-100">
					{(
						...
						<ApiMethodTabsTrigger
							key={tab.value}
							value={tab.value}
							className="transition-all duration-150 ease-in-out max-w-[100px] data-[state=active]:bg-fd-muted/80 hover:bg-fd-secondary/70 bg-background border hover:border-primary/15 cursor-pointer data-[state=active]:border-primary/10 rounded-md dark:bg-card dark:hover:bg-accent dark:data-[state=active]:bg-fd-muted/80 dark:border-border dark:hover:border-primary/25 dark:data-[state=active]:border-primary/30"
						>
							<svg
...
function createServerBody({
	...
	if (requireSession) {
		...
		fetchOptions += `\n    // ${
			headersComment ||
			"Pass the current request headers so RustAuth can read and set cookies."
		}\n    headers: await headers(),`;
	}
