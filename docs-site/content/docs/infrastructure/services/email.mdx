---
title: Email Service
description: RustAuth Infrastructure provides a managed transactional email service with pre-built templates for common authentication flows. Send verification emails, password resets, invitations, and more without managing email infrastructure.
---
...
The email service integrates seamlessly with RustAuth's authentication flows. Here's a complete example:

```ts title="auth.ts"
import { betterAuth } from "rustauth";
import { organization } from "rustauth/plugins";
import { sendEmail } from "@better-auth/infra";
