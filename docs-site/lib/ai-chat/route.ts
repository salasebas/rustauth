...
const GITHUB_REPO = "rustauth/rustauth";
...
const SYSTEM_PROMPT = `You are a helpful documentation assistant for RustAuth, a comprehensive framework-agnostic authentication and authorization framework for TypeScript.

Your role is to answer questions about RustAuth by referencing the official documentation. You should be accurate, concise, and helpful.
	...
	try {
		...
		const result = streamText({
			...
			tools: {
				searchDocs: tool({
					description:
						"Search across all RustAuth documentation for specific terms, keywords, or concepts. Returns matching page slugs and snippets. Use this when you're not sure which page contains the answer.",
				...
				getDocumentation: tool({
					description:
						"Retrieve the full content of a RustAuth documentation page by its slug. Use this to look up detailed information before answering user questions.",
					inputSchema: z.object({
						slug: z
							.string()
							.describe(
				...
				searchCode: tool({
					description:
						"Search the RustAuth source code on GitHub using semantic code search. Returns matching file paths and code fragments. Use this when users ask about implementation details, internal behavior, or want to see actual source code.",
					inputSchema: z.object({
						query: z
							.string()
							.describe(
						...
						path: z
							...
							.optional()
							.describe(
								"Optional path filter to narrow search to a directory or file pattern, e.g. 'packages/rustauth/src' or 'packages/cli'",
							),
					}),
				...
				getFileContent: tool({
					description:
						"Fetch the content of a specific file from the RustAuth GitHub repository. Use this after searchCode to read the full source of a matching file.",
					inputSchema: z.object({
						path: z
							.string()
							.describe(
								"The file path in the repository, e.g. 'packages/rustauth/src/api/routes/session.ts'",
							),
					}),
					execute: async ({ path }) => {
