...
Bare `cargo check` / `cargo build` at the repo root only builds
`default-members` (`rustauth-core`, `rustauth`, and `rustauth-cli`). Use
`cargo check --workspace` for full verification before release.

Prefer scoped commands for the crate or behavior you changed:

```bash
cargo check -p rustauth-core
cargo nextest run -p rustauth-core
cargo clippy -p rustauth-core --all-targets -- -D warnings
```
...
`CARGO_TARGET_DIR=/private/tmp/rustauth-...` for documented workflows; that
...
Include old RustAuth-owned `/private/tmp` artifacts only when explicitly
requested:
