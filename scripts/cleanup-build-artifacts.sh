...
Safely clean old RustAuth build artifacts.
...
Options:
  ...
  --include-private-tmp     include /private/tmp/rustauth-* target dirs and temp DBs
...
if [ "$include_private_tmp" = "true" ]; then
  echo
  echo "RustAuth-owned /private/tmp artifacts older than $days days:"
  tmp_matches="$(
    find /private/tmp -maxdepth 1 \
      \( \
        \( -type d -name 'rustauth-*-target' \) -o \
        \( -type d -name 'rustauth-publish-target*' \) -o \
        \( -type f -name 'rustauth_sqlx_*.db' \) \
      \) \
