...
| **Rust crate** | `crates/rustauth-i18n/` |
| **Parity level** | **~99%** (server plugin) |
| **Scope** | Server plugin only: error-message translation via response hook. No HTTP routes, DB schema, migrations, adapters, or rate limits. Re-exported from `rustauth` with feature `i18n`. |
...
RustAuth mirrors the upstream i18n server contract: on error responses, detect a locale,
...
(`rustauth-core` `finalize_response` / `finalize_response_async`). Rust adds explicit config
...
| `isAPIError` gate before translate | 🎯 | RustAuth uses status + JSON `code` on `on_response` (broader, still skips success) |
...
```bash
cargo nextest run -p rustauth-i18n
```

Cross-crate: `rustauth-core` session hydration (`session_request_state.rs`) is covered by
`session_detection_reads_locale_from_session_cookie_hydration`. Facade re-export tested in
`rustauth` `public_api.rs` (`i18n` feature).
...
| Session locale | Reads `ctx.context.session.user` field synchronously | `resolve_user_locale` + async cookie hydration on `handle_async` | DB-backed sessions in RustAuth |
...
| G5 | Broader translate gate vs `isAPIError` | 🎯 | RustAuth extension: non-`ApiErrorResponse` JSON errors with `code` translate on `on_response` |
...
| `better-auth/cookies` `parseCookies` | `src/cookie.rs` (`rustauth-core::cookies`) |
| `APIError` rewrite | `src/response.rs` (`translate_response`) |
| Locale catalog / matching | `src/locale.rs` |
| Config validation | `src/error.rs` |
| `finalize_response_async` session hydration | `rustauth-core/src/api/session_request_state.rs` |
| `on_response_async` (async locale pre-detection) | `rustauth-core/src/api/plugin_pipeline.rs` |
