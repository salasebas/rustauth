...
use rustauth_core::context::request_state::current_session_user;
...
/// i18n plugin for RustAuth: translates `message` on JSON error responses using `code` as the lookup key.
///
...
pub fn i18n(options: I18nOptions) -> Result<AuthPlugin, I18nConfigError> {
    ...
    if state.get_locale_async.is_some() {
        ...
        plugin = plugin.with_on_response_async(
            move |context, request, response| -> Pin<Box<dyn Future<Output = Result<(), RustAuthError>> + Send + '_>> {
