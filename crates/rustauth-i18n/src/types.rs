//! Options for the RustAuth i18n plugin (parity with `@better-auth/i18n`).
...
mod error_code_translation {
    ...
    use rustauth_core::api::ApiErrorCode;
    ...
    impl Sealed for ApiErrorCode {}
    impl Sealed for AuthFlowErrorCode {}
    impl Sealed for PluginErrorCode {}

    impl<T> TranslationKey for T
    where
        T: ErrorCode + Sealed,
    {
        fn into_translation_key(self) -> String {
...
/// Build a translation dictionary from string keys or typed RustAuth error-code enums.
///
/// # Examples
///
/// ```rust
/// use rustauth_i18n::translation_dictionary;
///
...
pub struct I18nOptions {
    ...
    /// Used when [`LocaleDetectionStrategy::Callback`] is enabled on [`AuthRouter::handle_async`](rustauth_core::api::AuthRouter::handle_async).
