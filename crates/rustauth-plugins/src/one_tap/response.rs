...
use rustauth_core::db::{DbAdapter, DbRecord, DbValue, FindOne, Session, User, Where};
...
async fn user_response_value(
    ...
    if context.options.user.additional_fields.is_empty() {
        return serde_json::to_value(user).map_err(|error| RustAuthError::Api(error.to_string()));
    }
    let record = adapter
        .find_one(
