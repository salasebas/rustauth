...
use rustauth_social_providers::advanced::google::{google, GoogleOptions};
...
pub(super) fn one_tap_callback_endpoint(
    ...
    create_auth_endpoint(
        ...
        move |context, request| {
            ...
            async move { handle_one_tap_callback(&context, request, options).await }
        },
    )
}
...
fn google_provider(
    ...
    context.social_provider(GOOGLE_PROVIDER_ID).ok_or_else(|| {
        RustAuthError::InvalidConfig("one-tap requires a configured google provider".to_owned())
    })
}
...
fn oauth_error_response(error: OAuthUserInfoError) -> Result<ApiResponse, RustAuthError> {
    match error {
