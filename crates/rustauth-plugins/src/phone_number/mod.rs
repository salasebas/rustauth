...
pub use options::{
    PhoneNumberCallback, PhoneNumberOptions, PhoneNumberOptionsBuilder, PhoneNumberSender,
    PhoneNumberValidator, PhoneNumberVerifier, SignUpOnVerification,
};
...
pub fn phone_number(options: PhoneNumberOptions) -> Result<AuthPlugin, RustAuthError> {
    options.validate()?;
    let options = Arc::new(options.with_defaults());
    let schema = options.schema.clone();
    let plugin = AuthPlugin::new(UPSTREAM_PLUGIN_ID)
