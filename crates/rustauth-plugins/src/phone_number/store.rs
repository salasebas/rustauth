...
use rustauth_core::db::{DbAdapter, DbRecord, DbValue, FindOne, Update, Where};
...
pub(crate) async fn find_by_phone(
    ...
    phone_number: &str,
) -> Result<Option<PhoneUser>, RustAuthError> {
    let record = adapter
...
pub(crate) async fn create_user_with_phone(
...
) -> Result<PhoneUser, RustAuthError> {
    ...
    update_phone(adapter, &user.id, Some(phone_number), true)
        .await?
        .ok_or_else(|| RustAuthError::Adapter("failed to update created user".to_owned()))
}

fn phone_user_from_record(record: DbRecord) -> Result<PhoneUser, RustAuthError> {
    Ok(PhoneUser {
...
fn string_field(record: &DbRecord, field: &str) -> Result<String, RustAuthError> {
...
fn bool_field(record: &DbRecord, field: &str) -> Result<bool, RustAuthError> {
    match record.get(field) {
