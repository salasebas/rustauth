...
where
    F: Fn(AuthContext, ApiRequest, SharedConfigurations) -> Fut + Send + Sync + Clone + 'static,
    Fut: std::future::Future<Output = Result<ApiResponse, RustAuthError>> + Send + 'static,
{
    create_auth_endpoint(
        ...
        move |context, request| {
            ...
            async move { handler(context, request, configurations).await }
        },
    )
}
...
pub(crate) fn error(status: StatusCode, code: &str) -> Result<ApiResponse, RustAuthError> {
    errors::error_response(status, code)
}

pub(crate) fn body<T: DeserializeOwned>(request: &ApiRequest) -> Result<T, RustAuthError> {
...
pub(crate) async fn current_identity(
    context: &AuthContext,
    request: &ApiRequest,
) -> Result<Option<CurrentIdentity>, RustAuthError> {
