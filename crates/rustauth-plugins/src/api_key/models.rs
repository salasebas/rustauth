...
pub(crate) fn record_from_db(record: DbRecord) -> Result<ApiKeyRecord, RustAuthError> {
    Ok(ApiKeyRecord {
...
fn required_string<'a>(record: &'a DbRecord, field: &str) -> Result<&'a str, RustAuthError> {
    match record.get(field) {
        ...
        Some(DbValue::Null) | None => Err(RustAuthError::Adapter(format!(
...
fn optional_number_field(record: &DbRecord, field: &str) -> Result<Option<i64>, RustAuthError> {
    match record.get(field) {
...
fn optional_json_field(record: &DbRecord, field: &str) -> Result<Option<Value>, RustAuthError> {
    match record.get(field) {
