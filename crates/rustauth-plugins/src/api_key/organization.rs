use rustauth_core::db::{DbRecord, DbValue, FindOne, Where};
...
pub async fn ensure_organization_permission(
    ...
    let adapter = context.adapter().ok_or_else(|| {
        RustAuthError::Adapter("organization API keys require a database adapter".to_owned())
    })?;
...
async fn role_has_permission(
    context: &rustauth_core::context::AuthContext,
    adapter: &std::sync::Arc<dyn rustauth_core::db::DbAdapter>,
    organization_id: &str,
    role: &str,
    action: ApiKeyAction,
...
fn organization_plugin_options(context: &rustauth_core::context::AuthContext) -> Option<&Value> {
    context
        .plugins
        .iter()
