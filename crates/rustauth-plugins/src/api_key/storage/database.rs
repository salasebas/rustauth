...
impl ApiKeyStore<'_> {
    pub async fn delete_expired(&self, now: OffsetDateTime) -> Result<u64, RustAuthError> {
    ...
    ) -> Result<ApiKeyRecord, RustAuthError> {
    ...
    pub(super) async fn delete_database(&self, id: &str) -> Result<(), RustAuthError> {
        let Some(adapter) = &self.adapter else {
            return Ok(());
        };
        adapter
