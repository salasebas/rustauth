...
pub fn sign_in_anonymous_endpoint(options: AnonymousOptions) -> AsyncAuthEndpoint {
    create_auth_endpoint(
        ...
        move |context, request| {
            ...
            async move { sign_in_anonymous(&context, request, options).await }
        },
    )
}
...
pub fn delete_anonymous_user_endpoint(options: AnonymousOptions) -> AsyncAuthEndpoint {
    create_auth_endpoint(
        ...
        move |context, request| {
            let options = options.clone();
            async move { delete_anonymous_user(&context, request, options).await }
        },
    )
}
...
async fn sign_in_anonymous(
    ...
    let session =
        match model::create_session(context, &user.id, anonymous_session_create_values(context))
            .await
        {
            Ok(session) => session,
            Err(_) => {
...
async fn delete_anonymous_user(
    ...
    if model::delete_anonymous_user_records(context, &session.user.id)
        .await
        .is_err()
    {
        return error_response(
