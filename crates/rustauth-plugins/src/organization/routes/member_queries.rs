...
use rustauth_core::db::{DbValue, SortDirection, WhereOperator};
    ...
    create_auth_endpoint(
        ...
        |context, request| async move {
            ...
            .await?
                ...
                return http::organization_error(StatusCode::BAD_REQUEST, "NO_ACTIVE_ORGANIZATION");
            };
            match store
                .member_by_org_user(&organization_id, &session.user.id)
                .await?
            {
                Some(_) => {
                    let target_user_id =
                        query_param(&request, "userId").unwrap_or_else(|| session.user.id.clone());
...
async fn resolve_organization_id(
...
) -> Result<Option<String>, rustauth_core::error::RustAuthError> {
    if let Some(slug) = query_param(request, "organizationSlug") {
        return match store.organization_by_slug(&slug).await? {
            ...
            None => Err(rustauth_core::error::RustAuthError::Api(
