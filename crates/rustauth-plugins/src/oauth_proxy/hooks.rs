...
use rustauth_core::api::{ApiRequest, ApiResponse, PathParams};
...
use rustauth_core::plugin::{PluginAfterHookAction, PluginBeforeHookAction};
use rustauth_core::plugin::{PluginAfterHookFuture, PluginBeforeHookFuture};
use rustauth_core::verification::DbVerificationStore;
use rustauth_oauth::oauth2::SocialAuthorizationCodeRequest;
    ...
    move |context, request| {
        ...
        Box::pin(async move {
            ...
            let provider_id = request
                ...
                .ok_or_else(|| RustAuthError::Api("missing path param `id`".to_owned()))?;
            ...
            rustauth_core::api::redirect_response(redirect_url.as_str(), Vec::new())
                .map(PluginBeforeHookAction::Respond)
        })
    }
}
...
) -> impl Fn(&AuthContext, &ApiRequest, ApiResponse) -> Result<PluginAfterHookAction, RustAuthError>
       + Send
       + Sync
       + 'static {
...
async fn load_state_data(
    context: &AuthContext,
    state: &str,
) -> Result<Option<OAuthStateData>, RustAuthError> {
...
fn original_callback_url(request: &ApiRequest) -> Option<String> {
    ...
    rustauth_core::api::parse_request_body::<serde_json::Value>(request)
        .ok()
        .and_then(|body| {
...
fn json_error(error: impl std::fmt::Display) -> RustAuthError {
