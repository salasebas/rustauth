...
pub(crate) fn production_url(context: &AuthContext, options: &OAuthProxyOptions) -> Option<Url> {
    let value = options
        .production_url
        .clone()
        .or_else(|| std::env::var("RUSTAUTH_URL").ok())
...
pub(crate) fn rewrite_callback_body(
    ...
    let object = body
        .as_object_mut()
        .ok_or_else(|| RustAuthError::Api("OAuth sign-in body must be an object".to_owned()))?;
    object.insert(
    ...
    if content_type.starts_with("application/x-www-form-urlencoded") {
        ...
        *request.body_mut() =
            serde_json::to_vec(&body).map_err(|error| RustAuthError::Api(error.to_string()))?;
...
pub(crate) fn redirect_error(error_url: &str, error: &str) -> Result<ApiResponse, RustAuthError> {
