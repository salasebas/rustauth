...
use rustauth_core::options::{SessionAdditionalField, UserAdditionalField};
use rustauth_core::plugin::{AuthPlugin, PluginInitOutput, PluginSchemaContribution};
...
impl AdditionalFieldsOptions {
    ...
    #[must_use]
    pub fn builder() -> AdditionalFieldsOptionsBuilder {
        AdditionalFieldsOptionsBuilder::default()
    }
...
pub struct AdditionalFieldsOptionsBuilder {
    user: Option<BTreeMap<String, AdditionalField>>,
    session: Option<BTreeMap<String, AdditionalField>>,
}

impl AdditionalFieldsOptionsBuilder {
    #[must_use]
    pub fn user(mut self, user: BTreeMap<String, AdditionalField>) -> Self {
        self.user = Some(user);
        self
    }
    ...
    #[must_use]
    pub fn session(mut self, session: BTreeMap<String, AdditionalField>) -> Self {
    ...
    #[must_use]
    pub fn build(self) -> AdditionalFieldsOptions {
        let defaults = AdditionalFieldsOptions::default();
        AdditionalFieldsOptions {
            user: self.user.unwrap_or(defaults.user),
            session: self.session.unwrap_or(defaults.session),
        }
    }
}

/// Create the additional-fields plugin.
#[must_use]
