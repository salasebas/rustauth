...
use rustauth_core::user::{CreateOAuthAccountInput, CreateUserInput};
    ...
    create_auth_endpoint(
        ...
        move |context, request| {
            ...
            async move {
                ...
                verify_request(&context, &options, body).await
            }
        },
    )
}
...
async fn verify_request(
    ...
    let _adapter = context.adapter_ref()?;
    ...
    let user = find_or_create_user(context, options, &body, &wallet_address).await?;
    let session = context
        .sessions()?
...
async fn find_or_create_user(
    ...
    if let Some(wallet) = &existing_wallet {
        if let Some(user) = store::user_for_wallet(context, wallet).await? {
            return Ok(user);
        }
    }
...
fn invalid_nonce() -> Result<ApiResponse, RustAuthError> {
    error_response(
        StatusCode::UNAUTHORIZED,
...
fn session_cookies(context: &AuthContext, token: &str) -> Result<Vec<Cookie>, RustAuthError> {
    set_session_cookie(
