...
pub(crate) fn redirect(location: &str, cookies: Vec<Cookie>) -> Result<ApiResponse, RustAuthError> {
    let mut response = http::Response::builder()
...
fn append_cookies(response: &mut ApiResponse, cookies: Vec<Cookie>) -> Result<(), RustAuthError> {
