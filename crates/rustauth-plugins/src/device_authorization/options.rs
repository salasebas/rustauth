...
pub type ClientValidationFuture = Pin<Box<dyn Future<Output = Result<bool, RustAuthError>> + Send>>;
pub type ClientValidator = Arc<dyn Fn(String) -> ClientValidationFuture + Send + Sync>;
pub type DeviceAuthRequestFuture = Pin<Box<dyn Future<Output = Result<(), RustAuthError>> + Send>>;
...
impl DeviceAuthorizationOptions {
    ...
    #[must_use]
    pub fn builder() -> DeviceAuthorizationOptionsBuilder {
        DeviceAuthorizationOptionsBuilder::default()
    }
...
pub struct DeviceAuthorizationOptionsBuilder {
    expires_in: Option<Duration>,
    interval: Option<Duration>,
    device_code_length: Option<usize>,
    user_code_length: Option<usize>,
    generate_device_code: Option<AsyncDeviceCodeGenerator>,
    generate_user_code: Option<AsyncDeviceCodeGenerator>,
    validate_client: Option<ClientValidator>,
    on_device_auth_request: Option<DeviceAuthRequestHook>,
    verification_uri: Option<String>,
    schema: Option<DeviceAuthorizationSchemaOptions>,
}
...
impl DeviceAuthorizationOptionsBuilder {
    ...
    #[must_use]
    pub fn device_code_length(mut self, length: usize) -> Self {
        self.device_code_length = Some(length);
        self
    }

    #[must_use]
    pub fn user_code_length(mut self, length: usize) -> Self {
        self.user_code_length = Some(length);
        self
    }

    #[must_use]
    pub fn generate_device_code(mut self, generator: AsyncDeviceCodeGenerator) -> Self {
        self.generate_device_code = Some(generator);
        self
    }

    #[must_use]
    pub fn generate_user_code(mut self, generator: AsyncDeviceCodeGenerator) -> Self {
        self.generate_user_code = Some(generator);
        self
    }

    #[must_use]
    pub fn validate_client(mut self, validator: ClientValidator) -> Self {
        self.validate_client = Some(validator);
        self
    }

    #[must_use]
    pub fn on_device_auth_request(mut self, hook: DeviceAuthRequestHook) -> Self {
        self.on_device_auth_request = Some(hook);
        self
    }
    ...
    pub fn build(self) -> Result<DeviceAuthorizationOptions, RustAuthError> {
        let defaults = DeviceAuthorizationOptions::default();
        let options = DeviceAuthorizationOptions {
            expires_in: self.expires_in.unwrap_or(defaults.expires_in),
            interval: self.interval.unwrap_or(defaults.interval),
            device_code_length: self
                .device_code_length
                .unwrap_or(defaults.device_code_length),
            user_code_length: self.user_code_length.unwrap_or(defaults.user_code_length),
            generate_device_code: self.generate_device_code.or(defaults.generate_device_code),
            generate_user_code: self.generate_user_code.or(defaults.generate_user_code),
            validate_client: self.validate_client.or(defaults.validate_client),
            on_device_auth_request: self
                .on_device_auth_request
                .or(defaults.on_device_auth_request),
            verification_uri: self.verification_uri.unwrap_or(defaults.verification_uri),
