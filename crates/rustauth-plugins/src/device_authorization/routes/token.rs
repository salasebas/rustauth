...
use rustauth_core::context::{request_state, AuthContext};
            ...
            async move {
                ...
                match record.status {
                    ...
                    DeviceAuthorizationStatus::Approved => {
                        approved_response(&context, &store, &record, &request).await
                    }
                }
            }
        },
...
async fn approved_response(
    ...
    let Some(user) = context.users()?.find_user_by_id(user_id).await? else {
        return token_oauth_error_response(
            StatusCode::INTERNAL_SERVER_ERROR,
            OAuthDeviceError::ServerError,
            "User not found",
