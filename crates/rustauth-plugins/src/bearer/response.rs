...
use rustauth_core::cookies::parse_set_cookie_header;
use rustauth_core::error::RustAuthError;
use rustauth_core::plugin::{PluginRequest, PluginResponse};
...
fn expose_auth_token_header(response: &mut PluginResponse) -> Result<(), RustAuthError> {
    ...
    let value = value
        .parse()
        .map_err(|error| RustAuthError::Api(format!("invalid expose header: {error}")))?;
    response
        .headers_mut()
