...
use rustauth_core::user::{CreateCredentialAccountInput, DbUserStore};
...
impl<'a> AdminStore<'a> {
    ...
    ) -> Result<AdminUser, RustAuthError> {
        let now = OffsetDateTime::now_utc();
        let id = rustauth_core::crypto::random::generate_random_string(32);
            ...
            DbUserStore::new(self.adapter)
                .create_credential_account(
                    CreateCredentialAccountInput::new(user.id.clone(), password_hash)
                        .id(rustauth_core::crypto::random::generate_random_string(32)),
                )
                .await?;
        }
        Ok(user)
    ...
    pub async fn find_user_by_id(&self, user_id: &str) -> Result<Option<AdminUser>, RustAuthError> {
        self.adapter
            .find_one(
    ...
    pub async fn find_user_by_email(
        &self,
        email: &str,
    ) -> Result<Option<AdminUser>, RustAuthError> {
        self.adapter
            .find_one(
                FindOne::new(USER_MODEL)
    ...
    pub async fn list_users(&self, input: ListUsersQuery) -> Result<ListUsers, RustAuthError> {
        ...
        let mut count = rustauth_core::db::Count::new(USER_MODEL);
        for clause in clauses {
    ...
    pub async fn unban_user(&self, user_id: &str) -> Result<Option<AdminUser>, RustAuthError> {
        self.update_user_fields(
            user_id,
            DbRecord::from([
    ...
    ) -> Result<AdminSession, RustAuthError> {
        let now = OffsetDateTime::now_utc();
        let record = self
            .adapter
            .create(
                Create::new(SESSION_MODEL)
                    .data(
                        "id",
                        DbValue::String(rustauth_core::crypto::random::generate_random_string(32)),
                    )
    ...
    pub async fn find_session(
        &self,
        token: &str,
    ) -> Result<Option<(AdminSession, AdminUser)>, RustAuthError> {
    ...
    pub async fn list_user_sessions(
        &self,
        user_id: &str,
    ) -> Result<Vec<AdminSession>, RustAuthError> {
        let now = OffsetDateTime::now_utc();
        self.adapter
            .find_many(
...
pub fn role_value(value: serde_json::Value) -> Result<DbValue, RustAuthError> {
    match value {
...
pub fn ban_expires_from_now(duration: time::Duration) -> Option<OffsetDateTime> {
    (!duration.is_zero()).then(|| OffsetDateTime::now_utc() + duration)
}
