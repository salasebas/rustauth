...
pub(crate) fn admin_user_from_record(record: DbRecord) -> Result<AdminUser, RustAuthError> {
    Ok(AdminUser {
...
pub(crate) fn admin_session_from_record(record: DbRecord) -> Result<AdminSession, RustAuthError> {
    Ok(AdminSession {
        id: string(&record, "id")?.to_owned(),
...
fn string<'a>(record: &'a DbRecord, field: &str) -> Result<&'a str, RustAuthError> {
...
fn timestamp(record: &DbRecord, field: &str) -> Result<OffsetDateTime, RustAuthError> {
    match record.get(field) {
