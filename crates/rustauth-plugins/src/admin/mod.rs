...
pub use access::{has_permission, AdminRole, PermissionMap};
pub use errors::ADMIN_ERROR_CODES;
pub use models::{AdminSession, AdminUser};
pub use options::{AdminOptions, AdminOptionsBuilder, AdminSchemaOptions};
...
pub mod access_control {
    pub use super::access::{
        default_access_control, default_roles, default_statements, has_permission, AdminRole,
        PermissionMap,
    };
}
...
pub fn admin(options: AdminOptions) -> Result<AuthPlugin, RustAuthError> {
    let options = options.with_defaults();
    options.validate().map_err(RustAuthError::InvalidConfig)?;
    Ok(build_admin_plugin(options))
}

fn build_admin_plugin(options: AdminOptions) -> AuthPlugin {
...
fn default_role_hook(default_role: String) -> PluginDatabaseHook {
    PluginDatabaseHook::before_create("admin_default_user_role", move |_context, mut query| {
        if query.model == "user" && !query.data.contains_key("role") {
            query.data.insert(
                "role".to_owned(),
                rustauth_core::db::DbValue::String(default_role.clone()),
            );
        }
    ...
    PluginDatabaseHook::before_create_async(
        ...
        move |context, query| {
            ...
            Box::pin(async move {
                ...
                let Some(rustauth_core::db::DbValue::String(user_id)) = query.data.get("user_id")
                else {
                    return Ok(PluginDatabaseBeforeAction::Continue(
                        PluginDatabaseBeforeInput::Create(query),
                    ));
                ...
                if context.request_path.as_deref().is_some_and(|path| {
                    ...
                    return Ok(PluginDatabaseBeforeAction::Cancel(RustAuthError::Api(
...
fn filter_impersonated_sessions_hook<'a>(
    context: &'a rustauth_core::context::AuthContext,
    _request: &'a rustauth_core::api::ApiRequest,
    response: rustauth_core::api::ApiResponse,
    ...
    Box::pin(async move {
        ...
        let body = serde_json::to_vec(&filtered).map_err(|error| {
            RustAuthError::Api(format!("failed to serialize filtered sessions: {error}"))
        })?;
