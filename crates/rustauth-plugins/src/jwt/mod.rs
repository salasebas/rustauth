...
pub use options::{
    ...
    JwtGetSubjectHandler, JwtJwksOptions, JwtOptions, JwtOptionsBuilder, JwtSessionContext,
...
pub fn jwt_options_from_context(
    context: &rustauth_core::context::AuthContext,
) -> Option<Arc<JwtOptions>> {
    context
        .plugin(UPSTREAM_PLUGIN_ID)
        .and_then(|plugin| plugin.state::<JwtOptions>())
}

fn expose_auth_jwt_header(headers: &mut header::HeaderMap) -> Result<(), RustAuthError> {
    let current = headers
