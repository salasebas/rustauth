...
use rustauth_core::db::{Create, DbAdapter, DbRecord, DbValue, FindMany, Sort, SortDirection};
...
pub(crate) async fn get_latest_key(
    context: &AuthContext,
    options: &JwtOptions,
) -> Result<Option<Jwk>, RustAuthError> {
...
fn adapter(context: &AuthContext) -> Result<std::sync::Arc<dyn DbAdapter>, RustAuthError> {
    context.adapter().ok_or_else(|| {
        RustAuthError::InvalidConfig("JWT plugin requires a database adapter".to_owned())
    })
}

fn jwk_from_record(record: DbRecord) -> Result<Jwk, RustAuthError> {
    Ok(Jwk {
...
fn parse_algorithm(value: &str) -> Result<JwkAlgorithm, RustAuthError> {
    match value {
        ...
        other => Err(RustAuthError::Adapter(format!(
...
fn optional_string<'a>(
    record: &'a DbRecord,
    field: &str,
) -> Result<Option<&'a str>, RustAuthError> {
    match record.get(field) {
...
fn required_timestamp(
    record: &DbRecord,
    field: &str,
) -> Result<time::OffsetDateTime, RustAuthError> {
    match record.get(field) {
...
fn optional_timestamp(
    record: &DbRecord,
    field: &str,
) -> Result<Option<time::OffsetDateTime>, RustAuthError> {
    match record.get(field) {
...
fn missing_field(field: &str) -> RustAuthError {
    RustAuthError::Adapter(format!("jwks record is missing `{field}`"))
}

fn invalid_field(field: &str, expected: &str) -> RustAuthError {
    RustAuthError::Adapter(format!("jwks record field `{field}` must be {expected}"))
}
