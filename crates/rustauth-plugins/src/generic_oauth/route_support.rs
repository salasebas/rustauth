...
use rustauth_core::auth::oauth::OAuthUserInfoError;
...
pub(super) async fn resolved_config(
    ...
    provider_id: &str,
) -> Result<GenericOAuthConfig, RustAuthError> {
...
pub(super) fn path_param<'a>(
    ...
    request
        ...
        .ok_or_else(|| RustAuthError::Api(format!("missing path param `{name}`")))
}
...
pub(super) fn config_error_response(error: RustAuthError) -> Result<ApiResponse, RustAuthError> {
    let RustAuthError::Api(code) = error else {
        return Err(error);
    };
...
fn api_error_value(code: &str) -> RustAuthError {
    RustAuthError::Api(code.to_owned())
}
