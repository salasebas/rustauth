...
use rustauth_core::api::{core_auth_async_endpoints, ApiErrorResponse, AuthRouter};
...
use rustauth_core::plugin::PluginPasswordValidationInput;
use rustauth_core::test_utils::with_integration_test_defaults;
use rustauth_plugins::have_i_been_pwned::{
    have_i_been_pwned, HaveIBeenPwnedCheckError, HaveIBeenPwnedChecker, HaveIBeenPwnedOptions,
    UPSTREAM_PLUGIN_ID,
};
...
#[test]
fn default_factory_is_disabled() -> Result<(), Box<dyn std::error::Error>> {
    let plugin = have_i_been_pwned(HaveIBeenPwnedOptions::default());
...
fn enabled_hibp_options() -> HaveIBeenPwnedOptions {
...
{
    let plugin = have_i_been_pwned(HaveIBeenPwnedOptions {
        enabled: false,
...
async fn compromised_password_blocks_account_creation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let router = router_with_adapter(
        adapter.clone(),
        have_i_been_pwned(enabled_hibp_options().checker(checker)),
    )?;
...
#[tokio::test]
async fn default_factory_skips_compromised_check() -> Result<(), Box<dyn std::error::Error>> {
    let checker = Arc::new(FakeChecker::compromised());
    let adapter = Arc::new(MemoryAdapter::default());
    let router = router_with_adapter(
        adapter.clone(),
        have_i_been_pwned(HaveIBeenPwnedOptions::default().checker(checker)),
    )?;
...
async fn custom_compromised_message_is_returned() -> Result<(), Box<dyn std::error::Error>> {
    let checker = Arc::new(FakeChecker::compromised());
    let router = router(have_i_been_pwned(
        HaveIBeenPwnedOptions {
            enabled: true,
...
async fn checker_receives_hash_prefix_and_suffix_not_raw_password(
    ...
    let checker = Arc::new(FakeChecker::uncompromised());
    let router = router(have_i_been_pwned(
        enabled_hibp_options().checker(checker.clone()),
    ))?;
...
async fn empty_password_skips_checker() -> Result<(), Box<dyn std::error::Error>> {
    let checker = Arc::new(FakeChecker::compromised());
    let plugin = have_i_been_pwned(enabled_hibp_options().checker(checker.clone()));
...
async fn http_status_failure_returns_status_specific_500() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let router = router(have_i_been_pwned(enabled_hibp_options().checker(checker)))?;
...
fn router(plugin: rustauth_core::plugin::AuthPlugin) -> Result<AuthRouter, RustAuthError> {
...
fn json_request(path: &str, body: &str) -> Result<rustauth_core::api::ApiRequest, http::Error> {
...
fn json_request_with_cookie(
    ...
    body: &str,
    cookie: &str,
) -> Result<rustauth_core::api::ApiRequest, http::Error> {
    http::Request::builder()
        .method(http::Method::POST)
        .uri(format!("http://localhost:3000{path}"))
        .header(http::header::CONTENT_TYPE, "application/json")
