...
use rustauth_core::options::{
    AdvancedOptions, IpAddressOptions, RateLimitOptions, RateLimitPathRule, RateLimitRule,
    RustAuthOptions,
};
...
use rustauth_plugins::captcha::{captcha, CaptchaOptions, CaptchaProvider};

#[test]
fn exposes_captcha_plugin_id() {
    assert_eq!(rustauth_plugins::captcha::UPSTREAM_PLUGIN_ID, "captcha");
}
...
fn captcha_rejects_empty_secret_key() {
    ...
    assert!(matches!(
        result,
        Err(RustAuthError::InvalidConfig(message)) if message.contains("secret")
    ));
}
...
async fn captcha_returns_500_when_provider_is_unavailable() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    assert_error(response.status(), response.body(), "CAPTCHA_UNKNOWN_ERROR");
    Ok(())
}
...
fn router_with_advanced(
    ...
    let context = create_auth_context(rustauth_core::test_utils::with_integration_test_defaults(
        RustAuthOptions {
            plugins: vec![plugin],
            secret: Some("secret-a-at-least-32-chars-long!!".to_owned()),
            advanced: AdvancedOptions {
...
fn assert_error(status: StatusCode, body: &[u8], code: &str) {
    ...
    match code {
        ...
        "CAPTCHA_UNKNOWN_ERROR" => assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR),
