...
use rustauth_core::options::{AdvancedOptions, CookieCacheOptions, RustAuthOptions};
use rustauth_plugins::additional_fields::{
    additional_fields, AdditionalField, AdditionalFieldsOptions,
};
use rustauth_plugins::anonymous::{anonymous, AnonymousOptions};
...
fn exposes_plugin_schema_and_error_codes() {
    ...
    assert_eq!(rustauth_plugins::anonymous::UPSTREAM_PLUGIN_ID, "anonymous");
    assert_eq!(plugin.id, "anonymous");
    assert!(plugin
        .schema
        .iter()
        .any(|schema| matches!(schema, rustauth_core::plugin::PluginSchemaContribution::Field { table, logical_name, .. } if table == "user" && logical_name == "is_anonymous")));
    assert!(plugin
        .error_codes
        .iter()
...
async fn sign_in_anonymous_sets_cookie_cache_when_enabled() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let router = router_with_options(
        ...
        RustAuthOptions {
            ...
            session: rustauth_core::options::SessionOptions {
                cookie_cache: CookieCacheOptions {
                    enabled: true,
                    ..CookieCacheOptions::default()
                },
                ..rustauth_core::options::SessionOptions::default()
            },
...
async fn custom_field_name_stores_physical_and_returns_logical(
    ...
    assert_eq!(sign_in_body["user"]["isAnonymous"], true);
    ...
    assert_eq!(session_body["user"]["isAnonymous"], true);
    Ok(())
}
...
async fn delete_anonymous_user_deletes_user_and_expires_cookie(
    ...
    assert!(set_cookie_values(&response)
        .iter()
        .any(|cookie| cookie.starts_with("rustauth.session_token=; Max-Age=0")));
    Ok(())
}
