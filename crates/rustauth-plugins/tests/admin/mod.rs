...
#[test]
fn admin_options_builder_validates_admin_roles() -> Result<(), Box<dyn std::error::Error>> {
    let invalid = AdminOptions::builder()
        .admin_roles(vec!["missing-role".to_owned()])
        .build();
    assert!(invalid.is_err());

    let options = AdminOptions::builder()
        .admin_role("admin")
        .default_role("user")
        .build()?;
    assert_eq!(options.default_role, "user");
    assert!(options.admin_roles.contains(&"admin".to_owned()));
    Ok(())
}
