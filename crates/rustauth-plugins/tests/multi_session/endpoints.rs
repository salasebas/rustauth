...
use rustauth_core::options::{
    RustAuthOptions, SessionAdditionalField, SessionOptions, UserAdditionalField, UserOptions,
};
use rustauth_core::session::DbSessionStore;
use rustauth_plugins::multi_session::{MultiSessionOptions, INVALID_SESSION_TOKEN};
...
async fn list_device_sessions_returns_configured_additional_fields(
    ...
    let fixture =
        Fixture::with_options(MultiSessionOptions::default(), additional_field_options()).await?;
    let response = fixture
        .sign_in("ada@example.com", "secret123", None)
        .await?;
...
async fn set_active_accepts_multi_session_cookie_without_active_cookie(
    ...
    let cookie = cookie_header_from_response(&response)
        .split("; ")
        .filter(|cookie| !cookie.starts_with("rustauth.session_token="))
        .collect::<Vec<_>>()
        .join("; ");
...
async fn revoke_active_session_deletes_active_cookie_when_no_next_session(
    ...
    assert!(set_cookie_values(&response).iter().any(|cookie| {
        cookie.starts_with("rustauth.session_token=;") && cookie.contains("Max-Age=0")
    }));
    Ok(())
}
...
async fn set_active_preserves_dont_remember_session_cookie(
    ...
    let active_cookie = cookies
        .iter()
        .find(|cookie| cookie.starts_with("rustauth.session_token="))
        .ok_or("missing active session cookie")?;

    assert!(!active_cookie.contains("Max-Age="));
    assert!(cookies
        .iter()
        .any(|cookie| cookie.starts_with("rustauth.dont_remember=")));
    Ok(())
}
...
{
    let fixture = Fixture::with_cookie_cache(MultiSessionOptions::default()).await?;
    let response = fixture
        .sign_in("ada@example.com", "secret123", None)
        .await?;
    let token = response_token(&response)?;
