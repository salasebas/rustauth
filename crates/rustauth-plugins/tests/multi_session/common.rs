...
use rustauth_core::cookies::sign_cookie_value;
use rustauth_core::db::{Create, DbAdapter, DbRecord, DbValue, MemoryAdapter, User};
use rustauth_core::error::RustAuthError;
use rustauth_core::options::{
    AdvancedOptions, CookieCacheOptions, RustAuthOptions, SessionOptions,
};
use rustauth_core::session::{CreateSessionInput, DbSessionStore};
use rustauth_core::test_utils::{fast_hash_password, with_integration_test_defaults};
...
impl Fixture {
    pub async fn new(config: MultiSessionOptions) -> Result<Self, Box<dyn std::error::Error>> {
        Self::with_options(config, RustAuthOptions::default()).await
    }
    ...
    pub async fn with_options(
        ...
        let context = create_auth_context_with_adapter(
            with_integration_test_defaults(RustAuthOptions {
                ...
                plugins: vec![multi_session(config)],
    ...
    pub async fn sign_in(
        ...
        password: &str,
        cookie: Option<&str>,
    ) -> Result<http::Response<Vec<u8>>, RustAuthError> {
...
pub fn multi_cookie_name(token: &str) -> String {
    format!("rustauth.session_token_multi-{}", token.to_lowercase())
}

pub fn signed_multi_cookie(token: &str) -> Result<String, RustAuthError> {
    Ok(format!(
        "{}={}",
