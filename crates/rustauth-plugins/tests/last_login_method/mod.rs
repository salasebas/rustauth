...
use rustauth_core::options::{AdvancedOptions, CookieConfig, RustAuthOptions};
use rustauth_plugins::last_login_method::{
...
fn default_resolver_matches_upstream_login_routes() {
    ...
    for (path, expected) in cases {
        ...
        assert_eq!(
            rustauth_plugins::last_login_method::default_login_method(&context).as_deref(),
            expected
        );
    }
}
...
fn default_resolver_ignores_missing_or_unknown_path() {
    assert_eq!(
        rustauth_plugins::last_login_method::default_login_method(&LoginMethodContext::new("")),
        None
    );
    assert_eq!(
        rustauth_plugins::last_login_method::default_login_method(&LoginMethodContext::new(
...
async fn async_after_hook_sets_last_method_cookie_when_session_cookie_is_created(
    ...
    assert!(last_method.contains(&format!(
        "Max-Age={}",
        DEFAULT_COOKIE_MAX_AGE.whole_seconds()
    )));
    ...
    let context = create_auth_context(RustAuthOptions {
        ...
        advanced: rustauth_core::options::AdvancedOptions {
            default_cookie_attributes: rustauth_core::options::CookieAttributesOverride {
                ...
                ..rustauth_core::options::CookieAttributesOverride::default()
            },
...
async fn sign_in_email_persists_last_login_method_and_get_session_returns_it(
    ...
    assert_eq!(body["user"]["lastLoginMethod"], "email");
    Ok(())
}
...
async fn sign_in_sets_last_method_cookie_on_localhost_cross_origin_development(
    ...
    let router = router_with_plugin_options(
        ...
        RustAuthOptions {
            ...
            ..RustAuthOptions::default().development(true)
        },
    )?;
