...
use rustauth_plugins::custom_session::{custom_session, CustomSessionOptions, UPSTREAM_PLUGIN_ID};
...
async fn callback_receives_context_and_can_read_request() -> Result<(), Box<dyn std::error::Error>>
{
    let adapter = Arc::new(TestAdapter::default());
    let cookie = seed_authenticated_session(&adapter).await?;
    let plugin = custom_session(CustomSessionOptions::default(), |_input, context| {
        Box::pin(async move {
            Ok(json!({
...
async fn user_preserves_additional_fields() -> Result<(), Box<dyn std::error::Error>> {
    ...
    adapter
        ...
        .await;
    let plugin = custom_session(CustomSessionOptions::default(), |input, _context| {
        Box::pin(async move { Ok(json!({ "role": input.user["role"] })) })
    });
    let router = router(
        adapter,
        plugin,
...
async fn list_device_sessions_is_not_mutated_by_default() -> Result<(), Box<dyn std::error::Error>>
{
    let adapter = Arc::new(TestAdapter::default());
    let plugin = custom_session(CustomSessionOptions::default(), |_input, _context| {
        Box::pin(async { Ok(json!({ "mutated": true })) })
    });
