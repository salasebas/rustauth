...
use rustauth_plugins::siwe::{siwe, SiweOptions, SiweVerifyMessageArgs};
...
#[test]
fn plugin_exposes_serializable_non_callback_options_metadata() -> Result<(), RustAuthError> {
    let plugin = siwe(
        options()
            .anonymous(false)
            .email_domain_name("wallet.example.com")
            .schema(rustauth_plugins::siwe::SiweSchemaOptions::new().table_name("wallet_address")),
    )?;
    let metadata = plugin
        .options
        .ok_or_else(|| RustAuthError::Api("siwe plugin options metadata missing".to_owned()))?;
...
#[test]
fn plugin_endpoint_registry_includes_siwe_operation_ids() -> Result<(), RustAuthError> {
...
fn public_types_include_wallet_address_and_extended_cacao_fields() {
    let _wallet = rustauth_plugins::siwe::WalletAddress {
    ...
    let cacao = rustauth_plugins::siwe::Cacao {
        h: rustauth_plugins::siwe::CacaoHeader {
            t: "caip122".to_owned(),
        },
        p: rustauth_plugins::siwe::CacaoPayload {
        ...
        s: rustauth_plugins::siwe::CacaoSignature {
            t: "eip1271".to_owned(),
