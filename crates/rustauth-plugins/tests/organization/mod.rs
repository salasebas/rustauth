...
use rustauth_plugins::organization::{
    has_permission, organization, provision_organization_member, MemberHookData, OrganizationHooks,
    OrganizationOptions, OrganizationPermission, OrganizationRole, OrganizationSchemaOptions,
...
fn exposes_organization_plugin_surface() -> Result<(), Box<dyn std::error::Error>> {
    assert_eq!(
        rustauth_plugins::organization::UPSTREAM_PLUGIN_ID,
        "organization"
    );

    let plugin = organization(OrganizationOptions::default());
...
fn organization_schema_registers_core_tables_and_session_field(
    ...
    let context = create_auth_context_with_adapter(
        RustAuthOptions {
            plugins: vec![organization(OrganizationOptions::default())],
...
fn test_router(
    ...
    let adapter_dyn: Arc<dyn rustauth_core::db::DbAdapter> = adapter;
    let context = create_auth_context_with_adapter(
        with_integration_test_defaults(RustAuthOptions {
            plugins: vec![organization(options)],
...
) -> Result<(), rustauth_plugins::access::AccessError> {
    let roles = rustauth_plugins::organization::access::default_roles()?;
...
#[test]
fn organization_owner_can_delete_organization() -> Result<(), rustauth_plugins::access::AccessError>
{
    let owner = rustauth_plugins::organization::access::owner_role()?;

    assert_eq!(
        owner.authorize_all(rustauth_plugins::access::request([(
            "organization",
            ["delete"]
        )])),
        Ok(())
...
fn organization_admin_can_update_but_not_delete_organization(
    ...
    let admin = rustauth_plugins::organization::access::admin_role()?;

    assert_eq!(
        admin.authorize_all(rustauth_plugins::access::request([(
            "organization",
            ["update"]
        )])),
        Ok(())
    ...
    assert_eq!(
        ...
        Err(
            rustauth_plugins::access::AccessError::UnauthorizedResource {
...
fn organization_member_can_read_access_control_only(
    ...
    let member = rustauth_plugins::organization::access::member_role()?;

    assert_eq!(
        member.authorize_all(rustauth_plugins::access::request([("ac", ["read"])])),
        Ok(())
    );
    assert_eq!(
        member.authorize_all(rustauth_plugins::access::request([("member", ["create"])])),
        Err(
            rustauth_plugins::access::AccessError::UnauthorizedResource {
