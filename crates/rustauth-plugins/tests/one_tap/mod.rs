...
use rustauth_core::api::{core_auth_async_endpoints, AuthRouter, Body};
...
use rustauth_plugins::one_tap::{one_tap, OneTapOptions};
...
fn exposes_one_tap_plugin_builder() {
    ...
    assert_eq!(rustauth_plugins::one_tap::UPSTREAM_PLUGIN_ID, "one-tap");
    ...
    let router = router_with_options(
        ...
        vec![additional_fields(
            AdditionalFieldsOptions::new().user_field(
                "role",
                AdditionalField::new(rustauth_core::db::DbFieldType::String)
...
fn router(
    ...
    router_with_options(adapter, options, RustAuthOptions::default(), vec![])
}
...
fn router_with_options(
    ...
    extra_plugins: Vec<rustauth_core::plugin::AuthPlugin>,
) -> Result<AuthRouter, RustAuthError> {
...
trait AdapterSeed {
    fn insert_user(&self, user: rustauth_core::db::User) -> AdapterFuture<'_, ()>;
...
fn test_user() -> rustauth_core::db::User {
    let now = OffsetDateTime::now_utc();
    rustauth_core::db::User {
        id: "user_1".to_owned(),
        name: "Ada".to_owned(),
...
fn user_record(user: rustauth_core::db::User) -> DbRecord {
