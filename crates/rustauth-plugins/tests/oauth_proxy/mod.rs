...
use rustauth_plugins::oauth_proxy::{oauth_proxy, OAuthProxyOptions};
...
fn exposes_oauth_proxy_plugin_id() {
    assert_eq!(
        rustauth_plugins::oauth_proxy::UPSTREAM_PLUGIN_ID,
        "oauth-proxy"
    );
}
...
#[tokio::test]
async fn rustauth_url_sets_production_redirect_uri() -> Result<(), Box<dyn std::error::Error>> {
    let _env = TestEnv::set([("RUSTAUTH_URL", "https://login.example.com")]).await;
    let router = router(
...
#[tokio::test]
async fn rustauth_url_is_used_for_token_exchange_redirect_uri(
) -> Result<(), Box<dyn std::error::Error>> {
    let _env = TestEnv::set([("RUSTAUTH_URL", "https://login.example.com")]).await;
    let router = router(
        Arc::new(MemoryAdapter::default()),
...
async fn oauth2_sign_in_route_uses_oauth_proxy() -> Result<(), Box<dyn std::error::Error>> {
    let router = router_with_options(
        ...
        RustAuthOptions {
            ...
            plugins: vec![
                oauth_proxy(OAuthProxyOptions::new().current_url("http://preview.example.com")),
    ...
    let sign_in = router
        .handle_async(json_request(
            ...
            r#"{"providerId":"custom-idp","callbackURL":"/dashboard"}"#,
        )?)
        .await?;
    let body: Value = serde_json::from_slice(sign_in.body())?;
    let provider_url = body["url"].as_str().ok_or("missing provider url")?;
    ...
    assert_eq!(
        query_value(provider_url, "redirect_uri").as_deref(),
        Some("https://login.example.com/api/auth/oauth2/callback/custom-idp")
    );
    assert!(query_value(provider_url, "state").is_some());
    Ok(())
}
...
async fn vendor_env_sets_current_preview_callback() -> Result<(), Box<dyn std::error::Error>> {
    let _env = TestEnv::set([
        ("RUSTAUTH_URL", "https://login.example.com"),
...
async fn upstream_vendor_env_names_set_current_preview_callback(
    ...
    for (key, value, expected) in [
        ...
        let _env =
            TestEnv::set([("RUSTAUTH_URL", "https://login.example.com"), (key, value)]).await;
        let router = router(
            Arc::new(MemoryAdapter::default()),
            "https://fallback.example.com/api/auth",
            OAuthProxyOptions::new(),
...
async fn rejects_invalid_and_expired_profiles() -> Result<(), Box<dyn std::error::Error>> {
    let router = router(
        Arc::new(MemoryAdapter::default()),
        "http://preview.example.com/api/auth",
        OAuthProxyOptions::new().max_age(time::Duration::seconds(5)),
    )?;
    let invalid = router
        .handle_async(json_request(
            Method::GET,
