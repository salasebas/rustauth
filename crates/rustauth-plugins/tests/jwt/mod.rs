...
use rustauth_plugins::jwt::{
    jwt, sign_jwt, to_exp_jwt, verify_jwt, JwkAlgorithm, JwtClaims, JwtJwksOptions, JwtOptions,
    JwtSigningOptions, TimeInput, UPSTREAM_PLUGIN_ID,
};
...
async fn jwks_rotation_keeps_keys_during_grace_period() -> Result<(), Box<dyn std::error::Error>> {
    let adapter = Arc::new(MemoryAdapter::new());
    let context = create_auth_context_with_adapter(
        options_with_plugin(jwt(JwtOptions {
            jwks: JwtJwksOptions {
