...
) -> Result<(Arc<TestAdapter>, AuthRouter), RustAuthError> {
    router_with_plugin_and_options(plugin, RustAuthOptions::default())
}
...
pub(super) async fn seed_user_and_credential_account(
    ...
    adapter
        .create(create_query(
            "account",
            credential_account_record("user_1", &fast_hash_password("secret123")?, now),
        ))
        .await?;
    Ok(())
}
