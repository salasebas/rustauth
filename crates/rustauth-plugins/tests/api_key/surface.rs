...
use rustauth_plugins::api_key::{
    api_key, ApiKeyConfiguration, ApiKeyOptions, ApiKeyReference, ApiKeyStorageMode, API_KEY_MODEL,
    API_KEY_TABLE, UPSTREAM_PLUGIN_ID,
};
use serde_json::json;
...
fn exposes_api_key_plugin_surface() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plugin = api_key(ApiKeyOptions::default())?;
