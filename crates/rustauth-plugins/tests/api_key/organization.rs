...
use rustauth_plugins::api_key::{
    api_key, ApiKeyConfiguration, ApiKeyOptions, ApiKeyReference, INSUFFICIENT_API_KEY_PERMISSIONS,
    INVALID_REFERENCE_ID_FROM_API_KEY, KEY_NOT_FOUND, ORGANIZATION_PLUGIN_REQUIRED,
...
async fn organization_owned_keys_require_membership_and_do_not_mock_sessions(
    ...
    let router = test_router_with_plugins(
        adapter,
        vec![organization(OrganizationOptions::default()), api_key_plugin],
    )?;
...
async fn organization_custom_api_key_permission_controls_org_key_access(
    ...
    let organization_plugin = organization(
