...
use rustauth_plugins::api_key::{
    api_key, ApiKeyConfiguration, ApiKeyExpirationOptions, ApiKeyOptions, ApiKeyReference,
    StartingCharactersConfig, API_KEY_MODEL, EXPIRES_IN_IS_TOO_LARGE, EXPIRES_IN_IS_TOO_SMALL,
    INVALID_PREFIX_LENGTH, KEY_NOT_FOUND, NAME_REQUIRED, NO_VALUES_TO_UPDATE,
    REFILL_INTERVAL_AND_AMOUNT_REQUIRED, SERVER_ONLY_PROPERTY, UNAUTHORIZED_SESSION,
};
...
async fn create_verify_get_list_update_and_delete_user_api_key(
    ...
    assert_ne!(
        rows[0].get("key").and_then(|value| match value {
            rustauth_core::db::DbValue::String(value) => Some(value.as_str()),
            _ => None,
        }),
        Some(key),
    );
...
async fn update_rejects_empty_patch_and_disabled_keys_do_not_verify(
    ...
    assert_eq!(
        ...
        rustauth_plugins::api_key::KEY_DISABLED
    );
    Ok(())
}
