...
fn role() -> Result<rustauth_plugins::access::Role, AccessError> {
...
fn validates_or_connector_for_specific_resource() -> Result<(), AccessError> {
    let mut allowed = rustauth_plugins::access::AccessRequest::new();
    allowed.insert(
        "project".to_string(),
    ...
    let mut denied = rustauth_plugins::access::AccessRequest::new();
...
fn authorizes_resource_with_any_request() -> Result<(), AccessError> {
    let mut request = rustauth_plugins::access::AccessRequest::new();
    request.insert(
        "project".to_string(),
        ResourceRequest::any(["create", "delete-many"]),
    );
...
fn rejects_empty_requests() -> Result<(), AccessError> {
    assert_eq!(
        role()?.authorize(
            rustauth_plugins::access::AccessRequest::new(),
            Connector::And
        ),
