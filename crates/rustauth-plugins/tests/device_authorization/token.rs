...
use rustauth_core::options::SessionAdditionalField;
...
async fn approved_token_session_uses_secondary_storage_when_configured(
    ...
    let mut auth_options = RustAuthOptions::default().secondary_storage(storage.clone());
    auth_options.session.store_session_in_database = true;
    let router = router_with_rustauth_options(
        adapter.clone(),
        DeviceAuthorizationOptions::default(),
        auth_options,
    )?;
...
async fn approved_token_session_records_request_metadata_and_additional_fields(
    ...
    let adapter = Arc::new(TestAdapter::default());
    let mut auth_options = RustAuthOptions::default();
...
async fn approved_token_records_new_session_for_after_hooks(
    ...
    let auth_options = RustAuthOptions {
        plugins: vec![
