...
fn exposes_device_authorization_plugin_id() {
    assert_eq!(
        rustauth_plugins::device_authorization::UPSTREAM_PLUGIN_ID,
...
fn schema_contributes_device_code_table() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let context = create_auth_context_with_adapter(
        RustAuthOptions {
            plugins: vec![device_authorization(DeviceAuthorizationOptions::default())?],
...
fn schema_options_customize_physical_table_and_field_names(
    ...
    let context = create_auth_context_with_adapter(
        RustAuthOptions {
            plugins: vec![device_authorization(
                DeviceAuthorizationOptions::default().schema(
                    rustauth_plugins::device_authorization::DeviceAuthorizationSchemaOptions::new()
