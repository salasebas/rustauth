# Upstream Parity: openauth-telemetry
...
`openauth-telemetry` tracks Better Auth's Node/server telemetry entry: publisher
...
| No endpoint behavior | ✅ | Hard no-op unless `OPENAUTH_TELEMETRY_ENDPOINT` or `TelemetryContext::custom_track` exists. |
...
| --- | --- | --- | --- |
| Env prefix | `BETTER_AUTH_TELEMETRY*` | `OPENAUTH_TELEMETRY*` | Keeps Rust crate configuration under the OpenAuth namespace. |
| Env false handling | `getBooleanEnvVar(..., false)` participates as an opt-in fallback | `OPENAUTH_TELEMETRY=false` or `0` is a hard opt-out | Fail-closed behavior for authentication-adjacent telemetry. |
...
| Collector ownership | Better Auth endpoint env | Deployer-supplied OpenAuth endpoint or custom sink | OpenAuth does not send telemetry to a maintainer endpoint by default. |
...
- `OPENAUTH_TELEMETRY=false` and `OPENAUTH_TELEMETRY=0` fail closed even when application options enable telemetry.
