...
| **Rust crate** | `crates/rustauth-cli/` |
...
RustAuth CLI matches Better Auth’s server SQL migration path: load auth config,
...
`rustauth.toml` instead of executable server config modules. Schema emitters
...
| `generate` | ✅ | SQL via `rustauth-sqlx`; matches Kysely SQL path including `--adapter kysely/sqlx --dialect`; no Prisma/Drizzle emitters by design |
| `migrate` | ✅ | sqlx apply, confirm/`--yes`; adds `--dry-run` and unsafe-plan guard |
| `init` (server config) | ➖ | Seeds `rustauth.toml` + env; generated TypeScript server auth module is out of scope |
...
| Config resolution | ✅ | Static `rustauth.toml` + `.env`/`.env.local`; executable TS config loading is intentionally not used |
...
| Telemetry (`cli_generate`, `cli_migrate`) | ✅ | Outcomes via `rustauth-telemetry` |
| Programmatic schema API | ➖ | Upstream exports `auth/api` for TypeScript generators; Rust uses crate APIs (`rustauth_sqlx`, `schema print`) instead |
...
| Surface | RustAuth (Rust) | Upstream (server) | Notes |
| --- | --- | --- | --- |
| **Total** | **70** | **219** | Rust: `cargo test -p rustauth-cli -- --list` (68 run by default; 2 Docker tests ignored) |
...
```bash
cargo nextest run -p rustauth-cli
# Postgres/MySQL migrate (ignored by default):
cargo nextest run -p rustauth-cli --run-ignored all
```
...
| --- | --- | --- | --- |
| Server config | Executable server config module + `get-config.ts` | Static `rustauth.toml` | Rust projects; explicit parse errors |
...
| Binaries | `better-auth`, `auth` | `rustauth` + compatibility aliases + `cargo-*` shims | Rust ecosystem |
...
- **Config:** missing `rustauth.toml` is non-fatal for read-only commands; parse failures fail closed.
...
| `src/generators/{index,kysely,types}.ts` | Audited | `schema.rs`, `db.rs`, `rustauth-sqlx` |
...
| `src/utils/config-paths.ts` | Audited | `paths.rs` (`possibleAuthConfigPaths` → fixed `rustauth.toml`) |
...
| --- | --- |
| `src/index.ts` | `src/bin/rustauth.rs`, `src/app.rs` |
...
| `src/generators/kysely.ts` | `src/db.rs`, `src/schema.rs`, `rustauth-sqlx`; includes `--adapter kysely/sqlx --dialect` mock generation |
