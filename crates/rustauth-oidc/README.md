...
OIDC relying-party helpers for RustAuth.

## What It Is

`rustauth-oidc` is for RustAuth acting as a client of external OpenID Connect
identity providers such as Okta, Microsoft Entra ID, Auth0, Google Workspace,
or Keycloak.

It is not an OAuth/OIDC authorization server. If you want your RustAuth
application to issue OAuth access tokens, ID tokens, discovery metadata, JWKS,
userinfo, or client credentials, enable `oauth-provider` on the `rustauth` crate
or use `rustauth-oauth-provider` directly.

The umbrella `rustauth` crate does not re-export `rustauth-oidc` at the root.
Enterprise SSO apps typically use `rustauth::sso` (which re-exports
`rustauth::sso::oidc`) or depend on `rustauth-oidc` when building custom
relying-party flows.
...
// `rustauth-sso` plugin wires one in automatically and blocks private/internal
// IPs by default).
...
Most applications do not wire this crate directly. Use `rustauth-sso` when you
want the full enterprise SSO plugin with provider storage, login routes, domain
verification, and account linking.

## How It Fits

- `rustauth-oidc`: low-level relying-party config, discovery, and redirect
  helpers.
- `rustauth-sso`: RustAuth plugin that uses this crate to consume external OIDC
  IdPs.
- `rustauth-oauth-provider`: RustAuth plugin that turns your app into an
  OAuth/OIDC provider.
...
with Better Auth 1.6.9 where it matters; RustAuth is not a line-by-line port.
...
- [SSO HTTP flow (`rustauth-sso`)](../rustauth-sso/README.md)
