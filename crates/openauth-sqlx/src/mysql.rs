...
use sqlx::{Arguments, MySql, MySqlPool, Row, Transaction};
...
struct MySqlState<'a, 'tx> {
    schema: &'a DbSchema,
    executor: MySqlExecutor<'a, 'tx>,
}

enum MySqlExecutor<'a, 'tx> {
    Pool(&'a MySqlPool),
...
impl MySqlState<'_, '_> {
    ...
    async fn count(mut self, query: Count) -> Result<u64, OpenAuthError> {
        ...
        u64::try_from(count)
            .map_err(|_| OpenAuthError::Adapter("mysql returned a negative count".to_owned()))
    }
    ...
    async fn update(mut self, query: Update) -> Result<Option<DbRecord>, OpenAuthError> {
        ...
        let select_where_sql = where_sql(table, &query.where_clauses, &mut select_args)?;
        ...
        let Some(row) = self.fetch_optional(select_sql, select_args).await? else {
            return Ok(None);
        };
        let mut record = row_record(&row, &selection)?;

        let mut update_args = MySqlArguments::default();
        let mut assignments = Vec::new();
        for (field, value) in &query.data {
            let (_, metadata) = resolve_field(table, field)?;
            assignments.push(format!("{} = ?", quote_identifier(&metadata.name)?));
            bind_value(&mut update_args, metadata, value)?;
            record.insert(field.clone(), value.clone());
        }
        let update_where = where_sql(table, &query.where_clauses, &mut update_args)?;
        let sql = format!(
            "UPDATE {} SET {}{} LIMIT 1",
            quote_identifier(&table.name)?,
            assignments.join(", "),
        ...
        self.execute(sql, update_args).await?;
        Ok(Some(record))
    }
    ...
    async fn execute(&mut self, sql: String, args: MySqlArguments) -> Result<u64, OpenAuthError> {
...
fn mysql_type(logical_name: &str, field: &DbField) -> String {
...
fn row_record(row: &MySqlRow, selection: &[(&str, &DbField)]) -> Result<DbRecord, OpenAuthError> {
    selection
        .iter()
...
fn row_value(row: &MySqlRow, field: &DbField) -> Result<DbValue, OpenAuthError> {
    match field.field_type {
...
fn validate_identifier(identifier: &str) -> Result<(), OpenAuthError> {
    ...
    let Some(first) = chars.next() else {
        return Err(OpenAuthError::Adapter(
            "mysql identifier cannot be empty".to_owned(),
        ));
    };
...
fn invalid_identifier(identifier: &str) -> OpenAuthError {
    OpenAuthError::Adapter(format!("invalid mysql identifier `{identifier}`"))
}
