...
#[tokio::test]
async fn postgres_adapter_satisfies_shared_adapter_contract() -> Result<(), OpenAuthError> {
...
#[tokio::test]
async fn postgres_adapter_filters_sorts_limits_and_counts_records() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn postgres_adapter_treats_like_wildcards_as_literal_user_input() -> Result<(), OpenAuthError>
{
    let adapter = adapter().await?;
...
#[tokio::test]
async fn postgres_adapter_create_schema_writes_requested_schema_file() -> Result<(), OpenAuthError>
{
    let schema = test_schema();
...
#[tokio::test]
async fn postgres_adapter_updates_and_deletes_matching_records() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn postgres_rate_limit_store_rejects_negative_persisted_counts() -> Result<(), OpenAuthError>
{
    let prefix = unique_prefix();
    let rate_limits_table = format!("{prefix}_rate_limits");
    let schema = auth_schema(AuthSchemaOptions {
...
#[tokio::test]
async fn postgres_adapter_rolls_back_failed_transactions() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn postgres_adapter_supports_where_operators() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn postgres_adapter_supports_forward_reverse_and_limited_joins() -> Result<(), OpenAuthError>
{
    let adapter = adapter().await?;
...
#[tokio::test]
async fn postgres_adapter_supports_multi_joins_inside_transactions() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
    seed_join_user(&adapter, "user_1", "ada@example.com").await?;
    seed_join_account(&adapter, "account_1", "user_1").await?;
    seed_join_session(&adapter, "session_1", "user_1").await?;
...
#[tokio::test]
async fn postgres_hooked_adapter_preserves_native_transaction_rollback() -> Result<(), OpenAuthError>
{
...
fn router(adapter: Arc<PostgresAdapter>) -> Result<AuthRouter, OpenAuthError> {
