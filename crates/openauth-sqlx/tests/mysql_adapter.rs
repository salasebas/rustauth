...
#[tokio::test]
async fn mysql_adapter_satisfies_shared_adapter_contract() -> Result<(), OpenAuthError> {
...
#[tokio::test]
async fn mysql_adapter_filters_sorts_limits_and_counts_records() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_adapter_treats_like_wildcards_as_literal_user_input() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_adapter_create_schema_writes_requested_schema_file() -> Result<(), OpenAuthError> {
    let schema = test_schema();
    let pool = test_pool(1).await?;
    let adapter = MySqlAdapter::with_schema(pool.clone(), schema.clone());
    let prefix = unique_prefix();
...
#[tokio::test]
async fn mysql_adapter_updates_and_deletes_matching_records() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_rate_limit_store_allows_exactly_one_concurrent_request() -> Result<(), OpenAuthError>
{
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_rate_limit_store_uses_physical_column_names() -> Result<(), OpenAuthError> {
    let prefix = unique_prefix();
...
#[tokio::test]
async fn mysql_adapter_uses_physical_names_from_auth_schema() -> Result<(), OpenAuthError> {
    let prefix = unique_prefix();
    let schema = auth_schema(AuthSchemaOptions {
...
#[tokio::test]
async fn mysql_adapter_rolls_back_failed_transactions() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_adapter_supports_where_operators() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_adapter_supports_forward_reverse_and_limited_joins() -> Result<(), OpenAuthError> {
    let adapter = adapter().await?;
...
fn router(adapter: Arc<MySqlAdapter>) -> Result<AuthRouter, OpenAuthError> {
