...
async fn restore_subscription_releases_active_schedule_and_clears_local_schedule(
    ...
    rustauth_core::db::DbAdapter::update(
        ...
        rustauth_core::db::Update::new("subscription")
            .where_clause(rustauth_core::db::Where::new(
                "id",
...
async fn cancel_subscription_preserves_trial_history_for_upgrade_guard(
    ...
    .subscription(SubscriptionOptions::enabled(vec![StripePlan::new("pro")
        .price_id("price_pro")
        .free_trial(rustauth_stripe::options::FreeTrialOptions::new(7))]));
    let plugin = stripe(options).unwrap();
...
async fn billing_portal_for_organization_uses_organization_customer(
    ...
    .organization(rustauth_stripe::options::OrganizationStripeOptions::enabled())
    .subscription(
            ...
            .authorize_reference(|input, _| {
                Box::pin(async move {
                    Ok(input.reference_id == "org_1"
                        && input.action
                            == rustauth_stripe::options::AuthorizeReferenceAction::BillingPortal)
                })
            }),
    );
    let plugin = stripe(options).unwrap();
        ...
        SubscriptionOptions::enabled(vec![StripePlan::new("pro").price_id("price_pro")])
            .authorize_reference(|input, _| {
                Box::pin(async move {
                    Ok(input.reference_id == "org_1"
                        && input.action
                            == rustauth_stripe::options::AuthorizeReferenceAction::CancelSubscription)
                })
            }),
    );
    let plugin = stripe(options).unwrap();
        ...
        SubscriptionOptions::enabled(vec![StripePlan::new("pro").price_id("price_pro")])
            .authorize_reference(|input, _| {
                Box::pin(async move {
                    Ok(input.reference_id == "org_1"
                        && input.action
                            == rustauth_stripe::options::AuthorizeReferenceAction::ListSubscription)
                })
            }),
    );
    let plugin = stripe(options).unwrap();
...
async fn restore_subscription_for_organization_clears_pending_cancel(
    ...
    let options = organization_options_for_action(
        Arc::clone(&transport) as Arc<dyn StripeTransport>,
        rustauth_stripe::options::AuthorizeReferenceAction::RestoreSubscription,
    );
    let plugin = stripe(options).unwrap();
    let endpoint = plugin
        .endpoints
...
fn organization_options_for_action(
    ...
    expected_action: rustauth_stripe::options::AuthorizeReferenceAction,
) -> StripeOptions {
    StripeOptions::new(
        StripeClient::with_transport("sk_test", transport),
        "whsec_test",
...
async fn set_active_organization(
    ...
    rustauth_core::db::DbAdapter::update(
        adapter,
        rustauth_core::db::Update::new("session")
            .where_clause(rustauth_core::db::Where::new(
                "token",
