...
use rustauth_core::db::{Create, DbAdapter, DbValue, MemoryAdapter, User};
...
use rustauth_stripe::stripe_api::{StripeClient, StripeTransport};
use std::sync::Arc;
...
pub fn plugin_endpoint<'a>(
    plugin: &'a rustauth_core::plugin::AuthPlugin,
    path: &str,
) -> Option<&'a rustauth_core::api::AsyncAuthEndpoint> {
    plugin
        .endpoints
        .iter()
...
pub fn stripe_plugin(transport: Arc<dyn StripeTransport>) -> rustauth_core::plugin::AuthPlugin {
