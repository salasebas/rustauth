...
use rustauth_core::db::{Create, DbAdapter, DbRecord, DbValue, MemoryAdapter};
...
async fn webhook_endpoint_wraps_event_hook_errors() -> Result<(), Box<dyn std::error::Error>> {
    let options = StripeOptions::new(StripeClient::new("sk_test"), "whsec_test").on_event(|_| {
        Box::pin(async { Err(rustauth_core::error::RustAuthError::Api("boom".to_owned())) })
    });
    let plugin = stripe(options).unwrap();
    let endpoint = plugin
        .endpoints
...
) -> Result<DbRecord, rustauth_core::error::RustAuthError> {
