...
use rustauth_core::cookies::{parse_set_cookie_header, sign_cookie_value};
...
use rustauth_core::options::{AdvancedOptions, RustAuthOptions, SessionOptions};
use rustauth_core::test_utils::{
    ...
    fn create<'a>(&'a self, query: Create) -> AdapterFuture<'a, DbRecord> {
        ...
        if should_inject {
            return Box::pin(async move {
                ...
                Err(RustAuthError::Adapter("duplicate key".to_owned()))
            });
        }
        self.inner.create(query)
    }
...
pub fn passkey_challenge_cookie_name() -> Result<String, Box<dyn std::error::Error>> {
    Ok(rustauth_core::cookies::create_auth_cookie(
        &RustAuthOptions {
            base_url: Some("http://localhost:3000".to_owned()),
            secret: Some("secret-a-at-least-32-chars-long!!".to_owned()),
            ..RustAuthOptions::default()
...
pub async fn session_cookie_for_created_at(
    ...
    let session = rustauth_core::db::Session {
    ...
    let user = rustauth_core::db::User {
        id: user_id.to_owned(),
        name: "Ada".to_owned(),
    ...
    let mut headers = cookies
        ...
        .collect::<Vec<_>>();
    headers.extend(
        rustauth_core::cookies::set_cookie_cache(
            &context.auth_cookies,
            &context.secret,
            &rustauth_core::cookies::CookieCachePayload {
                session,
                user,
...
impl PasskeyWebAuthnBackend for FakeWebAuthnBackend {
    ...
    ) -> Result<PasskeyRegistrationStart, rustauth_core::error::RustAuthError> {
    ...
    ) -> Result<PasskeyAuthenticationStart, rustauth_core::error::RustAuthError> {
        let mut options = json!({
