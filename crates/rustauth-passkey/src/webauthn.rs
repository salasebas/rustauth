...
pub trait PasskeyWebAuthnBackend: Send + Sync {
    ...
    ) -> Result<PasskeyRegistrationStart, RustAuthError>;
    ...
    fn finish_registration(
        ...
        response: Value,
        state: Value,
    ) -> Result<VerifiedPasskeyCredential, RustAuthError> {
    ...
    ) -> Result<PasskeyAuthenticationStart, RustAuthError>;
    ...
    ) -> Result<VerifiedAuthentication, RustAuthError> {
...
///
/// `WebauthnCore::new_unsafe_experts_only` is required here because RustAuth must
...
fn core(config: &WebAuthnConfig) -> Result<WebauthnCore, RustAuthError> {
    ...
    for origin in &config.origins {
        let url = Url::parse(origin).map_err(|error| RustAuthError::Api(error.to_string()))?;
...
fn option_value(options: CreationChallengeResponse) -> Result<Value, RustAuthError> {
    serde_json::to_value(options)
        .map(|mut value| value.pointer_mut("/publicKey").cloned().unwrap_or(value))
        .map_err(json_error)
}

fn auth_option_value(options: RequestChallengeResponse) -> Result<Value, RustAuthError> {
    serde_json::to_value(options)
        .map(|mut value| value.pointer_mut("/publicKey").cloned().unwrap_or(value))
        .map_err(json_error)
}
...
fn credential_value_to_passkey(value: Value) -> Result<Passkey, RustAuthError> {
    serde_json::from_value::<Passkey>(value).map_err(json_error)
}

fn parse_exclude_credential_id(value: Value) -> Result<CredentialID, RustAuthError> {
    ...
    let id = value
        ...
        .ok_or_else(|| RustAuthError::Api("invalid passkey exclude credential entry".to_owned()))?;
    serde_json::from_value(json!(id)).map_err(json_error)
}

/// Reconstruct `webauthn-rs` credential state from legacy passkey columns.
///
/// Rows created before RustAuth stored hidden `webauthn_credential` JSON only
...
pub(crate) fn legacy_passkey_credential_value(
    ...
    let cbor = serde_cbor_2::from_slice::<serde_cbor_2::Value>(&cose_bytes)
        .map_err(|error| RustAuthError::Api(error.to_string()))?;
    let cose_key =
        COSEKey::try_from(&cbor).map_err(|error| RustAuthError::Api(error.to_string()))?;
    ...
    let counter = u32::try_from(counter)
        .map_err(|_| RustAuthError::Api("passkey counter exceeds u32 range".to_owned()))?;
...
fn decode_stored_public_key(public_key: &str) -> Result<Vec<u8>, RustAuthError> {
    use base64::Engine;
...
fn parse_stored_transports(value: &str) -> Result<Vec<AuthenticatorTransport>, RustAuthError> {
    value
        .split(',')
        .map(str::trim)
        ...
        .map(|part| {
            AuthenticatorTransport::from_str(part)
                .map_err(|_| RustAuthError::Api(format!("unsupported passkey transport `{part}`")))
        })
        .collect()
}

fn credential_output(passkey: Passkey) -> Result<VerifiedPasskeyCredential, RustAuthError> {
...
fn cose_public_key_bytes(key: &COSEKey) -> Result<Vec<u8>, RustAuthError> {
...
fn cose_algorithm_id(algorithm: COSEAlgorithm) -> Result<i128, RustAuthError> {
    match algorithm {
        ...
        COSEAlgorithm::PinUvProtocol => Err(RustAuthError::Api(
...
mod tests {
    ...
    #[test]
    fn webauthn_builds_for_production_and_loopback_configs() -> Result<(), RustAuthError> {
