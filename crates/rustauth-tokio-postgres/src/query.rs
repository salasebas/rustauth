...
pub fn postgres_params(
    params: &[SqlParam],
) -> Result<Vec<Box<dyn ToSql + Sync + Send>>, RustAuthError> {
...
fn postgres_param(param: &SqlParam) -> Result<Box<dyn ToSql + Sync + Send>, RustAuthError> {
    match &param.value {
        DbValue::String(value) if param.generated_id == Some(IdGeneration::Uuid) => {
            let value = uuid::Uuid::parse_str(value).map_err(|error| {
                RustAuthError::Adapter(format!("invalid PostgreSQL UUID value `{value}`: {error}"))
            })?;
            Ok(Box::new(value))
        }
