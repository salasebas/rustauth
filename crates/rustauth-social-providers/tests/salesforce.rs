...
use rustauth_social_providers::ProviderIdentity;
use std::pin::Pin;
use std::sync::Arc;
...
use rustauth_social_providers::advanced::salesforce::{
...
fn salesforce_provider_exposes_upstream_metadata_and_production_endpoints() {
    let provider = salesforce(salesforce_options()).expect("provider should construct");
...
fn salesforce_authorization_url_uses_default_scopes_custom_scopes_and_pkce(
    ...
    let provider = salesforce(options).expect("provider should construct");
...
fn salesforce_authorization_url_can_disable_default_scopes_and_override_redirect(
    ...
    let provider = salesforce(SalesforceOptions {
        oauth: ProviderOptions {
            ...
            client_secret: Some(
                ClientSecret::new("salesforce-secret").expect("valid client secret"),
            ),
...
fn salesforce_authorization_url_requires_client_id_and_code_verifier() {
    assert!(matches!(
        salesforce(SalesforceOptions::default()),
        Err(rustauth_oauth::oauth2::OAuthError::MissingOption(
            "client_id"
        ))
    ));
...
#[tokio::test]
async fn salesforce_validate_authorization_code_requires_code_verifier() {
    let provider = salesforce(salesforce_options()).expect("provider should construct");
...
fn salesforce_profile_deserializes_and_maps_to_user_defaults(
    ...
    let provider = salesforce(salesforce_options()).expect("provider should construct");
    let user_info = provider.map_profile(profile);
...
fn salesforce_profile_image_falls_back_to_thumbnail() {
    let provider = salesforce(salesforce_options()).expect("provider should construct");
    let user_info = provider.map_profile(SalesforceProfile {
...
fn salesforce_partial_mapper_overrides_selected_user_fields() {
    let provider = salesforce(SalesforceOptions {
        ...
        map_profile_to_user: Some(Arc::new(|profile| {
            let mut patch =
                rustauth_social_providers::advanced::salesforce::SalesforceUserPatch::default();
        ...
        get_user_info: Some(Arc::new(|_tokens| {
            Box::pin(async {
                Ok(Some(SalesforceUserInfo {
                    user: rustauth_oauth::oauth2::OAuth2UserInfo {
...
fn provider_options() -> ProviderOptions {
    ProviderOptions {
        client_id: Some(ClientId::from("salesforce-client")),
        client_secret: Some(ClientSecret::new("salesforce-secret").expect("valid client secret")),
        ..ProviderOptions::default()
    }
}
