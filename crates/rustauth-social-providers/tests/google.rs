...
use rustauth_social_providers::advanced::google::{
    google, GoogleAccessType, GoogleAuthorizationCodeRequest, GoogleAuthorizationUrlRequest,
    GoogleDisplay, GoogleOptions, GoogleProfile, GoogleProvider,
};
...
fn authorization_url_requires_client_id_secret_and_code_verifier() {
    ...
    let missing_secret = google(GoogleOptions {
        oauth: ProviderOptions {
            client_id: Some(ClientId::from("google-client")),
            ..ProviderOptions::default()
        },
    ...
    let missing_verifier = google(valid_options()).expect("provider should construct");
...
async fn google_validate_authorization_code_requires_code_verifier() {
    let provider = google(valid_options()).expect("provider should construct");
