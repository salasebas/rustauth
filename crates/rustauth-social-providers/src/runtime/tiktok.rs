...
impl_social_oauth_provider!(
    crate::tiktok::TiktokProvider,
    options | provider | { provider.options().clone() },
    authorization | provider,
    input | {
        provider.create_authorization_url(crate::tiktok::TiktokAuthorizationUrlRequest {
            state: input.state,
    ...
    input | {
        provider
            .validate_authorization_code(crate::tiktok::TiktokValidateAuthorizationCodeRequest {
                code: input.code,
                redirect_uri: input.redirect_uri,
            })
            .await
