...
use rustauth_oauth::oauth2::{ClientId, ClientSecret, OAuthError, ProviderOptions};

/// Stable provider identifier used by RustAuth social sign-in routes.
...
impl ProviderId {
    ...
    /// Returns the stable string id registered with RustAuth.
...
impl SocialProviderConfig {
    ...
    /// Loads `{prefix}_CLIENT_ID` and `{prefix}_CLIENT_SECRET` from the environment.
    ///
    /// For providers that require an extra credential (for example TikTok's
    /// `{prefix}_CLIENT_KEY`), use [`Self::from_env_with_keys`].
    pub fn from_env(prefix: &str) -> Result<Self, OAuthError> {
        Self::from_env_with_keys(prefix, &[])
    }

    /// Loads standard OAuth credentials plus any optional extra keys from the environment.
    ///
    /// `extra_keys` are suffixes appended to `prefix` (for example `"CLIENT_KEY"`
    /// reads `{prefix}_CLIENT_KEY`). Missing optional keys are ignored.
    pub fn from_env_with_keys(prefix: &str, extra_keys: &[&str]) -> Result<Self, OAuthError> {
        let mut builder = SocialProviderConfig::builder()
            .client_id(required_env_var(&format!("{prefix}_CLIENT_ID"))?)
            .client_secret(required_env_var(&format!("{prefix}_CLIENT_SECRET"))?);

        for key in extra_keys {
            let env_name = format!("{prefix}_{key}");
            if let Ok(value) = env::var(&env_name) {
                if !value.is_empty() && *key == "CLIENT_KEY" {
                    builder = builder.client_key(value);
                }
            }
        }
...
fn required_env_var(name: &str) -> Result<String, OAuthError> {
    env::var(name).map_err(|error| match error {
        env::VarError::NotPresent => {
            OAuthError::InvalidConfiguration(format!("missing environment variable `{name}`"))
        }
        env::VarError::NotUnicode(_) => OAuthError::InvalidConfiguration(format!(
...
mod tests {
    ...
    #[test]
    fn from_env_loads_client_credentials() {
        with_env_vars(
            &[
            ...
            || {
                let config = SocialProviderConfig::from_env("GITHUB").expect("config should load");
                assert_eq!(config.client_id(), "github-id");
                assert_eq!(config.client_secret(), "github-secret");
    ...
    fn from_env_reports_missing_variables() {
        with_env_vars(
            ...
            || {
                let error = SocialProviderConfig::from_env("SPOTIFY")
                    .expect_err("missing env vars should fail");
                assert!(matches!(error, OAuthError::InvalidConfiguration(_)));
                assert!(error.to_string().contains("SPOTIFY_CLIENT_ID"));
            },
        );
    }

    #[test]
    fn from_env_with_keys_loads_optional_client_key() {
        with_env_vars(
            &[
                ("TIKTOK_CLIENT_ID", Some("tiktok-id")),
                ("TIKTOK_CLIENT_SECRET", Some("tiktok-secret")),
                ("TIKTOK_CLIENT_KEY", Some("tiktok-key")),
            ],
            || {
                let config = SocialProviderConfig::from_env_with_keys("TIKTOK", &["CLIENT_KEY"])
                    .expect("config should load");
                assert_eq!(config.client_id(), "tiktok-id");
                assert_eq!(config.client_secret(), "tiktok-secret");
                assert_eq!(config.client_key(), Some("tiktok-key"));
            },
        );
    }

    #[test]
    fn from_env_with_keys_ignores_missing_optional_client_key() {
        with_env_vars(
            &[
                ("TIKTOK_CLIENT_ID", Some("tiktok-id")),
                ("TIKTOK_CLIENT_SECRET", Some("tiktok-secret")),
