...
impl AtlassianProvider {
    #[deprecated(note = "use advanced::atlassian::atlassian() instead")]
    ...
    fn with_endpoints(
        ...
        use rustauth_oauth::oauth2::OAuthHttpClient;
...
mod tests {
    ...
    fn atlassian_provider_contract_and_authorization_url_match_upstream_behavior() {
        let provider =
            atlassian(AtlassianOptions::new("client-id", "client-secret")).expect("provider");
    ...
    fn authorization_url_appends_configured_scopes_and_prompt() {
        ...
        let provider = atlassian(options).expect("provider");
    ...
    fn authorization_url_validates_required_options() {
        assert!(matches!(
            atlassian(AtlassianOptions::default()),
