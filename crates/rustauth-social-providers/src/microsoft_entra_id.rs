...
/// Alias for [`microsoft_entra_id`].
pub fn microsoft(options: MicrosoftEntraIdOptions) -> Result<MicrosoftEntraIdProvider, OAuthError> {
    microsoft_entra_id(options)
}
...
impl MicrosoftEntraIdProvider {
    #[deprecated(
        note = "use advanced::microsoft_entra_id::microsoft_entra_id() or advanced::microsoft_entra_id::microsoft() instead"
    )]
