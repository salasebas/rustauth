# Upstream parity — rustauth-social-providers
...
| **Rust crate** | `crates/rustauth-social-providers/` |
...
| **Scope** | Server-side provider definitions (URLs, scopes, profile mapping, token auth). Out of scope: HTTP routes (`/sign-in/social`, callbacks) → [`rustauth-core`](../rustauth-core/UPSTREAM.md); OAuth client primitives → [`rustauth-oauth`](../rustauth-oauth/UPSTREAM.md). |
...
fail-closed hardening. RustAuth adds **326** provider-focused Rust tests where
...
| `SocialOAuthProvider` trait | 🎯 Extension | Async trait via `rustauth-oauth`; replaces upstream sync provider functions |
...
| Social sign-in HTTP routes | ➖ Out of scope | `/sign-in/social`, callbacks, account linking → `rustauth-core` |
| Social E2E (`social.test.ts`) | ➖ Out of scope | Route-level E2E owned by `rustauth-core`, not this crate |

## Test coverage

| Surface | RustAuth (Rust) | Upstream | Notes |
| --- | --- | --- | --- |
| Provider wire + mapping tests | **326** | **0** in `social-providers/` | `rg '#\[(tokio::)?test\]' crates/rustauth-social-providers` |
...
| Social route E2E | — | `social.test.ts` | Mapped to `rustauth-core/tests/api/routes/social_oauth.rs` |

```bash
cargo nextest run -p rustauth-social-providers
```
...
| SP-3 | OAuth route and account-linking E2E | ➖ Sibling scope | Owned by `rustauth-core`; this crate only owns provider definitions and runtime provider implementations |
...
- Outbound provider HTTP uses `rustauth-oauth` SSRF-aware clients where applicable.
...
| `better-auth` `social.test.ts` | `rustauth-core/tests/api/routes/social_oauth.rs` |
...
- [`rustauth-oauth`](../rustauth-oauth/UPSTREAM.md) — OAuth client primitives consumed by providers
- [`rustauth-core`](../rustauth-core/UPSTREAM.md) — social sign-in routes and account linking
