//! Telemetry-related environment variables use the **`RUSTAUTH_*`** prefix.
...
fn bool_env(suffix: &str, fallback: bool) -> bool {
    env_var(suffix)
...
pub fn telemetry_endpoint() -> Option<String> {
    env_var("TELEMETRY_ENDPOINT")
}

/// Three-state read of the `RUSTAUTH_TELEMETRY` master switch.
///
...
///
/// [`TelemetryOptions`]: rustauth_core::options::TelemetryOptions
pub fn telemetry_env_setting() -> Option<bool> {
    env_var("TELEMETRY").map(|value| parse_bool(&value))
}

pub fn telemetry_debug_env() -> bool {
    bool_env("TELEMETRY_DEBUG", false)
}
...
mod tests {
    ...
    #[test]
    fn telemetry_env_setting_reads_rustauth_prefix() {
        let _guard = lock_env();
        let _restore = EnvRestore::unset(&["RUSTAUTH_TELEMETRY"]);
        std::env::set_var("RUSTAUTH_TELEMETRY", "false");

        assert_eq!(telemetry_env_setting(), Some(false));
    }
