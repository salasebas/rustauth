...
use rustauth_core::options::{
    AccountLinkingOptions, AccountOptions, AdvancedOptions, EmailPasswordOptions, RustAuthOptions,
...
pub fn router_with_options(
    options: SsoOptions,
) -> Result<(Arc<MemoryAdapter>, AuthRouter), RustAuthError> {
...
pub fn router_with_options_and_account_linking(
    ...
    let router = AuthRouter::with_async_endpoints(context, Vec::new(), core_auth_async_endpoints());
    Ok((adapter, router?))
}
