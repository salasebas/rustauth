...
use crate::utils;
use rustauth_saml::SpBuildOptions;
    ...
    create_auth_endpoint(
            ...
            .openapi(OpenApiOperation::new(operation_id).tag("SSO")),
        {
            ...
            move |context, request| {
                let options = Arc::clone(&options);
                async move { handle_acs(&context, options, request).await }
            }
        },
    )
}
...
async fn handle_acs(
    ...
    options: Arc<SsoOptions>,
    request: ApiRequest,
) -> Result<ApiResponse, rustauth_core::error::RustAuthError> {
...
async fn load_saml_provider(
    ...
    provider_id: &str,
) -> Result<SamlProviderLoadResult, rustauth_core::error::RustAuthError> {
    let provider =
...
async fn parse_and_validate_saml_response(
    ...
    relay_state: Option<&str>,
    saml_response: &str,
) -> Result<Result<ValidatedSamlResponse, ApiResponse>, rustauth_core::error::RustAuthError> {
...
async fn verify_saml_signature(
    ...
    parsed: &ParsedSamlResponse,
    saml_response: &str,
) -> Result<Result<Option<VerifiedSamlSignature>, ApiResponse>, rustauth_core::error::RustAuthError>
{
...
struct SamlLoginInput<'a> {
    ...
    adapter: &'a dyn rustauth_core::db::DbAdapter,
