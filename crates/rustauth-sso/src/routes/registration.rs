    ...
    create_auth_endpoint(
            ...
            ),
        {
            ...
            move |context, request| {
                let options = Arc::clone(&options);
                async move {
                    let Some((adapter, user)) =
                        authenticated_session_user(&context, &request).await?
                    else {
                        return utils::json(
                    ...
                    let oidc_config = match body.oidc_config {
                        ...
                        .await
                        {
                            ...
                            Err(BuildOidcConfigError::Serialize(message)) => {
                                return Err(rustauth_core::error::RustAuthError::Api(message));
                            }
                        },
                        None => None,
                    };
                    ...
                    let created = store
                        ...
                        .await?;
                    let sanitized =
                        created.sanitized_with_options(&context.base_url, Some(&options));
