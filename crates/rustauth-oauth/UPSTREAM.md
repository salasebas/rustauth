...
`rustauth-oauth` tracks the low-level OAuth 2.0 and OIDC client helper surface
...
encryption; those map to `rustauth-core`, not this helper crate.
...
| OAuth state, account linking, token encryption | ➖ Out of scope | Upstream `packages/better-auth/src/oauth2/{state,link-account,utils,errors}.ts`; covered by `rustauth-core/src/auth/oauth/`. |
| HTTP routes, cookies, sessions, schema | ➖ Out of scope | Server runtime behavior covered by `rustauth-core`, provider crates, and integration crates. |
| Social provider implementations | ➖ Out of scope | Upstream `packages/core/src/social-providers/*` consumes these helpers; mapped by `rustauth-social-providers`. |
| Social routes, generic-oauth, and SSO/SAML OAuth usage | ➖ Out of scope | Server route/plugin behavior belongs to `rustauth-core`, `rustauth-plugins`, `rustauth-sso`, and provider crates. |
...
| --- | --- | --- | --- |
| Crate total | 63 by `rg '#\[(tokio::)?test\]' crates/rustauth-oauth` | 15 Vitest cases in `packages/core/src/oauth2/*.test.ts` | Local `cargo nextest run -p rustauth-oauth -- --list-tests` is unsupported by this nextest version; use the `rg` fallback for counts. Verify with `cargo nextest run -p rustauth-oauth`. |
...
| Runtime OAuth state/account linking/token encryption | Not in this crate | `packages/better-auth/src/oauth2/*.test.ts` and `social.test.ts` | Mapped to `rustauth-core/src/auth/oauth/`, not `rustauth-oauth`. |
| Social providers | Not in this crate | Provider-specific upstream tests and `social.test.ts` | Mapped to `rustauth-social-providers`; they consume this crate's helper contracts. |
| Social routes, generic-oauth, oauth-proxy, provider/OIDC server, SSO/SAML | Not in this crate | Covered by upstream route/plugin tests | Belongs to `rustauth-core`, `rustauth-plugins`, `rustauth-oauth-provider`, `rustauth-sso`, or OIDC/SAML crates. |
...
| OA-OAUTH-3 | Live provider conformance matrix is not maintained in this helper crate | ➖ | Provider-specific quirks belong to `rustauth-social-providers` and integration smoke tests. This crate covers shared OAuth wire contracts without requiring live third-party provider credentials. |
...
3. Compare exports from upstream core `index.ts` with
   `crates/rustauth-oauth/src/oauth2/mod.rs`.
4. Treat `packages/better-auth/src/oauth2/{state,link-account,utils,errors}.ts`
   as server runtime OAuth flow code for `rustauth-core/src/auth/oauth/`, not
   this crate.
5. Treat `packages/core/src/social-providers/*` as provider implementations for
   `rustauth-social-providers`; they are consumers of these primitives.
...
9. Compare upstream tests in `validate-token.test.ts` and
   ...
   `crates/rustauth-oauth/tests/oauth2_helpers.rs`, `src/oauth2/claims.rs`, and
   `src/oauth2/ssrf.rs`.
10. Verify with `cargo nextest run -p rustauth-oauth`.
...
| `packages/better-auth/src/oauth2/state.ts` | `rustauth-core/src/auth/oauth/state.rs` |
| `packages/better-auth/src/oauth2/link-account.ts` | `rustauth-core/src/auth/oauth/account_linking.rs` |
| `packages/better-auth/src/oauth2/utils.ts` | `rustauth-core/src/auth/oauth/tokens.rs` |
| `packages/better-auth/src/oauth2/errors.ts` | `rustauth-core/src/auth/oauth/errors.rs` |
| `packages/core/src/social-providers/*` | `rustauth-social-providers/src/*` |
| `packages/better-auth/src/api/routes/{sign-in,callback,account}.ts` | `rustauth-core` social/OAuth route layer |
| `packages/better-auth/src/plugins/generic-oauth/*` | `rustauth-plugins` / provider crates |
| `packages/sso/src/routes/*` OAuth usage | `rustauth-sso`, `rustauth-oidc`, `rustauth-saml` |
| `packages/oauth-provider/*`, `plugins/oidc-provider/*`, `plugins/oauth-proxy/*` | `rustauth-oauth-provider`, SSO/OIDC/plugin crates |
