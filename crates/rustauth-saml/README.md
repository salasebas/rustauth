...
SAML 2.0 service-provider helpers for RustAuth enterprise SSO.

## Status

This crate provides SAML SP building blocks used by `rustauth-sso`. Cryptography
...
The `rustauth-sso` `saml` feature enables `rustauth-saml/saml-signed`.
...
- XML hardening, timestamp validation, destination/recipient checks, and replay
  state key helpers.
- Algorithm policy types and stable RustAuth error codes.

## Quick Start

Use `rustauth-sso` for the plugin-level SAML routes. Depend on
`rustauth-saml` directly only when you need the lower-level SAML helpers.
...
Enable SAML routes through the `rustauth-sso` `saml` feature:

```toml
rustauth-sso = { version = "...", features = ["saml"] }
```

## How It Fits

- `rustauth-saml`: low-level SAML service-provider helpers + `opensaml` adapter.
- `rustauth-sso`: enterprise SSO plugin that composes OIDC and optional SAML
  routes.
- `rustauth-oidc`: recommended relying-party path for modern enterprise IdPs.
...
- [SSO HTTP parity (`rustauth-sso`)](../rustauth-sso/UPSTREAM.md)
