...
pub use rustauth_core::api::{core_auth_async_endpoints, AuthRouter};
pub use rustauth_core::context::create_auth_context_with_adapter;
pub use rustauth_core::cookies::{set_session_cookie, Cookie, SessionCookieOptions};
pub use rustauth_core::db::{
    Create, DbAdapter, DbRecord, DbValue, Delete, FindMany, MemoryAdapter, Session, Update, Where,
};
pub use rustauth_core::error::RustAuthError;
pub use rustauth_core::options::{AdvancedOptions, RateLimitRule, RustAuthOptions};
pub use rustauth_core::plugin::AuthPlugin;
pub use rustauth_oauth_provider::mcp::{
...
pub use rustauth_oauth_provider::{
    apply_jwt_metadata_defaults, delete_consent, find_consent, has_granted_scopes, oauth_provider,
...
pub fn signed_oauth_query(exp: i64) -> Result<String, RustAuthError> {
...
pub fn default_jwt_plugin() -> Result<AuthPlugin, RustAuthError> {
    rustauth_plugins::jwt::jwt(rustauth_plugins::jwt::JwtOptions::default())
}

pub fn default_plugins() -> Result<Vec<AuthPlugin>, RustAuthError> {
    Ok(vec![
        default_jwt_plugin()?,
        default_provider().map_err(|error| RustAuthError::InvalidConfig(error.to_string()))?,
    ])
}
...
pub fn options_with_provider(plugin: AuthPlugin) -> RustAuthOptions {
    options_with_plugins(vec![plugin])
}

pub fn options_with_plugins(plugins: Vec<AuthPlugin>) -> RustAuthOptions {
    RustAuthOptions {
...
pub fn router_with_default_plugins(
    adapter: Arc<MemoryAdapter>,
) -> Result<AuthRouter, RustAuthError> {
    let context = create_auth_context_with_adapter(
        options_with_plugins(default_plugins()?),
        adapter.clone(),
    )?;
...
pub fn router(
    ...
    if plugins[0].id == "oauth-provider" {
        plugins.insert(0, default_jwt_plugin()?);
    }
    let context = create_auth_context_with_adapter(options_with_plugins(plugins), adapter.clone())?;
    AuthRouter::with_async_endpoints(context, Vec::new(), core_auth_async_endpoints())
}
...
pub async fn seed_user_session(adapter: &MemoryAdapter) -> Result<(), RustAuthError> {
...
pub fn signed_session_cookie(token: &str) -> Result<String, RustAuthError> {
    let context = create_auth_context_with_adapter(
        RustAuthOptions {
            secret: Some(SECRET.to_owned()),
            ..RustAuthOptions::default()
