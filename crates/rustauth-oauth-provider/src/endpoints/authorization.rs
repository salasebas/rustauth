...
pub(super) async fn load_pending_authorization(
    ...
    options: &ResolvedOAuthProviderOptions,
    request_id: &str,
) -> Result<Option<PendingAuthorizationValue>, RustAuthError> {
...
pub(super) async fn issue_authorization_code_redirect(
    ...
    let redirect_uri = value
        .redirect_uri
        .as_deref()
        .ok_or_else(|| RustAuthError::Api("authorization redirect_uri is required".to_owned()))?;
    let mut redirect =
        url::Url::parse(redirect_uri).map_err(|error| RustAuthError::Api(error.to_string()))?;
...
pub(super) fn page_redirect_with_request_id(
    ...
    let base = url::Url::parse(base_url).map_err(|error| RustAuthError::Api(error.to_string()))?;
