    ...
    .await?
    {
        ...
        AuthorizeDecision::RedirectError { error, description } => {
            let redirect_uri = pending
                ...
                .as_deref()
                .ok_or_else(|| {
                    RustAuthError::Api("authorization redirect_uri is required".to_owned())
                })?;
...
fn consent_owner_error_response() -> Result<ApiResponse, RustAuthError> {
...
fn parse_consent_scope_update(
    update: Option<&serde_json::Value>,
) -> Result<Option<Vec<String>>, RustAuthError> {
