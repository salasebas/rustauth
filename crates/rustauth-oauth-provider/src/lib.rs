...
pub use rustauth_plugins::jwt::JwtOptions;
...
use rustauth_core::plugin::{AuthPlugin, PluginInitOutput, PluginRateLimitRule};
...
pub fn oauth_provider(
    ...
    auth_plugin = auth_plugin.with_init(move |context| {
        if shared_for_init.disable_jwt_plugin {
            return Ok(PluginInitOutput::default());
        }
        if !context.has_plugin("jwt") {
            return Err(RustAuthError::InvalidConfig(
                "oauth-provider requires the jwt plugin when disable_jwt_plugin is false; \
                 register jwt(...) in plugins before oauth_provider(...)"
                    .to_owned(),
            ));
        }
...
pub fn apply_jwt_metadata_defaults(resolved: &mut ResolvedOAuthProviderOptions, jwt: &JwtOptions) {
...
fn rate_limit_rules(options: &OAuthProviderRateLimits) -> Vec<PluginRateLimitRule> {
    [
        (
            "/oauth2/token",
            RateLimitRule::new(time::Duration::seconds(60), 20),
            &options.token,
        ),
        (
            "/oauth2/authorize",
            RateLimitRule::new(time::Duration::seconds(60), 30),
            &options.authorize,
        ),
        (
            "/oauth2/introspect",
            RateLimitRule::new(time::Duration::seconds(60), 100),
            &options.introspect,
        ),
        (
            "/oauth2/revoke",
        ...
        (
            "/oauth2/register",
            RateLimitRule::new(time::Duration::seconds(60), 5),
            &options.register,
        ),
        (
            "/oauth2/userinfo",
            RateLimitRule::new(time::Duration::seconds(60), 60),
            &options.userinfo,
        ),
    ]
    .into_iter()
