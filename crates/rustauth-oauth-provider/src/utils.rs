...
pub(crate) fn no_content() -> Result<ApiResponse, RustAuthError> {
    Response::builder()
...
pub(crate) fn empty_success_response() -> Result<ApiResponse, RustAuthError> {
    Response::builder()
        .status(StatusCode::OK)
        .body(Vec::new())
        .map_err(|error| RustAuthError::Api(error.to_string()))
...
pub(crate) fn error_response(error: OAuthProviderError) -> Result<ApiResponse, RustAuthError> {
    json_response(
        error.status,
        &OAuthErrorBody {
            error: &error.error,
...
pub(crate) fn redirect_response(uri: &str) -> Result<ApiResponse, RustAuthError> {
    Response::builder()
...
pub(crate) fn parse_body<T: DeserializeOwned>(request: &ApiRequest) -> Result<T, RustAuthError> {
...
pub(crate) async fn current_session(
    ...
    request: &ApiRequest,
) -> Result<Option<(Session, rustauth_core::db::User, Vec<Cookie>)>, RustAuthError> {
...
pub(crate) fn hmac_sha256_base64url(value: &str, secret: &str) -> Result<String, RustAuthError> {
...
pub(crate) fn hmac_sha256_base64(value: &str, secret: &str) -> Result<String, RustAuthError> {
    let mut mac = HmacSha256::new_from_slice(secret.as_bytes())
        .map_err(|error| RustAuthError::Crypto(error.to_string()))?;
    mac.update(value.as_bytes());
...
pub(crate) fn user_from_record(record: DbRecord) -> Result<rustauth_core::db::User, RustAuthError> {
...
fn required_string(record: &DbRecord, field: &str) -> Result<String, RustAuthError> {
    string(record, field).ok_or_else(|| missing_field(field))
}

fn missing_field(field: &str) -> RustAuthError {
    RustAuthError::Api(format!("missing required field `{field}`"))
}
