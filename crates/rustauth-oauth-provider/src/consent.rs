use rustauth_core::db::{DbAdapter, DbRecord, DbValue, Delete, FindOne, Update, Where};
...
pub async fn find_consent(
    ...
    user_id: &str,
    client_id: &str,
) -> Result<Option<OAuthConsent>, RustAuthError> {
...
pub async fn upsert_consent(
...
) -> Result<OAuthConsent, RustAuthError> {
    ...
    if let Some(existing) = existing {
        ...
        return adapter
            ...
            .map(consent_from_record)
            .transpose()?
            .ok_or_else(|| RustAuthError::Adapter("oauth consent disappeared".to_owned()));
    }
...
pub(crate) fn consent_from_record(record: DbRecord) -> Result<OAuthConsent, RustAuthError> {
    Ok(OAuthConsent {
...
fn required_string(record: &DbRecord, field: &str) -> Result<String, RustAuthError> {
    string(record, field)
        .ok_or_else(|| RustAuthError::Adapter(format!("oauth consent missing {field}")))
}
