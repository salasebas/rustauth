...
Use this crate when your RustAuth server should become the authorization server:
...
- Optional JWT/JWKS integration through `rustauth-plugins::jwt`.
...
```toml
[dependencies]
rustauth = { version = "0.2.0", features = ["oauth-provider", "plugins"] }
```
...
- `rustauth-oauth-provider`: provider-side OAuth/OIDC server behavior.
- `rustauth-oauth`: low-level OAuth client primitives and token helpers.
- `rustauth-oidc`: relying-party helpers for external IdPs.
- `rustauth-sso`: enterprise login plugin that consumes external OIDC/SAML IdPs.
