# Upstream parity: openauth-oauth-provider
...
| Rust crate | `crates/openauth-oauth-provider` |
...
| --- | ---: | ---: | --- |
| Total measured server/runtime tests | 96 | 261 | Rust count from `rg '#\[(test\|tokio::test)\]' crates/openauth-oauth-provider`; upstream count from `rg '^\s*(it\|test)\(' packages/oauth-provider/src`. |
...
| JWT integration | Discovers the Better Auth JWT plugin at runtime. | `oauth_provider_with_jwt()` wires metadata through `openauth-plugins::jwt`. | Keeps plugin composition explicit and typed. |
...
5. Compare upstream `*.test.ts` files with `crates/openauth-oauth-provider/tests/` and the maintainer matrix in `tests/upstream_mapping.md`.
6. Verify with `cargo nextest run -p openauth-oauth-provider`.
