...
pub async fn run_adapter_contract(adapter: &dyn DbAdapter) -> Result<(), RustAuthError> {
...
async fn contract_updates_and_deletes(
    ...
    let updated = adapter
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter("adapter contract did not update row".to_owned()))?;
...
async fn contract_transactions(adapter: &dyn DbAdapter, prefix: &str) -> Result<(), RustAuthError> {
...
async fn contract_joins(adapter: &dyn DbAdapter, prefix: &str) -> Result<(), RustAuthError> {
    let now = OffsetDateTime::now_utc();
    ...
    let user = adapter
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter("adapter contract join missed user".to_owned()))?;
