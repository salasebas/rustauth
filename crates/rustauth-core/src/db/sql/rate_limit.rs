...
pub fn rate_limit_count_from_i64(count: i64) -> Result<u64, RustAuthError> {
    u64::try_from(count).map_err(|_| {
        RustAuthError::Adapter("negative rate limit count persisted in database".to_owned())
    })
}

/// Encodes an in-memory rate-limit count for SQL `BIGINT` storage.
pub fn rate_limit_count_to_i64(count: u64) -> Result<i64, RustAuthError> {
    i64::try_from(count)
        .map_err(|_| RustAuthError::Adapter("rate limit count exceeds SQL BIGINT range".to_owned()))
}
...
) -> Result<(RateLimitDecision, RateLimitRecord, bool), RustAuthError> {
