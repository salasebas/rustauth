...
{
    pub async fn create(mut self, query: Create) -> Result<DbRecord, RustAuthError> {
    ...
    pub async fn find_one(mut self, query: FindOne) -> Result<Option<DbRecord>, RustAuthError> {
    ...
    pub async fn find_many(mut self, query: FindMany) -> Result<Vec<DbRecord>, RustAuthError> {
        if !query.joins.is_empty() {
    ...
    pub async fn count(mut self, query: Count) -> Result<u64, RustAuthError> {
        let statement = count_statement(self.dialect, self.schema, &query)?;
        let count = self.executor.fetch_scalar_i64(statement).await?;
        u64::try_from(count).map_err(|_| RustAuthError::NumericOutOfRange {
            context: "SQL count result",
        })
    }

    pub async fn update(mut self, query: Update) -> Result<Option<DbRecord>, RustAuthError> {
    ...
    pub async fn update_many(mut self, query: UpdateMany) -> Result<u64, RustAuthError> {
        if query.data.is_empty() {
            return Ok(0);
        }
    ...
    pub async fn delete(mut self, query: Delete) -> Result<(), RustAuthError> {
        let plan = delete_one_statement(self.dialect, self.schema, &query)?;
        self.executor.execute(plan.statement).await?;
        Ok(())
    }

    pub async fn delete_many(mut self, query: DeleteMany) -> Result<u64, RustAuthError> {
