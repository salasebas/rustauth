...
where
    F: for<'a> Fn(PasswordResetEmail, Option<&'a Request<Vec<u8>>>) -> OutboundSendFuture
        + Send
        + Sync
        + 'static,
{
...
where
    F: for<'a> Fn(PasswordResetPayload, Option<&'a Request<Vec<u8>>>) -> Result<(), RustAuthError>
        + Send
        + Sync
        + 'static,
{
...
pub struct PasswordOptions {
    ...
    pub reset_password_token_expires_in: Option<Duration>,
...
impl PasswordOptions {
    ...
    #[must_use]
    pub fn reset_password_token_expires_in(mut self, expires_in: Duration) -> Self {
        self.reset_password_token_expires_in = Some(expires_in);
        self
    }
