...
impl DeleteUserOptions {
    ...
    #[must_use]
    pub fn delete_token_expires_in(mut self, expires_in: Duration) -> Self {
        self.delete_token_expires_in = Some(expires_in);
        self
    }
}
...
where
    F: for<'a> Fn(&User, Option<&'a Request<Vec<u8>>>) -> Result<(), RustAuthError>
        + Send
        + Sync
        + 'static,
{
