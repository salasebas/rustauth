...
use crate::api::http_json::{snake_to_camel, user_to_http_value};
use crate::context::AuthContext;
use crate::db::{DbAdapter, DbFieldType, DbRecord, DbValue, FindOne, User};
...
/// Like [`insert_returned_fields`] but emits camelCase HTTP keys.
pub(crate) fn insert_returned_fields_http<F>(
    object: &mut serde_json::Map<String, Value>,
    fields: &BTreeMap<String, F>,
    record: &DbRecord,
) -> Result<(), RustAuthError>
    ...
    F: AdditionalField,
{
    for (name, field) in fields {
        ...
        let value = record
            ...
            .or_else(|| field.default_value())
            .unwrap_or(&DbValue::Null);
        object.insert(snake_to_camel(name), db_value_to_json(value)?);
    }
    Ok(())
}
...
pub fn db_value_to_json(value: &DbValue) -> Result<Value, RustAuthError> {
    match value {
        ...
        DbValue::Timestamp(value) => {
            serde_json::to_value(value).map_err(|error| RustAuthError::Serialization {
...
pub async fn user_response_value(
    ...
    if fields.is_empty() {
        return user_to_http_value(user);
    }
    ...
    if let Some(record) = record {
        insert_returned_fields_http(object, fields, &record)?;
    }
    Ok(value)
}

fn db_record_to_json(record: &DbRecord) -> Result<Value, RustAuthError> {
    record
        .iter()
