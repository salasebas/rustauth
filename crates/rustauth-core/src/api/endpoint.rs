...
pub type EndpointHandler = fn(&AuthContext, ApiRequest) -> Result<ApiResponse, RustAuthError>;
pub type EndpointFuture<'a> =
    Pin<Box<dyn Future<Output = Result<ApiResponse, RustAuthError>> + Send + 'a>>;
...
pub type EndpointMiddlewareFuture<'a> =
    Pin<Box<dyn Future<Output = Result<Option<ApiResponse>, RustAuthError>> + Send + 'a>>;
...
where
    ...
    Fut: Future<Output = Result<ApiResponse, RustAuthError>> + Send + 'static,
{
    move |context: &AuthContext, request: ApiRequest| {
        let handler = handler.clone();
        let context = context.clone();
        Box::pin(async move { handler(context, request).await })
    }
}
...
where
    ...
    Fut: Future<Output = Result<ApiResponse, RustAuthError>> + Send + 'static,
{
    create_auth_endpoint_raw(path, method, options, async_auth_handler(handler))
}
...
where
    ...
    for<'a> Fut: Future<Output = Result<Option<ApiResponse>, RustAuthError>> + Send + 'a,
{
    EndpointMiddleware::new(move |context: &AuthContext, request: &ApiRequest| {
        let handler = handler.clone();
        let context = context.clone();
        Box::pin(async move { handler(context, request).await })
    })
