...
pub(super) async fn persist_refreshed_tokens(
    ...
    users
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter("failed to update account".to_owned()))
}
...
pub(super) fn access_token_response_from_tokens(
    ...
    tokens: OAuth2Tokens,
    account: &Account,
) -> Result<AccessTokenResponse, RustAuthError> {
...
pub(super) fn account_cookie(
    context: &crate::context::AuthContext,
    account: &Account,
) -> Result<Vec<crate::cookies::Cookie>, RustAuthError> {
...
pub(super) fn tokens_from_account(
    context: &crate::context::AuthContext,
    account: &Account,
) -> Result<OAuth2Tokens, RustAuthError> {
    Ok(OAuth2Tokens {
...
#[cfg(feature = "oauth")]
pub(super) fn is_refresh_unsupported(error: &rustauth_oauth::oauth2::OAuthError) -> bool {
    error
        .to_string()
...
#[cfg(feature = "oauth")]
pub(super) fn provider_not_supported(provider_id: &str) -> Result<ApiResponse, RustAuthError> {
    error_response(
...
#[cfg(feature = "oauth")]
pub(super) fn account_not_found() -> Result<ApiResponse, RustAuthError> {
    error_response(
        StatusCode::BAD_REQUEST,
        "ACCOUNT_NOT_FOUND",
        "Account not found",
