...
use crate::api::services::user::{DeleteUserError, DeleteUserErrorOrRustAuth, DeleteUserResult};
use crate::api::{
...
pub(super) fn delete_user_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        ...
        move |context, request| async move {
            if !context.options.user.delete_user.enabled {
                return error_response(StatusCode::NOT_FOUND, error_codes::NOT_FOUND, "Not found");
            }
            let body: DeleteUserBody = parse_request_body(&request)?;
            if let Some(token) = body.token.as_deref() {
                return delete_user_by_token(&context, &request, token, None).await;
            }
...
pub(super) fn delete_user_callback_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        ...
        move |context, request| async move {
            ...
            delete_user_by_token(&context, &request, &token, callback_url.as_deref()).await
        },
    )
}
...
async fn delete_user_by_token(
    ...
    let Some((_, user, _)) = sensitive_session(context, request).await? else {
        return super::shared::unauthorized();
    };
    if let Err(error) =
        user_service::delete_user_with_token(context, Some(request), &user, token).await
    {
...
fn invalid_token() -> Result<crate::api::ApiResponse, crate::error::RustAuthError> {
    error_response(
...
fn delete_user_error_response(
    error: DeleteUserErrorOrRustAuth,
) -> Result<crate::api::ApiResponse, crate::error::RustAuthError> {
    match error {
        DeleteUserErrorOrRustAuth::RustAuth(error) => Err(error),
        DeleteUserErrorOrRustAuth::Service(error) => match error {
