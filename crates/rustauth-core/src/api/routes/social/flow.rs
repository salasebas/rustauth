...
use crate::user::{CreateOAuthAccountInput, UpdateAccountInput, UpdateUserInput};
...
pub(super) async fn sign_in_with_id_token(
    ...
    let user = user_response_value(context, &data.user).await?;
    json_response(
        StatusCode::OK,
        &SocialSessionBody {
            redirect: false,
...
fn link_status_response() -> Result<ApiResponse, RustAuthError> {
    json_response(
        StatusCode::OK,
        &LinkStatusBody {
            url: String::new(),
...
enum LinkOAuthAccountError {
    ...
    CreateFailed,
    Source(RustAuthError),
}

impl From<RustAuthError> for LinkOAuthAccountError {
...
pub(super) fn lookup_provider(
    context: &crate::context::AuthContext,
    provider_id: &str,
) -> Result<Arc<dyn SocialOAuthProvider>, RustAuthError> {
    context
        .social_provider(provider_id)
        .ok_or_else(|| RustAuthError::Api(format!("social provider `{provider_id}` was not found")))
}

fn normalize_user_info(info: &OAuth2UserInfo) -> Result<OAuthUserInfo, RustAuthError> {
    let email = info
        .email
        .clone()
