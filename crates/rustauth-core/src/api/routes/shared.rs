...
use crate::auth::email_password::AuthFlowError;
...
use crate::db::{DbRecord, DbValue, Session, User};
...
pub(super) fn record_new_session(session: &Session, user: &User) -> Result<(), RustAuthError> {
...
pub(super) fn auth_flow_error_response(error: AuthFlowError) -> Result<ApiResponse, RustAuthError> {
    json_response(error.http_status(), &error.to_api_response(), Vec::new())
}
...
where
    T: Serialize,
{
    let body = serde_json::to_vec(body).map_err(|error| RustAuthError::Serialization {
...
pub(super) async fn current_session(
    ...
    current_session_with_cache_policy(context, request, false).await
}
...
pub(super) async fn sensitive_session(
    ...
    current_session_with_cache_policy(context, request, true).await
}
...
async fn current_session_with_cache_policy(
    ...
    let Some(result) = SessionAuth::new(context)?.get_session(input).await? else {
        return Ok(None);
    };
