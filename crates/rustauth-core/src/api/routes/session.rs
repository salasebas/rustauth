...
use crate::api::http_json::session_to_http_value;
...
use crate::api::services::session::{
    CurrentSessionInput, UpdateSessionError, UpdateSessionErrorOrRustAuth,
};
use crate::api::{
...
pub(super) fn get_session_endpoint(method: Method) -> AsyncAuthEndpoint {
    create_auth_endpoint(
        ...
        move |context, request| async move {
            ...
            .await?
                ...
                return json_response(StatusCode::OK, &Option::<SessionUserBody>::None, Vec::new());
            };
            ...
            json_response(
                StatusCode::OK,
                &SessionUserBody {
                    session: session_output_value(context.adapter_ref()?, &context, &session)
                        .await?,
                    user: user_response_value(&context, &user).await?,
...
pub(super) fn list_sessions_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        "/list-sessions",
        Method::GET,
        AuthEndpointOptions::new()
        ...
        move |context, request| async move {
            ...
            let sessions = session_service::list_sessions(&context, &user).await?;
            let http_sessions: Vec<Value> = sessions
                .iter()
                .map(session_to_http_value)
                .collect::<Result<Vec<_>, _>>()?;
            json_response(StatusCode::OK, &http_sessions, cookies)
        },
    )
}

pub(super) fn revoke_session_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        "/revoke-session",
        Method::POST,
        AuthEndpointOptions::new()
        ...
        move |context, request| async move {
            ...
            session_service::revoke_session(&context, &user, &body.token).await?;
            json_response(StatusCode::OK, &StatusBody { status: true }, cookies)
        },
    )
}

pub(super) fn revoke_sessions_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        "/revoke-sessions",
        Method::POST,
        ...
        move |context, request| async move {
            ...
            session_service::revoke_sessions(&context, &user).await?;
            json_response(StatusCode::OK, &StatusBody { status: true }, cookies)
        },
    )
}

pub(super) fn revoke_other_sessions_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        ...
        move |context, request| async move {
            ...
            session_service::revoke_other_sessions(&context, &current_session, &user).await?;
            json_response(StatusCode::OK, &StatusBody { status: true }, cookies)
        },
    )
}

pub(super) fn update_session_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        "/update-session",
        Method::POST,
        AuthEndpointOptions::new()
        ...
        move |context, request| async move {
            let Some((current, _user, cookies)) = sensitive_session(&context, &request).await?
            else {
                return unauthorized();
            };
            let body: Value = parse_request_body(&request)?;
            let record = match session_service::update_session(&context, &current, body).await {
                Ok(record) => record,
...
fn update_session_error_response(
    error: UpdateSessionErrorOrRustAuth,
) -> Result<crate::api::ApiResponse, RustAuthError> {
    match error {
        UpdateSessionErrorOrRustAuth::RustAuth(error) => Err(error),
        UpdateSessionErrorOrRustAuth::Service(error) => match error {
