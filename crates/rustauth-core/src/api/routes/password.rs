...
use crate::api::services::password::{
    ChangePasswordInput, PasswordServiceError, PasswordServiceErrorOrRustAuth,
...
pub(super) fn change_password_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        ...
        move |context, request| async move {
                ...
                let user = super::shared::user_response_value(&context, &user).await?;
                json_response(StatusCode::OK, &TokenUserResponse { token, user }, cookies)
        },
    )
}

pub(super) fn set_password_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        "/set-password",
        Method::POST,
        AuthEndpointOptions::new()
...
pub(super) fn verify_password_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        "/verify-password",
        Method::POST,
        AuthEndpointOptions::new()
...
pub(super) fn request_password_reset_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
...
pub(super) fn reset_password_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        "/reset-password",
        Method::POST,
        AuthEndpointOptions::new()
...
pub(super) fn reset_password_callback_endpoint() -> AsyncAuthEndpoint {
    create_auth_endpoint(
        ...
        move |context, request| async move {
            ...
            if password_service::reset_password_callback_token_is_valid(&context, token).await? {
...
fn password_service_error_response(
    error: PasswordServiceErrorOrRustAuth,
) -> Result<crate::api::ApiResponse, RustAuthError> {
    match error {
        PasswordServiceErrorOrRustAuth::RustAuth(error) => Err(error),
        PasswordServiceErrorOrRustAuth::Service(error) => match error {
