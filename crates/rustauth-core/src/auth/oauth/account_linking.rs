...
// `rustauth-core` is built without the route layer (for example as a
// dependency of `rustauth-sso`).
#[cfg(feature = "oauth")]
...
pub async fn handle_oauth_user_info(
...
) -> Result<HandleOAuthUserInfoResult, RustAuthError> {
    let users = DbUserStore::with_schema(adapter, context.db_schema.clone());
    ...
    if let Some(lookup) = db_user {
        ...
        if let Some(linked_account) = linked_account {
            ...
            let linked_account = users
                ...
                .await
                .map_err(|_| RustAuthError::Adapter("unable to link OAuth account".to_owned()))?;
    ...
    let session = match created_session {
        ...
        None => SessionStore::with_storage_and_schema(
            adapter,
        ...
        .create_session(CreateSessionInput::new(
            &user.id,
            OffsetDateTime::now_utc() + context.session_config.expires_in,
        ))
        .await
        .map_err(|_| RustAuthError::Adapter("unable to create OAuth session".to_owned()))?,
    };
...
) -> Result<CreatedOAuthSessionUser, RustAuthError> {
        ...
        .transaction(Box::new(move |transaction| {
            ...
            Box::pin(async move {
                ...
                let created = users
                    ...
                    .await
                    .map_err(|_| {
                        RustAuthError::Adapter("unable to create OAuth user".to_owned())
                    })?;
...
fn take_created_oauth_session_user(
    result: &Mutex<Option<CreatedOAuthSessionUser>>,
) -> Result<Option<CreatedOAuthSessionUser>, RustAuthError> {
    result
        .lock()
...
#[cfg(not(feature = "jose"))]
fn encode_account_cookie_data(_account: &Account) -> Result<String, RustAuthError> {
    Err(RustAuthError::FeatureDisabled { feature: "jose" })
}
...
fn account_input(
    ...
    account: &OAuthAccountInput,
    user_id: &str,
) -> Result<CreateOAuthAccountInput, RustAuthError> {
...
fn account_update_input(
    context: &AuthContext,
    account: &OAuthAccountInput,
) -> Result<UpdateAccountInput, RustAuthError> {
