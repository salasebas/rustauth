...
impl<'a> SessionAuth<'a> {
    ...
    pub async fn get_session(
        ...
        let session_store = SessionStore::new(self.context)?;
        ...
        let user_store = DbUserStore::from_context(self.context)?;
        ...
        if needs_refresh && !input.defer_refresh {
            let refreshed_expires_at = OffsetDateTime::now_utc()
                + Duration::seconds(self.context.session_config.expires_in.whole_seconds());
    ...
    ) -> Result<SignOutResult, RustAuthError> {
