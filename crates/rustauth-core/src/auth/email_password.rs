...
pub type PasswordHashFn = fn(&str) -> Result<String, RustAuthError>;
pub type PasswordVerifyFn = fn(&str, &str) -> Result<bool, RustAuthError>;
...
impl ErrorCode for AuthFlowErrorCode {
    fn as_str(&self) -> &str {
        (*self).as_str()
    }
...
impl AuthFlowError {
    ...
    pub fn storage(error: RustAuthError) -> Self {
        Self {
    ...
    pub async fn sign_up(
            ...
            .transaction(Box::new(move |transaction| {
                Box::pin(async move {
                    ...
                    match outcome {
                        ...
                        Err(error) => {
                            let transaction_error = RustAuthError::Adapter(error.to_string());
        ...
        match transaction_status {
            Ok(()) => match take_sign_up_result(&result)? {
                ...
                None => Err(AuthFlowError::storage(RustAuthError::Adapter(
...
fn take_sign_up_result(
    ...
    result
        .lock()
        .map_err(|_| {
            AuthFlowError::storage(RustAuthError::LockPoisoned {
                context: "sign-up result",
            })
        })
...
mod tests {
    ...
    #[test]
    fn auth_flow_error_code_implements_error_code_trait() {
        assert_error_code(
            AuthFlowErrorCode::InvalidEmail,
            "INVALID_EMAIL",
            "Invalid email",
    ...
    #[test]
    fn auth_flow_error_to_api_response_preserves_code_and_message() {
        let error = AuthFlowError::new(AuthFlowErrorCode::InvalidEmailOrPassword);
        let response = error.to_api_response();
        assert_eq!(response.code, "INVALID_EMAIL_OR_PASSWORD");
        assert_eq!(response.message, "Invalid email or password");
        assert_eq!(response.original_message, None);
    }

    #[test]
    fn auth_flow_error_http_status_mapping_matches_route_helpers() {
        let cases = [
            (
                AuthFlowErrorCode::InvalidEmailOrPassword,
            ...
            (AuthFlowErrorCode::EmailNotVerified, StatusCode::FORBIDDEN),
            (
                AuthFlowErrorCode::FailedToCreateSession,
                StatusCode::INTERNAL_SERVER_ERROR,
            ),
            (
                AuthFlowErrorCode::UserAlreadyExists,
        ...
        for (code, expected_status) in cases {
            let error = AuthFlowError::new(code);
