...
impl GovernorMemoryRateLimitStore {
    ...
    fn cleanup_if_due(&self, now_ms: i64) -> Result<(), RustAuthError> {
            ...
            let decision = match existing {
                ...
                Some(record) if input.now_ms.saturating_sub(record.last_request) <= window_ms => {
                    ...
                    self.storage.set(
                        ...
                        input.rule.window.whole_seconds() as u64,
                        true,
                    )?;
...
pub fn hash_rate_limit_scope(secret: &str, scope: &str) -> Result<String, RustAuthError> {
    let mut mac = Hmac::<Sha256>::new_from_slice(secret.as_bytes()).map_err(|_| {
        RustAuthError::InvalidConfig("secret is invalid for rate limit scope HMAC".to_owned())
    })?;
    ...
    let key = match resolve_rate_limit_key_plan(context, request, path, Some(&scope_suffix))? {
        ...
        RateLimitKeyPlan::Deny => {
            return Ok(Some(RateLimitRejection {
                retry_after: rule.window.whole_seconds() as u64,
            }));
        }
...
pub fn on_request_rate_limit(
    ...
    match resolve_config(context, request)? {
        ...
        RateLimitPlan::Consume(_) => Err(RustAuthError::Api(
...
fn resolve_config(
    context: &AuthContext,
    request: &Request<Body>,
) -> Result<RateLimitPlan, RustAuthError> {
...
) -> Result<RateLimitKeyPlan, RustAuthError> {
...
fn store(context: &AuthContext) -> Result<Arc<dyn RateLimitStore>, RustAuthError> {
    ...
    match context.rate_limit.storage {
        ...
        RateLimitStorageOption::Database => Err(RustAuthError::InvalidConfig(
            "database rate limit storage requires a concrete RateLimitStore".to_owned(),
        )),
        RateLimitStorageOption::SecondaryStorage => Err(RustAuthError::InvalidConfig(
...
fn denied_decision(input: &RateLimitConsumeInput, last_request: i64) -> RateLimitDecision {
    let window_ms = i64::try_from(input.rule.window.whole_milliseconds()).unwrap_or(i64::MAX);
