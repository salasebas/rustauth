...
use crate::db::{
    ...
    DeleteMany, FindMany, FindOne, SchemaTable, Session, Update, UpdateMany, Where, WhereOperator,
};
...
impl<'a> DbSessionStore<'a> {
    ...
    fn sessions(&self) -> Result<SchemaTable<'_>, RustAuthError> {
        SchemaTable::new(&self.schema, SESSION_MODEL)
    }

    fn parse_session(&self, record: DbRecord) -> Result<Session, RustAuthError> {
        session_from_record(self.sessions()?.map_record(record)?)
    }
    ...
    pub async fn find_session_including_expired(
        &self,
        token: &str,
    ) -> Result<Option<Session>, RustAuthError> {
        self.adapter
            .find_one(
            ...
            .await?
            .map(|record| self.parse_session(record))
            .transpose()
    }
    ...
    pub async fn update_session_expiry(
        ...
        record.map(|record| self.parse_session(record)).transpose()
    }
...
impl<'a> SessionStore<'a> {
    pub fn new(context: &'a AuthContext) -> Result<Self, RustAuthError> {
        Ok(Self {
            database: DbSessionStore::from_context(context)?,
    ...
    pub fn with_storage_and_schema(
        adapter: &'a dyn DbAdapter,
        schema: DbSchema,
        secondary_storage: Option<Arc<dyn SecondaryStorage>>,
        store_session_in_database: bool,
        ...
        Self {
            database: DbSessionStore::with_schema(adapter, schema),
            secondary_storage,
            store_session_in_database,
            preserve_session_in_database,
        }
...
fn serialize_session(session: &Session) -> Result<String, RustAuthError> {
    serde_json::to_string(session).map_err(|error| RustAuthError::Serialization {
        context: "serializing session",
        message: error.to_string(),
    })
}

fn deserialize_session(value: &str) -> Result<Session, RustAuthError> {
...
fn serialize_user_session_tokens(tokens: &[String]) -> Result<String, RustAuthError> {
    serde_json::to_string(tokens).map_err(|error| RustAuthError::Serialization {
        context: "serializing user session index",
        message: error.to_string(),
    })
}

fn deserialize_user_session_tokens(value: &str) -> Result<Vec<String>, RustAuthError> {
    serde_json::from_str(value).map_err(|error| RustAuthError::Serialization {
...
fn session_from_record(record: DbRecord) -> Result<Session, RustAuthError> {
    Ok(Session {
