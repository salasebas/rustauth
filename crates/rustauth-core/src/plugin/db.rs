...
impl PluginDatabaseHook {
    ...
    where
        F: Fn(&PluginDatabaseHookContext<'_>, &Create, &DbRecord) -> Result<(), RustAuthError>
            + Send
            + Sync
            + 'static,
    {
    ...
    where
        F: Fn(&PluginDatabaseHookContext<'_>, &UpdateMany, u64) -> Result<(), RustAuthError>
            + Send
            + Sync
            + 'static,
    {
    ...
    where
        F: Fn(&PluginDatabaseHookContext<'_>, &Delete, &[DbRecord]) -> Result<(), RustAuthError>
            + Send
            + Sync
            + 'static,
    {
