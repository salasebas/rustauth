...
use rustauth_core::session::{CreateSessionInput, SessionStore};
use rustauth_core::test_utils::MemorySecondaryStorage;
...
#[tokio::test]
async fn secondary_session_store_user_index_receives_ttl() -> Result<(), RustAuthError> {
...
#[tokio::test]
async fn secondary_session_store_user_index_ttl_tracks_latest_expiry() -> Result<(), RustAuthError>
{
    let adapter = MemoryAdapter::new();
    let storage = Arc::new(MemorySecondaryStorage::default());
    let store = secondary_store(&adapter, storage.clone());
    ...
    let index_ttl = storage
        ...
        .ok_or_else(|| RustAuthError::Adapter("missing user session index TTL".to_owned()))?;
    assert!(index_ttl > 2 * 60 * 60);
...
#[tokio::test]
async fn secondary_session_store_expired_user_index_is_removed() -> Result<(), RustAuthError> {
    let adapter = MemoryAdapter::new();
    let storage = Arc::new(MemorySecondaryStorage::default());
    let store = secondary_store(&adapter, storage.clone());
...
#[tokio::test]
async fn secondary_session_store_list_sessions_after_ttl_cleanup() -> Result<(), RustAuthError> {
    let adapter = MemoryAdapter::new();
    let storage = Arc::new(MemorySecondaryStorage::default());
    let store = secondary_store(&adapter, storage.clone());
...
async fn secondary_session_store_update_expiry_refreshes_user_index_ttl(
    ...
    let initial_ttl = storage
        .ttl_for_key(&user_index_key("user_1"))?
        .ok_or_else(|| RustAuthError::Adapter("missing initial index TTL".to_owned()))?;
    ...
    let refreshed_ttl = storage
        .ttl_for_key(&user_index_key("user_1"))?
        .ok_or_else(|| RustAuthError::Adapter("missing refreshed index TTL".to_owned()))?;
...
async fn secondary_session_store_refresh_user_sessions_bumps_updated_at(
    ...
    let initial_ttl = storage
        ...
        .ok_or_else(|| RustAuthError::Adapter("missing initial session TTL".to_owned()))?;

    let refreshed = store.refresh_user_sessions("user_1").await?;
    let session = store
        .find_session("token_1")
        .await?
        .ok_or_else(|| RustAuthError::Adapter("missing refreshed session".to_owned()))?;
    let refreshed_ttl = storage
        .ttl_for_key("session:token_1")?
        .ok_or_else(|| RustAuthError::Adapter("missing refreshed session TTL".to_owned()))?;
