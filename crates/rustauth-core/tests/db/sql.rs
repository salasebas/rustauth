...
use rustauth_core::db::validate_rate_limit_rule;
...
use rustauth_core::options::{RateLimitConsumeInput, RateLimitRecord, RateLimitRule};

#[test]
fn sql_module_reexports_public_adapter_api_after_internal_split() {
    let _statement = rustauth_core::db::sql::SqlStatement::new("SELECT 1");
    let _fragment = rustauth_core::db::sql::SqlFragment::new("WHERE 1 = 1");
    let _dialect = rustauth_core::db::sql::SqlDialect::Postgres;
    let _names = rustauth_core::db::sql::SqlRateLimitNames::new("rate_limits");
}

#[test]
fn sql_dialect_where_clause_uses_postgres_placeholders_and_params() -> Result<(), RustAuthError> {
...
#[test]
fn sql_dialect_compiles_empty_in_predicates_as_boolean_constants() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
    let table = schema
        .table("user")
        .ok_or_else(|| RustAuthError::TableNotFound {
...
#[test]
fn sql_dialect_groups_and_or_predicates_deterministically() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
    let table = schema
        .table("user")
        .ok_or_else(|| RustAuthError::TableNotFound {
...
#[test]
fn sql_dialect_escapes_like_wildcards_in_string_pattern_predicates() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
    let table = schema
        .table("user")
        .ok_or_else(|| RustAuthError::TableNotFound {
    ...
    let fragment = SqlDialect::Postgres.where_clause(
        ...
        &[Where {
            ...
            connector: rustauth_core::db::Connector::And,
...
#[test]
fn sql_dialect_supports_database_generated_id_column_definitions() -> Result<(), RustAuthError> {
...
#[test]
fn create_statement_returns_generated_ids_for_returning_dialects() -> Result<(), RustAuthError> {
...
#[test]
fn plan_schema_migration_creates_missing_tables_in_schema_order() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
...
fn ensure_executable_migration_plan_rejects_warning_plans_and_accepts_clean_plans(
    ...
    assert!(matches!(
        ...
        Err(RustAuthError::Adapter(message)) if message.contains("non-executable migration warnings")
    ));
...
#[test]
fn plan_schema_migration_repairs_missing_unique_indexes() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
    let snapshot = SqlSchemaSnapshot::default()
        .with_table("users")
        .with_column("users", SqlColumnSnapshot::new("id", "text"))
...
#[test]
fn plan_schema_migration_warns_for_constraint_and_id_mismatches() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions {
        id_policy: IdPolicy::new(IdGeneration::Serial),
        ..AuthSchemaOptions::default()
    });
...
#[test]
fn plan_schema_migration_warns_for_foreign_key_mismatch() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
    let snapshot = SqlSchemaSnapshot::default()
        .with_table("users")
        .with_column("users", SqlColumnSnapshot::new("id", "text"))
...
#[test]
fn joined_rows_groups_base_records_and_applies_join_limits() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
...
#[test]
fn create_statement_builds_insert_sql_and_params() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
...
#[test]
fn find_many_with_joins_statement_compiles_account_and_session_joins() -> Result<(), RustAuthError>
{
    let schema = auth_schema(AuthSchemaOptions::default());
...
#[test]
fn find_many_statement_builds_select_sort_limit_offset() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
...
#[test]
fn count_statement_builds_count_sql() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());

    let statement = count_statement(
        SqlDialect::Sqlite,
        &schema,
        &rustauth_core::db::Count::new("session")
...
#[test]
fn update_one_plan_uses_returning_for_postgres_and_preselect_for_mysql() -> Result<(), RustAuthError>
{
    let schema = auth_schema(AuthSchemaOptions::default());
...
#[test]
fn delete_one_statement_uses_dialect_specific_single_row_strategy() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
...
#[test]
fn rate_limit_consume_statements_are_dialect_specific() -> Result<(), RustAuthError> {
...
fn validate_rate_limit_rule_rejects_zero_window_max_and_oversized_windows(
    ...
    assert!(matches!(
        validate_rate_limit_rule(&RateLimitRule { window: time::Duration::seconds(0), max: 1 }),
        Err(RustAuthError::InvalidConfig(message))
            if message == "rate limit window must be greater than zero"
    ));
    assert!(matches!(
        validate_rate_limit_rule(&RateLimitRule { window: time::Duration::seconds(1), max: 0 }),
        Err(RustAuthError::InvalidConfig(message))
...
#[tokio::test]
async fn sql_adapter_runner_reads_rows_without_sqlx() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
...
fn fake_row_value(
    row: &FakeJoinedRow,
    field: &rustauth_core::db::DbField,
    alias: &str,
) -> Result<DbValue, RustAuthError> {
    match alias {
