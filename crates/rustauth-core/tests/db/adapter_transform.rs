...
#[test]
fn transform_create_query_maps_logical_names_to_physical_names() -> Result<(), RustAuthError> {
...
#[test]
fn transform_find_many_query_maps_where_sort_and_select_fields() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions {
    ...
    let query = FindMany::new("session")
        ...
        .sort_by(rustauth_core::db::Sort::new(
            "expires_at",
            rustauth_core::db::SortDirection::Asc,
        ))
        .select(["id", "user_id"]);
...
#[test]
fn transform_update_query_maps_update_data_and_where_fields() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions {
...
#[test]
fn transform_count_and_delete_many_queries_map_where_fields() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions {
...
#[test]
fn transform_find_many_query_coerces_number_strings_in_where_clauses() -> Result<(), RustAuthError>
{
    let schema = auth_schema(AuthSchemaOptions {
...
#[test]
fn transform_create_query_keeps_json_native_when_adapter_supports_json() -> Result<(), RustAuthError>
{
    let schema = auth_schema(AuthSchemaOptions {
        user: TableOptions::default()
...
#[test]
fn resolve_join_options_maps_forward_foreign_key() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
...
#[test]
fn resolve_join_options_maps_reverse_foreign_key_as_one_to_one() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions::default());
...
#[test]
fn resolve_join_options_adds_required_select_field() -> Result<(), RustAuthError> {
    let schema = auth_schema(AuthSchemaOptions {
...
fn resolve_join_options_returns_typed_error_without_foreign_key() {
    ...
    assert_eq!(
        ...
        Err(RustAuthError::JoinForeignKeyNotFound {
            base_model: "user".to_owned(),
