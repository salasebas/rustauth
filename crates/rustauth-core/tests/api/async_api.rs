...
use rustauth_core::api::{response, ApiRequest, AsyncAuthEndpoint, AuthEndpoint, AuthRouter};
use rustauth_core::context::{create_auth_context, AuthContext};
...
fn sync_handle_rejects_async_only_endpoint() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        error,
        RustAuthError::Api("async endpoint requires AuthRouter::handle_async".to_owned())
    );
    Ok(())
}
