...
async fn refresh_token_sets_account_cookie_when_enabled() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    assert!(cookies
        .iter()
        .any(|value| value.starts_with("rustauth.account_data=")));
    Ok(())
}
...
fn token_router(adapter: Arc<RouteAdapter>) -> Result<AuthRouter, RustAuthError> {
    router_with_options(
        adapter,
        RustAuthOptions {
            social_providers: vec![Arc::new(TokenProvider)],
...
fn encrypting_token_router(adapter: Arc<RouteAdapter>) -> Result<AuthRouter, RustAuthError> {
    router_with_options(
        adapter,
        RustAuthOptions {
            account: rustauth_core::options::AccountOptions {
...
fn encryption_context() -> Result<rustauth_core::context::AuthContext, RustAuthError> {
    create_auth_context(super::with_test_defaults(RustAuthOptions {
...
fn encrypt_token(value: &str) -> Result<String, RustAuthError> {
    let context = encryption_context()?;
    rustauth_core::auth::oauth::set_token_util(Some(value), &context)?
        .ok_or_else(|| RustAuthError::Adapter("missing encrypted token".to_owned()))
}

fn decrypt_token(value: &str) -> Result<String, RustAuthError> {
    let context = encryption_context()?;
    rustauth_core::auth::oauth::decrypt_oauth_token(value, &context)
}
