...
use rustauth_core::options::{
    EmailPasswordOptions, EmailVerificationOptions, ExistingUserSignUpPayload, UserAdditionalField,
...
async fn sign_up_email_route_rejects_by_default_without_explicit_opt_in(
    ...
    let adapter = Arc::new(RouteAdapter::default());
    let router = router_with_bare_options(adapter.clone(), RustAuthOptions::default())?;
...
async fn sign_up_email_route_allows_sign_in_when_enabled_but_sign_up_disabled(
    ...
    use rustauth_core::crypto::password::hash_password;
    ...
    let router = router_with_bare_options(
        adapter.clone(),
        RustAuthOptions::default().email_password(
...
async fn sign_up_email_route_can_skip_auto_sign_in() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(set_cookie_values(&response)
        .iter()
        .all(|cookie| !cookie.starts_with("rustauth.session_token=")));
    Ok(())
}
...
async fn sign_up_email_route_uses_secondary_storage_for_sessions(
    ...
    let router = router_with_options(
        adapter.clone(),
        RustAuthOptions::default().secondary_storage(storage.clone()),
    )?;
        ...
        EmailPasswordOptions::new()
            ...
            .on_existing_user_sign_up(
                move |payload: ExistingUserSignUpPayload,
                    ...
                    seen_for_hook
                        .lock()
                        .map_err(|_| RustAuthError::Api("hook sink poisoned".to_owned()))?
...
async fn sign_up_email_route_accepts_username_fields() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(body["user"]["displayUsername"], "Ada Lovelace");
