...
use rustauth_core::api::{
    async_endpoint_middleware, create_auth_endpoint, empty_openapi_response,
    fresh_session_middleware, path_param, query_param, redirect_openapi_response, response,
    AuthEndpointOptions, AuthRouter, BodyField, BodySchema, EndpointMiddleware, JsonSchemaType,
    OpenApiOperation,
};
use rustauth_core::context::create_auth_context;
use rustauth_core::context::request_state::set_current_session;
use rustauth_core::db::{Session, User};
use rustauth_core::error::RustAuthError;
use rustauth_core::options::{RustAuthOptions, SessionOptions};
...
async fn create_auth_endpoint_exposes_metadata_and_openapi(
    ...
    let endpoint = create_auth_endpoint(
        ...
        |_context, _request| async move { response(StatusCode::OK, br#"{"ok":true}"#.to_vec()) },
    );
    ...
    assert_eq!(openapi["info"]["title"], "RustAuth");
    assert_eq!(
...
async fn openapi_generation_matches_upstream_route_shape() -> Result<(), Box<dyn std::error::Error>>
{
    let reset_endpoint = create_auth_endpoint(
        ...
        |_context, _request| async move { response(StatusCode::OK, Vec::new()) },
    );
...
async fn openapi_contract_supports_summary_helpers_and_hidden_endpoints(
    ...
    let visible = create_auth_endpoint(
        ...
        |_context, _request| async move { response(StatusCode::FOUND, Vec::new()) },
    );
...
async fn create_auth_endpoint_validates_body_schema_before_handler(
    ...
    let endpoint = create_auth_endpoint(
        ...
        move |_context, _request| {
            ...
            async move { response(StatusCode::OK, Vec::new()) }
        },
    );
    let router = router(vec![endpoint])?;
...
async fn create_auth_endpoint_runs_endpoint_middleware_before_handler(
    ...
    let endpoint = create_auth_endpoint(
        ...
        |_context, _request| async move { response(StatusCode::OK, b"handler".to_vec()) },
    );
    let router = router(vec![endpoint])?;
...
async fn fresh_session_middleware_rejects_stale_session_before_handler(
    ...
    let endpoint = create_auth_endpoint(
        ...
        move |_context, _request| {
            called_in_handler.store(true, Ordering::SeqCst);
            async move { response(StatusCode::OK, b"handler".to_vec()) }
        },
    );
    let context = create_auth_context(with_test_defaults(RustAuthOptions {
        secret: Some("secret-a-at-least-32-chars-long!!".to_owned()),
        session: SessionOptions {
            fresh_age: Some(time::Duration::seconds(60)),
...
#[tokio::test]
async fn create_auth_endpoint_runs_without_box_pin() -> Result<(), Box<dyn std::error::Error>> {
    let endpoint = create_auth_endpoint(
        "/auth-endpoint-helper",
        Method::GET,
        AuthEndpointOptions::new(),
        |_context, _request| async move { response(StatusCode::OK, br#"{"helper":true}"#.to_vec()) },
    );
    let router = router(vec![endpoint])?;
    let response = router
        .handle_async(
            Request::builder()
                .method(Method::GET)
                .uri("/auth-endpoint-helper")
                .body(Vec::new())?,
        )
        .await?;
    assert_eq!(response.status(), StatusCode::OK);
    assert_eq!(response.body(), br#"{"helper":true}"#);
    Ok(())
}
...
fn router(
    endpoints: Vec<rustauth_core::api::AsyncAuthEndpoint>,
