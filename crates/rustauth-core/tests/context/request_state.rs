...
#[tokio::test]
async fn request_state_is_isolated_between_concurrent_scopes() -> Result<(), RustAuthError> {
...
#[tokio::test]
async fn request_state_lazily_initializes_once_per_scope() -> Result<(), RustAuthError> {
    let state = define_request_state(|| Marker { id: "initial" });
...
async fn request_state_returns_error_outside_scope() {
    ...
    assert_eq!(state.get(), Err(RustAuthError::RequestStateMissing));
}
...
#[tokio::test]
async fn should_skip_session_refresh_can_be_set_inside_scope() -> Result<(), RustAuthError> {
    run_with_request_state(async {
        assert!(!should_skip_session_refresh());
...
#[tokio::test]
async fn current_new_session_defaults_to_none_inside_scope() -> Result<(), RustAuthError> {
    run_with_request_state(async {
...
#[tokio::test]
async fn current_new_session_can_be_set_inside_scope() -> Result<(), RustAuthError> {
    let (session, user) = test_session_user();

    run_with_request_state(async {
        set_current_new_session(session.clone(), user.clone())?;
        let current = current_new_session()?.ok_or(RustAuthError::RequestStateMissing)?;
...
#[tokio::test]
async fn current_session_defaults_to_none_inside_scope() -> Result<(), RustAuthError> {
    run_with_request_state(async {
...
#[tokio::test]
async fn current_session_can_be_set_inside_scope() -> Result<(), RustAuthError> {
    let (session, user) = test_session_user();

    run_with_request_state(async {
        set_current_session(session.clone(), user.clone())?;
        let current = current_session()?.ok_or(RustAuthError::RequestStateMissing)?;
        assert_eq!(current.session, session);
        assert_eq!(current.user, user);
        Ok(())
    })
...
#[tokio::test]
async fn current_session_is_isolated_between_concurrent_scopes() -> Result<(), RustAuthError> {
    ...
    let first = run_with_request_state(async {
        ...
        tokio::time::sleep(std::time::Duration::from_millis(10)).await;
        Ok::<_, RustAuthError>(current_session()?.map(|current| current.session.id))
    });
