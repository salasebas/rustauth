...
fn get_cookies_uses_default_names_and_attributes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(cookies.session_token.name, "rustauth.session_token");
...
fn get_cookies_uses_secure_prefix_when_https_base_url() -> Result<(), Box<dyn std::error::Error>> {
    let cookies = get_cookies(&RustAuthOptions {
...
fn strip_secure_cookie_prefix_removes_secure_prefix() {
    assert_eq!(
        strip_secure_cookie_prefix("__Secure-rustauth.session_token"),
        "rustauth.session_token"
    );
}

#[test]
fn get_session_cookie_reads_default_and_secure_cookie_names() {
    let plain = get_session_cookie("rustauth.session_token=plain", None, None, false);
    let secure = get_session_cookie("__Secure-rustauth.session_token=secure", None, None, true);
...
#[test]
fn get_session_cookie_supports_explicit_cookie_prefix() {
    let token = get_session_cookie(
        "legacy-prefix.session_token=legacy",
        Some("legacy-prefix"),
        None,
        false,
    );
...
fn get_session_cookie_prefers_secure_name_when_both_present() {
    let header = "rustauth.session_token=attacker; __Secure-rustauth.session_token=victim";
...
fn get_session_cookie_ignores_unprefixed_name_in_secure_mode() {
    let resolved = get_session_cookie("rustauth.session_token=attacker", None, None, true);
...
fn get_session_cookie_ignores_secure_name_in_plain_mode() {
    let resolved = get_session_cookie("__Secure-rustauth.session_token=secure", None, None, false);
...
fn delete_session_cookie_expires_unprefixed_fallback_in_secure_mode(
    ...
    let expired = delete_session_cookie(&cookies, "rustauth.session_token=attacker", false);
...
fn delete_session_cookie_skips_unprefixed_fallback_in_plain_mode(
    ...
    let expired = delete_session_cookie(&cookies, "rustauth.session_token=token", false);

    assert_eq!(
        expired
            .iter()
            .filter(|cookie| cookie.name == "rustauth.session_token")
            .count(),
        1
    );
    Ok(())
