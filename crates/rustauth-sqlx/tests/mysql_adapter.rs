...
async fn test_pool(max_connections: u32) -> Result<sqlx::MySqlPool, RustAuthError> {
...
async fn adapter() -> Result<MySqlAdapter, RustAuthError> {
...
async fn mysql_adapter_reports_public_capabilities() -> Result<(), RustAuthError> {
    ...
    assert_eq!(
        capabilities,
        AdapterCapabilities::new("sqlx-mysql")
            .named("SQLx MySQL")
            .with_json()
            .with_arrays()
            .with_native_joins()
...
fn router(adapter: Arc<MySqlAdapter>) -> Result<AuthRouter, RustAuthError> {
