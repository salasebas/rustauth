...
async fn adapter() -> Result<SqliteAdapter, RustAuthError> {
...
#[tokio::test]
async fn sqlite_adapter_migration_plan_rolls_back_on_statement_failure() -> Result<(), RustAuthError>
{
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
...
#[tokio::test]
async fn sqlite_adapter_satisfies_shared_adapter_contract() -> Result<(), RustAuthError> {
...
#[tokio::test]
async fn sqlite_adapter_filters_sorts_limits_and_counts_records() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn sqlite_adapter_treats_like_wildcards_as_literal_user_input() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn sqlite_adapter_updates_and_deletes_matching_records() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn sqlite_adapter_reports_public_capabilities() -> Result<(), RustAuthError> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
#[tokio::test]
async fn sqlite_adapter_create_schema_writes_requested_schema_file() -> Result<(), RustAuthError> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
    ...
    let now = SystemTime::now()
        ...
        .map_err(|error| RustAuthError::Adapter(format!("clock error: {error}")))?;
    let path = std::env::temp_dir().join(format!(
        "rustauth-sqlx-create-schema-{}-{}.sql",
        std::process::id(),
        now.as_nanos()
    ));
...
#[tokio::test]
async fn sqlite_adapter_returns_database_generated_serial_ids() -> Result<(), RustAuthError> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
async fn sqlite_adapter_new_enforces_foreign_keys_on_multi_connection_pool(
    ...
    assert!(
        matches!(result, Err(RustAuthError::Adapter(message)) if message.contains("FOREIGN KEY"))
    );
    ...
    assert!(
        matches!(concurrent.0, Err(RustAuthError::Adapter(message)) if message.contains("FOREIGN KEY"))
    );
    assert!(
        matches!(concurrent.1, Err(RustAuthError::Adapter(message)) if message.contains("FOREIGN KEY"))
    );
    Ok(())
}

#[tokio::test]
async fn sqlite_connect_enables_foreign_keys_for_pooled_connections() -> Result<(), RustAuthError> {
    let database_path = std::env::temp_dir().join(format!(
        "rustauth_sqlx_fk_{}_{}.db",
        std::process::id(),
        OffsetDateTime::now_utc().unix_timestamp_nanos()
    ));
    let database_url = format!("sqlite://{}?mode=rwc", database_path.to_string_lossy());
...
#[tokio::test]
async fn sqlite_rate_limit_store_allows_exactly_one_concurrent_request() -> Result<(), RustAuthError>
{
    let adapter = adapter().await?;
    let store = SqliteRateLimitStore::from(&adapter);
    let rule = RateLimitRule {
...
async fn sqlite_rate_limit_store_serializes_concurrent_consumes_on_multi_connection_pool(
    ...
    let database_path = std::env::temp_dir().join(format!(
        "rustauth_sqlx_rate_limit_{}_{}.db",
        std::process::id(),
        OffsetDateTime::now_utc().unix_timestamp_nanos()
    ));
    let database_url = format!("sqlite://{}?mode=rwc", database_path.to_string_lossy());
...
#[tokio::test]
async fn sqlite_rate_limit_store_uses_physical_column_names() -> Result<(), RustAuthError> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
#[tokio::test]
async fn sqlite_rate_limit_store_rejects_negative_persisted_counts() -> Result<(), RustAuthError> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
#[tokio::test]
async fn sqlite_adapter_run_migrations_applies_plugin_aware_schema() -> Result<(), RustAuthError> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
#[tokio::test]
async fn sqlite_adapter_run_plugin_migrations_executes_sql_bodies() -> Result<(), RustAuthError> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
#[tokio::test]
async fn sqlite_adapter_compile_migrations_returns_semicolon_for_noop() -> Result<(), RustAuthError>
{
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
...
#[tokio::test]
async fn sqlite_adapter_plan_migrations_warns_for_foreign_key_mismatch() -> Result<(), RustAuthError>
{
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
...
#[tokio::test]
async fn sqlite_adapter_uses_physical_names_from_auth_schema() -> Result<(), RustAuthError> {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
#[tokio::test]
async fn sqlite_adapter_rolls_back_failed_transactions() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn sqlite_hooked_adapter_preserves_native_transaction_rollback() -> Result<(), RustAuthError>
{
...
#[tokio::test]
async fn sqlite_adapter_supports_where_operators() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn sqlite_adapter_supports_forward_reverse_and_limited_joins() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn sqlite_adapter_supports_multi_joins_inside_transactions() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
    seed_join_user(&adapter, "user_1", "ada@example.com").await?;
    seed_join_account(&adapter, "account_1", "user_1").await?;
    seed_join_session(&adapter, "session_1", "user_1").await?;
...
fn router(adapter: Arc<SqliteAdapter>) -> Result<AuthRouter, RustAuthError> {
...
#[allow(dead_code)]
fn signed_session_cookie(token: &str) -> Result<String, RustAuthError> {
