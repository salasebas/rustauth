...
fn sqlite_record(row: sqlx::sqlite::SqliteRow) -> Result<RateLimitRecord, RustAuthError> {
    Ok(RateLimitRecord {
        key: String::new(),
    ...
    fn run_plugin_migrations<'a>(
        ...
        Box::pin(async move {
            for migration in migrations {
                execute_plugin_migration_sqlite(&self.pool, migration).await?;
            }
            Ok(())
        })
    }
...
async fn execute_plugin_migration_sqlite(
    pool: &SqlitePool,
...
impl<'tx> SqliteTxAdapter<'tx> {
    async fn state<'a>(&'a self) -> Result<SqliteState<'a, 'tx>, RustAuthError> {
