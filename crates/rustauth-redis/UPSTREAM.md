# Upstream parity — rustauth-redis
...
| Shared connection bundle | ✅ High | `RedisRustAuthStores` — one `ConnectionManager` for both stores |
| Cross-adapter wire format | ✅ High | Byte-compatible with `rustauth-fred` on same Redis instance |
...
| Auto RL when secondary configured | ✅ High | `RedisRustAuthStores::apply_to_options` wires secondary storage and distributed RL together; core default policy remains explicit |
| Session payload interchange | ➖ Out of scope | Logical keys and JSON live in `rustauth-core`; this crate stores opaque strings only |
...
| Secondary-storage server flows | — | 4 `it()` | `packages/better-auth/src/db/secondary-storage.test.ts` (covered in `rustauth-fred` E2E) |
| Rate-limit middleware + storage mode | — | ~6 relevant | `rate-limiter.test.ts` + `create-context.test.ts` (middleware in `rustauth-core`) |
...
| `packages/core/src/db/type.ts` (`SecondaryStorage`) | `rustauth-core` `SecondaryStorage` trait → `src/secondary.rs` |
...
- [Sibling `rustauth-fred`](../rustauth-fred/UPSTREAM.md) — same contract, `fred` client
