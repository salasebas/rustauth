...
use rustauth_redis::{
    RedisRateLimitStore, RedisSecondaryStorage, RedisSecondaryStorageOptions, RedisStores,
};
use time::Duration;
...
fn redis_targets() -> Vec<RedisTestTarget> {
    redis_targets_from_env(
        std::env::var("RUSTAUTH_REDIS_URL").ok(),
        std::env::var("RUSTAUTH_VALKEY_URL").ok(),
    )
}
...
async fn redis_secondary_storage_supports_get_set_delete_list_and_clear(
    ...
    for target in available_redis_targets().await? {
        let prefix = format!("rustauth:test:{}:storage:", target.name);
...
#[tokio::test]
async fn redis_secondary_storage_set_if_not_exists_ttl_zero_is_non_destructive(
) -> Result<(), Box<dyn std::error::Error>> {
    for target in available_redis_targets().await? {
        let storage = RedisSecondaryStorage::connect_with_options(
            &target.url,
...
#[tokio::test]
async fn redis_rustauth_stores_share_one_connection_manager(
) -> Result<(), Box<dyn std::error::Error>> {
    for target in available_redis_targets().await? {
        let stores = RedisStores::connect(&target.url).await?;
        let key = format!(
            "bundle:{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)?
