            ...
            .allowed_media_types(["application/scim+json", "application/json"]),
        {
            ...
            move |context, request| {
                ...
                async move {
                    ...
                    .await?
                    .ok_or_else(|| {
                        RustAuthError::Adapter("created SCIM group is missing".to_owned())
                    })?;
            ...
            .allowed_media_types(["application/scim+json", "application/json"]),
        {
            ...
            move |context, request| {
                ...
                async move {
                    ...
                    .await?
                    .ok_or_else(|| {
                        RustAuthError::Adapter("updated SCIM group is missing".to_owned())
                    })?;
                    scim_json_projected_with_etag(
                        StatusCode::OK,
                        &resource,
            ...
            move |context, request| {
                ...
                async move {
                    ...
                    if !body.schemas.iter().any(|schema| schema == PATCH_OP_SCHEMA) {
                        return ScimError::bad_request("Invalid schemas for PatchOp")
                            .into_response();
                    }
                    ...
                    .await?
                    .ok_or_else(|| {
                        RustAuthError::Adapter("patched SCIM group is missing".to_owned())
                    })?;
                    scim_json_projected_with_etag(
                        StatusCode::OK,
                        &resource,
