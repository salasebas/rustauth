            ...
            .allowed_media_types(["application/json"]),
        {
            ...
            move |context, request| {
                ...
                async move {
                    ...
                    let Some(user) = current_user(&context, adapter.as_ref(), &request).await?
                    else {
                        return json_error(
                    ...
                    let member = if let Some(organization_id) = body.organization_id.as_deref() {
                        let Some(member) =
                            organization_member(adapter.as_ref(), organization_id, &user.id)
                                .await?
                        else {
                            return json_error(
                                StatusCode::FORBIDDEN,
                    ...
                    let stored_token =
                        store_scim_token(&context.secret, &options.token_storage, &base_token)
                            .await?;
                    let provider = store
