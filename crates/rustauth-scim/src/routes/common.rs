...
pub(super) fn scim_json_projected_with_etag<T: Serialize>(
    ...
    let mut body =
        serde_json::to_value(body).map_err(|error| RustAuthError::Api(error.to_string()))?;
...
pub(super) fn hook_error(error: ScimHookError) -> Result<ApiResponse, RustAuthError> {
...
pub(super) fn team_from_record(record: DbRecord) -> Result<ScimTeamRecord, RustAuthError> {
    Ok(ScimTeamRecord {
...
pub(super) fn required_string<'a>(
    ...
    match record.get(field) {
        ...
        None => Err(RustAuthError::Adapter(format!("user is missing `{field}`"))),
    }
}

pub(super) fn required_bool(record: &DbRecord, field: &str) -> Result<bool, RustAuthError> {
    match record.get(field) {
