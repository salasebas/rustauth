# Changelog

## [0.3.1](https://github.com/salasebas/rustauth/compare/v0.3.0...v0.3.1) - 2026-07-01

### Fixed

- fix scim filter complexity limits ([#177](https://github.com/salasebas/rustauth/pull/177))

## [0.2.0] - 2026-06-14

Initial public working release.

### Added

- SCIM 2.0 provisioning plugin: users, groups, bulk operations, filtering, patching, and metadata routes.
- Token handling, schema mappings, and adapter conformance coverage.
- `test-util` feature for integration-test helpers.

[0.2.0]: https://github.com/salasebas/rustauth/releases/tag/v0.2.0
