...
fn test_rustauth_options(plugins: Vec<rustauth_core::plugin::AuthPlugin>) -> RustAuthOptions {
...
pub(super) fn router() -> Result<AuthRouter, rustauth_core::error::RustAuthError> {
    router_with_adapter().map(|(_adapter, router)| router)
}

pub(super) fn router_with_adapter(
) -> Result<(Arc<MemoryAdapter>, AuthRouter), rustauth_core::error::RustAuthError> {
...
pub(super) fn router_with_context(
    ...
    rustauth_core::error::RustAuthError,
> {
    let adapter = Arc::new(MemoryAdapter::new());
    let context = create_auth_context_with_adapter(
        test_rustauth_options(vec![scim(options)]),
        adapter.clone(),
    )?;
    let router =
...
> {
    let adapter = Arc::new(MemoryAdapter::new());
    let context = create_auth_context_with_adapter(
        test_rustauth_options(vec![organization(organization_options), scim(options)]),
        adapter.clone(),
    )?;
    let router =
        AuthRouter::with_async_endpoints(context.clone(), Vec::new(), core_auth_async_endpoints())?;
