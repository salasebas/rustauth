...
async fn groups_route_rejects_duplicate_external_id_for_same_provider() {
    ...
    let profiles = adapter
        .find_many(
            rustauth_core::db::FindMany::new("scim_group_profile")
                .where_clause(rustauth_core::db::Where::new(
                    "provider_id",
                    rustauth_core::db::DbValue::String("okta".to_owned()),
                ))
                .where_clause(rustauth_core::db::Where::new(
                    "external_id",
                    rustauth_core::db::DbValue::String("eng".to_owned()),
                ))
                .select(["team_id"]),
        )
        .await
    ...
    let teams = adapter
        .find_many(
            rustauth_core::db::FindMany::new("team")
                .where_clause(rustauth_core::db::Where::new(
                    "organization_id",
                    rustauth_core::db::DbValue::String("org_1".to_owned()),
                ))
                .where_clause(rustauth_core::db::Where::new(
                    "name",
                    rustauth_core::db::DbValue::String("Engineering".to_owned()),
                ))
                .select(["id"]),
        )
        .await
