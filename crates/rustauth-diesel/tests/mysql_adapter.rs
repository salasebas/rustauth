...
#[path = "../../../tests/support/diesel_migration_atomicity_mysql.rs"]
mod diesel_migration_atomicity_mysql;
...
use rustauth_core::db::{MigrationStatementKind, SchemaMigrationWarning};
use rustauth_core::error::RustAuthError;
use rustauth_core::options::{
    AdvancedOptions, RateLimitConsumeInput, RateLimitRule, RateLimitStore, RustAuthOptions,
};
...
use rustauth_core::test_utils::fast_verify_password;
use rustauth_diesel::{DieselMysqlAdapter, DieselMysqlRateLimitStore};
...
async fn connect_adapter(schema: DbSchema) -> Result<DieselMysqlAdapter, RustAuthError> {
    let database_url = database_url();
    DieselMysqlAdapter::connect_with_schema(&database_url, schema)
        .await
        .map_err(|error| common::preflight_error("diesel mysql", &database_url, error))
}

async fn verification_pool(max_connections: u32) -> Result<sqlx::MySqlPool, RustAuthError> {
    let database_url = database_url();
...
#[test]
fn database_url_allows_mysql_env_override() {
    assert_eq!(
        database_url_from_env(Some("mysql://custom.example.test/db".to_owned())),
        "mysql://custom.example.test/db"
    );
}

async fn adapter() -> Result<DieselMysqlAdapter, RustAuthError> {
    let schema = test_schema();
    let adapter = connect_adapter(schema.clone()).await?;
    adapter.create_schema(&schema, None).await?;
    Ok(adapter)
}

#[tokio::test]
async fn mysql_adapter_migration_plan_rolls_back_on_statement_failure() -> Result<(), RustAuthError>
{
    let pool = verification_pool(1).await?;
    let schema = test_schema();
    let adapter = connect_adapter(schema.clone()).await?;
    diesel_migration_atomicity_mysql::assert_diesel_mysql_migration_plan_rolls_back(
        &adapter, &pool, &schema,
    )
    .await
}
...
#[tokio::test]
async fn mysql_adapter_satisfies_shared_adapter_contract() -> Result<(), RustAuthError> {
...
fn unique_prefix() -> String {
    ...
    format!("oa_my_{process:x}_{:08x}_{sequence:x}", nanos & 0xffff_ffff)
}

#[tokio::test]
async fn mysql_adapter_filters_sorts_limits_and_counts_records() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_adapter_treats_like_wildcards_as_literal_user_input() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_adapter_create_schema_writes_requested_schema_file() -> Result<(), RustAuthError> {
    let schema = test_schema();
    let pool = verification_pool(1).await?;
    let adapter = connect_adapter(schema.clone()).await?;
    let prefix = unique_prefix();
    let path = std::env::temp_dir().join(format!("{prefix}-mysql-schema.sql"));
    let path_string = path.to_string_lossy().into_owned();

    let creation = adapter
        .create_schema(&schema, Some(&path_string))
        .await?
        .ok_or_else(|| RustAuthError::Adapter("missing schema creation metadata".to_owned()))?;
    let code = tokio::fs::read_to_string(&path)
        .await
        .map_err(|error| RustAuthError::Adapter(format!("failed to read schema file: {error}")))?;
    let users_table_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?",
    )
    .bind(schema.table_name("user")?)
    .fetch_one(&pool)
    .await
...
#[tokio::test]
async fn mysql_adapter_updates_and_deletes_matching_records() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
    ...
    let updated = adapter
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter("missing updated session".to_owned()))?;
    let deleted = adapter
        .delete_many(
...
#[tokio::test]
async fn mysql_adapter_reports_public_capabilities() -> Result<(), RustAuthError> {
    let adapter = DieselMysqlAdapter::connect(&database_url())
        .await
        .map_err(|error| common::preflight_error("diesel mysql", &database_url(), error))?;
    ...
    assert_eq!(
        capabilities,
        AdapterCapabilities::new("diesel-mysql")
            .named("Diesel MySQL")
            .with_json()
            .with_arrays()
            .with_native_joins()
...
#[tokio::test]
async fn mysql_adapter_returns_database_generated_serial_ids() -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
...
async fn mysql_rate_limit_store_rejects_invalid_rules_before_database_access(
    ...
    let adapter = DieselMysqlAdapter::connect(&database_url()).await?;
    let store = DieselMysqlRateLimitStore::from(&adapter);
    sql_rate_limit_rule_validation::assert_sql_rate_limit_store_rejects_invalid_rules(&store).await
}

#[tokio::test]
async fn mysql_rate_limit_store_allows_exactly_one_concurrent_request() -> Result<(), RustAuthError>
{
    let adapter = adapter().await?;
    let store = DieselMysqlRateLimitStore::from(&adapter);
...
#[tokio::test]
async fn mysql_rate_limit_store_rejects_negative_persisted_counts() -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    let rate_limits_table = format!("{prefix}_rate_limits");
    let schema = auth_schema(AuthSchemaOptions {
        rate_limit_storage: RateLimitStorage::Database,
    ...
    assert!(
        matches!(result, Err(RustAuthError::Adapter(message)) if message.contains("negative rate limit count"))
    );
    Ok(())
}

#[tokio::test]
async fn mysql_rate_limit_store_uses_physical_column_names() -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    ...
    let stored_count: i64 =
        sqlx::query_scalar(&format!("SELECT rl_count FROM {table} WHERE rl_key = ?"))
            .bind(key)
            .fetch_one(&pool)
            .await
            .map_err(sql_error)?;
...
#[tokio::test]
async fn mysql_adapter_plan_migrations_reports_empty_database_tables_in_order(
) -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    let schema = auth_schema(AuthSchemaOptions {
        user: table_options(&prefix, "users"),
...
#[tokio::test]
async fn mysql_adapter_plan_migrations_reports_plugin_columns_indexes_and_sql(
) -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    ...
    let adapter = connect_adapter(base_schema.clone()).await?;
    ...
    assert_eq!(
        ...
        format!("idx_{prefix}_users_tenant_id")
    );
...
#[tokio::test]
async fn mysql_adapter_plan_migrations_reports_plugin_tables_with_deferred_indexes(
) -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    ...
    assert!(plan
        .to_be_created
        .iter()
        .any(|table| table.table_name == plugin_table));
    assert!(plan
        .indexes_to_be_created
        .iter()
        .any(|index| index.index_name == format!("idx_{plugin_table}_external_id")));
    let table_position = plan
        .statements
        .iter()
        .position(|statement| statement.sql.contains(&plugin_table))
        .ok_or_else(|| RustAuthError::Adapter("missing plugin table statement".to_owned()))?;
    let index_position = plan
        ...
        .position(|statement| {
            statement
                .sql
                .contains(&format!("idx_{plugin_table}_external_id"))
        })
        .ok_or_else(|| RustAuthError::Adapter("missing plugin index statement".to_owned()))?;
    assert!(table_position < index_position);
    Ok(())
}

#[tokio::test]
async fn mysql_adapter_compile_migrations_returns_semicolon_for_noop() -> Result<(), RustAuthError>
{
    let schema = test_schema();
    let adapter = connect_adapter(schema.clone()).await?;
...
#[tokio::test]
async fn mysql_adapter_plan_migrations_warns_for_type_mismatch_without_rewrite(
) -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    ...
    assert!(plan.warnings.iter().any(|warning| matches!(
        ...
        SchemaMigrationWarning::ColumnTypeMismatch {
            ...
            && actual.eq_ignore_ascii_case("bigint")
    )));
    assert!(!sql.contains("ALTER COLUMN"));
    assert!(!sql.contains("DROP"));
    assert!(!sql.contains("RENAME"));
...
async fn mysql_adapter_run_migrations_rejects_type_warnings_without_applying_statements(
    ...
    assert!(
        matches!(result, Err(RustAuthError::Adapter(message)) if message.contains("non-executable migration warnings"))
    );
...
async fn mysql_adapter_create_schema_rejects_type_warnings_without_applying_statements(
    ...
    assert!(
        matches!(result, Err(RustAuthError::Adapter(message)) if message.contains("migration warnings"))
    );
    let sessions_table_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?",
    )
...
#[tokio::test]
async fn mysql_adapter_plan_migrations_warns_for_foreign_key_mismatch() -> Result<(), RustAuthError>
{
    let prefix = unique_prefix();
    let users_table = format!("{prefix}_users");
    let sessions_table = format!("{prefix}_sessions");
...
#[tokio::test]
async fn mysql_adapter_run_plugin_migrations_executes_sql_bodies() -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
...
#[tokio::test]
async fn mysql_adapter_run_migrations_adds_plugin_columns_to_existing_tables(
) -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    let base_schema = auth_schema(AuthSchemaOptions {
        user: table_options(&prefix, "users"),
    ...
    let tenant_column_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = ? AND column_name = 'tenant_id'",
    )
    .bind(&users_table)
    .fetch_one(&pool)
    .await
    ...
    .bind(&users_table)
    .bind(format!("idx_{users_table}_tenant_id"))
    .fetch_one(&pool)
    .await
    .map_err(sql_error)?;
...
#[tokio::test]
async fn mysql_adapter_run_migrations_repairs_missing_indexes_on_existing_columns(
) -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    let users_table = format!("{prefix}_users");
    let mut schema = auth_schema(AuthSchemaOptions {
    ...
    adapter.run_migrations(&schema).await?;
    sqlx::query(&format!(
        "DROP INDEX idx_{users_table}_tenant_id ON {users_table}"
    ))
    .execute(&pool)
    .await
    .map_err(sql_error)?;
...
#[tokio::test]
async fn mysql_adapter_run_migrations_creates_plugin_tables_with_indexes_and_foreign_keys(
) -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    let users_table = format!("{prefix}_users");
    let plugin_table = format!("{prefix}_plugin_identities");
    ...
    .bind(&plugin_table)
    .bind(format!("idx_{plugin_table}_external_id"))
    .fetch_one(&pool)
    .await
    .map_err(sql_error)?;
    let fk_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM information_schema.referential_constraints rc JOIN information_schema.key_column_usage kcu ON rc.constraint_schema = kcu.constraint_schema AND rc.constraint_name = kcu.constraint_name WHERE rc.constraint_schema = DATABASE() AND kcu.table_name = ? AND kcu.column_name = 'user_id' AND rc.delete_rule = 'CASCADE'",
    )
    .bind(&plugin_table)
    .fetch_one(&pool)
    .await
...
#[tokio::test]
async fn mysql_adapter_uses_physical_names_from_auth_schema() -> Result<(), RustAuthError> {
    let prefix = unique_prefix();
    let schema = auth_schema(AuthSchemaOptions {
    ...
    let stored_email: String = sqlx::query_scalar(&format!(
        "SELECT primary_email FROM {prefix}_app_users LIMIT 1"
    ))
    .fetch_one(&pool)
    .await
    .map_err(sql_error)?;
...
#[tokio::test]
async fn mysql_adapter_rolls_back_failed_transactions() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
    ...
    let result = adapter
        .transaction(Box::new(|tx| {
            Box::pin(async move {
                ...
                .await?;
                Err(RustAuthError::Adapter("force rollback".to_owned()))
            })
        }))
        .await;
...
#[tokio::test]
async fn mysql_adapter_supports_where_operators() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
...
#[tokio::test]
async fn mysql_adapter_supports_forward_reverse_and_limited_joins() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
    ...
    let user = adapter
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter("missing joined user".to_owned()))?;
    ...
    let account = adapter
        ...
        .await?
        .ok_or_else(|| RustAuthError::Adapter("missing joined account".to_owned()))?;
...
#[tokio::test]
async fn mysql_adapter_supports_multi_joins_inside_transactions() -> Result<(), RustAuthError> {
    let adapter = adapter().await?;
    seed_join_user(&adapter, "user_1", "ada@example.com").await?;
    seed_join_account(&adapter, "account_1", "user_1").await?;
    seed_join_session(&adapter, "session_1", "user_1").await?;
            ...
            move |tx| {
                Box::pin(async move {
                    ...
                    *joined_records.lock().map_err(|_| {
                        RustAuthError::Adapter("joined records lock poisoned".to_owned())
                    })? = users;
                    Ok(())
                })
            }
    ...
    let users = joined_records
        .lock()
        .map_err(|_| RustAuthError::Adapter("joined records lock poisoned".to_owned()))?;
...
#[tokio::test]
async fn mysql_hooked_adapter_preserves_native_transaction_rollback() -> Result<(), RustAuthError> {
        ...
        vec![
            ...
            PluginDatabaseHook::after_create("after-create", {
                ...
                move |_context, _query, _result| {
                    events
                        .lock()
                        .map_err(|_| RustAuthError::Adapter("events lock poisoned".to_owned()))?
                        .push("after".to_owned());
                    Ok(())
                }
            }),
...
#[tokio::test]
async fn mysql_adapter_supports_core_auth_route_flows() -> Result<(), Box<dyn std::error::Error>> {
...
#[tokio::test]
async fn mysql_adapter_supports_password_reset_verifications(
) -> Result<(), Box<dyn std::error::Error>> {
    let adapter = Arc::new(adapter().await?);
    let router = router(adapter.clone())?;
...
fn router(adapter: Arc<DieselMysqlAdapter>) -> Result<AuthRouter, RustAuthError> {
...
fn sql_error(error: sqlx::Error) -> RustAuthError {
