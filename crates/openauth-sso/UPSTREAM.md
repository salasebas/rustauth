...
`openauth-sso` is the Better Auth `sso` plugin surface in Rust: provider CRUD,
...
cargo nextest run -p openauth-sso --features saml --test sso   # SAML routes
```
...
| --- | --- | --- | --- |
| Crate split | Single `@better-auth/sso` package | `openauth-sso` + `openauth-oidc` + `openauth-saml` | Idiomatic Rust modules; reuse helpers without plugin coupling |
| Re-exports | N/A | `openauth_sso::oidc`, `openauth_sso::saml` under features | Convenience for plugin users; parity audited per sibling crate |
...
| `src/oidc/` | [`openauth-oidc`](../openauth-oidc/UPSTREAM.md); re-exported as `openauth_sso::oidc` |
| `src/saml/`, `samlify.ts` | [`openauth-saml`](../openauth-saml/UPSTREAM.md); re-exported as `openauth_sso::saml` |
...
- [openauth-oidc UPSTREAM](../openauth-oidc/UPSTREAM.md) — discovery and OIDC types
- [openauth-saml UPSTREAM](../openauth-saml/UPSTREAM.md) — SAML helpers and crypto
