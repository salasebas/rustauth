...
pub async fn run(context: &AppContext, args: InfoArgs) -> Result<(), AppError> {
    ...
    let info = InfoReport {
        openauth: OpenAuthInfo {
            cli_version: report.openauth_version.clone(),
        },
...
fn print_human(info: &InfoReport) {
    ...
    println!("CLI: {}", info.openauth.cli_version);
    ...
    println!(
        ...
        if info.config_loaded {
            "loaded from openauth.toml"
        } else {
            "defaults (no openauth.toml)"
        }
    );
